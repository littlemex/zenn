...
まず、何が安くなって何が安くなっていないのかを分けます。文書解析の代表的なベンチマークである OmniDocBench では、上位モデルのスコアが 90 点台に密集しています。専用ツールも汎用 VLM も横並びで高得点を出す。単純な文字認識の精度は、たしかにコモディティ化しました。
...
長い明細を持つ文書に対して、汎用 VLM は途中で処理を打ち切ったり、行を取りこぼしたりします。ある大規模抽出のベンチマークでは、フロンティア級の VLM が数百件の文書のうち半分程度しか完走できなかったと報告されています。表のセルは正しく読めても構造的に誤った位置へ置いてしまい、その失敗が下流から見えない、という指摘もあります。
...
第一に、生成的な LLM を判定器に使うのは脆い。生成的な報酬モデルは、記号一つや定型的な書き出しだけで検証をすり抜けて高い報酬を出してしまう例が報告されています。複数モデルの合議でも相当数の誤答を通してしまう。帳票の合計整合やチェックデジットは、確率的な判定器ではなく決定論的なルールエンジンで判定すべきです。ニューラルな判定器より決定論ルールの方が報酬ハッキングに強い、という向きは複数の研究が支持しています。
...
on-policy distillation は、生徒モデル自身が生成した出力に対して、教師モデルがトークンごとに採点し、その差で生徒を更新する手法です。教師のデータをそのまま真似るのではなく、生徒が「自分が実際にとる行動」の上で教師の判断を学ぶので、訓練分布から外れて迷子になりにくい。報酬が疎な強化学習に比べてトークンごとに密な信号が得られるため、数学タスクでの報告では、強化学習の10分の1程度の計算量で近い性能に届くとされています。

文書理解でも、大型モデルを強化学習で先に鍛え、それを教師に小型モデルへ蒸留するパイプラインが登場し、小型モデルが主要ベンチマークで上位に入った例が報告されています。ただし蒸留単独でどれだけ効いたかは、報告からは切り分けられていません。効果分離は今後の検証課題です。

一つ、経験的に言われている注意点があります。教師が生徒の届かない「新しい能力」を持っていないと蒸留は効きにくい、という指摘です。汎用 VLM のゼロショット出力をそのまま蒸留するだけでは足りず、教師側に強化学習で鍛えた挙動や多段の推論を仕込む必要があると考えられます。

実務上の制約も正直に書いておきます。外部の商用 VLM の出力を蒸留の教師に使うことには、利用規約上の制限が伴います。ある大手提供者は、承認のないモデル蒸留を明確に禁止しています。この点でも、外部モデルに直接寄りかかるより、オープンウェイトモデルと公開データセットから自社の教師を立ち上げる方が安全です。

## 拡散言語モデルは OCR に効くのか

先に結論を言うと、拡散言語モデル(複数トークンを並列に生成できる非自己回帰型のモデル)は、今すぐの本命ではありません。それでも見ておく価値があるのは、前章のフライホイールが、この選択肢を「安く保有できるオプション」に変えるからです。

現状、拡散型 VLM は細かい文字認識を要するタスクで自己回帰型に大きく負けています。もっとも原因は拡散という仕組みではなく、画像特徴を圧縮するときに空間情報を捨てている点にある、と論文自身が特定しています。実際、視覚エンコーダの差し替えとデータの改善で差が大きく縮んだ報告もあります(ただし差はなお残ります)。

事業として賭ける意味があるかは、精度ではなくスループットあたりのコストで決まります。ここは慎重に見る必要があります。拡散モデルの速度優位はバッチサイズが小さいときの話で、バッチを大きく詰め込むと自己回帰型が逆転する、という解析があるからです。OCR のようにレイテンシがゆるくバッチが効く用途では、この解析はむしろ拡散に不利に働きます。

面白いのは、拡散モデルへの on-policy distillation が、2026年に立ち上がったばかりの領域だという点です。強化学習で拡散モデルを直接鍛えると固有の難しさに突き当たりますが、蒸留の枠組みなら教師と生徒を同じ条件で直接比べればよく、その難しさを回避できることが複数の研究で示唆されています。前章のフライホイールはデコーダの方式に依存しないので、それを先に作っておけば、この路線は「観測が揃ったら行使すればいいオプション」として残せます。
