...
本記事では、NxD Inference を使って XTTS v2 の GPT Transformer 部分を Trainium で動かすまでの実装を、7 ステップで解説します。
...
NxD に新しいモデルを組み込む際の最重要イタンフェースです。2 つのメソッドを実装します。
...
`nn.Module` について、コンパイルについては前回解説したのでそちらを確認してください。

**aliases が KV-Cache の核心**です。`{Parameter: 出力インデックス}` という辞書を渡すことで、NxD Inference は「この `nn.Parameter`（= KV-Cache のバッファ）を、forward の出力テンソルの N 番目で上書きせよ」という配線を Neuron コンパイラレベルで自動生成します。
...
`input_generator()` が返すテンソルの**形状と dtype** がそのままコンパイルされたモデルの入力仕様になります。ここを変えると別のモデルとして再コンパイルが必要になります。
...
`NeuronApplicationBase.compile()` の内部フローは以下の通りです。

```mermaid
flowchart LR
    A["compile(path) 呼び出し"] --> B["self.models を反復"]
    B --> C["wrapper.get_model_instance()\n= NeuronGPTInstance"]
    C --> D["instance.load_module()\nNeuronGPTTransformer を生成"]
    D --> E["instance.get()\nmodule + aliases を取得"]
    E --> F["wrapper.input_generator()\nサンプル入力を生成"]
    F --> G["torch.jit.trace(module, sample_input)\naliases を Neuron 配線に変換"]
    G --> H["neff ファイルとして保存\nGPTPrefill.neff / GPTDecode.neff"]
```

`load()` は逆に neff ファイルを読み込んで `self.traced_model` に格納します。
...
**ポイント**: `torch.scatter` は `cache_k` を直接書き換えるのではなく、新しいテンソルを返します。Aliases（後述）により、この出力が次のステップで `cache_k` として自動的に使われます。
...
:::message alert
**落とし穴**: `self.models = [prefill_model, decode_model]` としても、`self.traced_model` はインデックスアクセス不可の単一オブジェクトです。AWS Neuron の Whisper 実装（エンコーダー/デコーダー分離）と同じパターンで、**Prefill と Decode を完全に独立した `NeuronApplicationBase` サブクラスに分離**する必要があります。
:::
...
|-------------------|-----------|--------------------------|-----------|
| `h.{i}.attn.c_attn` | `[1024, 3072]` | `blocks.{i}.attn.{query,key,value}` | `[1024, 1024]`（.T後） |
| `h.{i}.attn.c_proj` | `[1024, 1024]` | `blocks.{i}.attn.out` | `[1024, 1024]`（.T後） |
| `h.{i}.mlp.c_fc` | `[1024, 4096]` | `blocks.{i}.mlp.up_proj` | `[4096, 1024]`（.T後） |
| `h.{i}.mlp.c_proj` | `[4096, 1024]` | `blocks.{i}.mlp.down_proj` | `[1024, 4096]`（.T後） |
...
```
[OK] CPU:    9.42秒, 音声長=4.64秒
[OK] Neuron: 3.44秒, 音声長=5.98秒, RTF=1.74x
