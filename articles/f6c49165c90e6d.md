---
title: "OpenAI Whisper モデルを NxD Inference で動かす"
emoji: "🔥"
type: "tech"
topics: ["AWSNeuron", "Whisper", "Trainium", "機械学習", "音声認識"]
published: false
---

**対象読者**: AWS Trainium/Inferentia2 で音声認識 (STT) をサービングしたい方
**前提知識**: AWS Neuron 関連の基礎知識、Transformers ライブラリの基本的な使い方

> 📚 **音声認識 (Speech Recognition) について学ぶ**
> 音声認識の基礎から実装まで体系的に学びたい方は、Hugging Face が提供する無料の学習コース [Audio Course - Chapter 5: Automatic Speech Recognition](https://huggingface.co/learn/audio-course/chapter5/introduction) をおすすめします。Whisper を含む最新の音声認識技術を実践的に学べます。私も音声は無知を極めているため学び始めました。
...
**参考実装**: [`whisper_nxd_model.py`](https://github.com/littlemex/data-science/blob/3fedc61/investigations/neuron-adapter/phase2-nxd-whisper/experiments/whisper_nxd_model.py#L49-L108)
...
**WhisperInferenceConfig の役割**は、上述した `NeuronConfig` と Whisper の `model_config` を統合することです。
...
**初期化処理**
1. `Whisper.__init__(config.dims)` を呼び出し、標準的な Whisper アーキテクチャを構築
2. 標準の `Encoder`/`Decoder` を Neuron Whisper 用に置き換え
...
**パラメータ**:
- `sr=16000`: Whisper の入力は 16kHz サンプリングレートが必須
- `mono=True`: ステレオをモノラルに変換
- `pad_or_trim(mel, 3000)`: 30 秒 (16000 Hz × 30 = 480000 samples → 3000 frames) に固定
  - 短い音声は 0 でパディング
  - 長い音声は切り捨て (チャンク分割が必要)
