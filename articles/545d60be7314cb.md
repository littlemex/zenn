...
実行すると、先ほど見た `linear → relu → add` の FX graph が表形式で出力されます。
...
たとえば `command.txt` を開くと、HLO を入力に NEFF を出力する `neuronx-cc` の呼び出しが
そのまま確認できますし、`log-neuron-cc.txt` を `grep MLIR` すれば、前章で触れた MLIR パスの実行が
見つかります。FX graph から HLO、そして NEFF まで、翻訳のリレーの各段が実ファイルとして手元に
残るので、一つずつ開いて中身を確かめられます。
