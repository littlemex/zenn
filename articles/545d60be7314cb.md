...
ここまでの 3 段で、同じ演算がどう姿を変えるかを 1 つの表にまとめます。同じ演算を各段で
追いかけると、翻訳のリレーが一望できます。
...
一点、先に予告しておきます。段階が進むほど、変数名（`a` や `b`）や元の関数の形は残らなくなります。
HLO 以降は「名前」ではなく「どんな shape のデータに、どの種類の演算がかかるか」で追うことになります。
最適化で演算がまとめられたり分解されたりもするので、1 対 1 の対応が崩れる箇所も出てきます。
...
必要なのは、vLLM Neuron の DLAMI に付属する専用 venv だけです。以下はそのままコピー&ペーストで
再現できます（1 つのシェルセッションで上から順に実行してください）。
...
CPU モードでは、コンパイル backend が FX graph をそのまま実行に返します。そこにフックを
差し込めば、FX graph を表示できます。次のスクリプトをヒアドキュメントで作成して実行します。
...
表の 1 行が 1 ノードです。`opcode` はノードの種類で、`placeholder` が入力（`x` と重み・バイアス）、
`call_function` が演算、`output` が出力です。`target` が呼ばれた関数（`linear` / `relu` / `add`）、
`args` がその入力です。`add` の `args` に `(relu, 1.0)` とある通り、`relu` の結果に `1.0` を足す、という
`forward` の最後の行がそのままノードになっています。書いた Python の 3 行が、演算の依存グラフとして
そっくり取り出せていることが読み取れます。

### HLO と NEFF を生成する

次に、CPU コンパイルモードで HLO と NEFF を実ファイルとして生成します。同じくヒアドキュメントで
スクリプトを作って実行します。`VLLM_NEURON_CPU_COMPILE=1` と `NEURON_PLATFORM_TARGET_OVERRIDE=trn2`
を設定するのがポイントです。
...
実行すると、コンパイルが完了してキャッシュに成果物が保存された、という趣旨のログが出ます
（実機の出力から抜粋）。
...
注目してほしいのは、最初の 3 行で「FX → HLO の変換」と「HLO → NEFF のコンパイル」が完了し、
成果物がキャッシュに保存されている点です。最後の `ERROR` 行は、出来上がった NEFF を NeuronCore に
ロードしようとして失敗したものです。つまりコンパイル（NEFF を作るところ）までは CPU で完走し、
その NEFF を「実行」する段でだけ NeuronCore が要る、ということがログからも読み取れます。CPU で
内部を観察するのが目的なので、この段のエラーは無視して構いません。
...
コンパイルが終わると、`VLLM_CACHE_ROOT`（ここでは `/tmp/artifact_demo`）の下に各段階の成果物が
実ファイルで残ります。次のコマンドで一覧できます。
...
`--framework XLA`（入力は XLA 由来の HLO）、`--target trn2`（Trainium2 向け）、`--output ...neff`
（出力は NEFF）と読めます。前章で説明した「HLO が入力、NEFF が出力」がそのまま引数に現れています。

さらに `log-neuron-cc.txt` を `grep -iE "Pipeline|MLIR"` すると、コンパイラ内部のパイプラインと
MLIR パスの実行が見つかります。
...
1 行目が前章のパイプライン（`HLOToTensorizer → Frontend → StaticIOTranspose → WalrusDriver →
NeffWrapper`）そのもの、2 行目が `hlo2penguin` による HLO の変換、3 行目が MLIR パスの一つ
（RemoveOptimizationBarriers）の実行です。「MLIR は neuronx-cc の内部で使われている」という話が、
憶測ではなくログの実物として確認できます。

FX graph から HLO、そして NEFF まで、翻訳のリレーの各段が実ファイルとして手元に残るので、
一つずつ開いて中身を確かめられます。

ひとつ正直に補足します。ここで CPU だけで作れるのは NEFF という成果物までです。その NEFF を
実際に「実行」するには、NeuronCore にロードして走らせる Neuron Runtime が必要で、これは Trainium
実機の上でしか動きません。つまり、この記事の観察は「バイナリが出来上がるまで」を CPU で追う体験で、
出来上がったバイナリを動かすところからは実機の出番になります。
...
この記事では極小モデルで流れを追いましたが、実際の LLM でも通る道は同じです。上の観察手順は
CPU モードで NeuronCore を使わずに動くので、中間表現がどのように姿を変えていくかを、ぜひ自分の
手元で確かめてみてください。

なお、v0.21 の各機能を実機で一通り動かした記録は前回の記事「vLLM Neuron プラグイン v0.21 を
Trainium で実際に動かして分かったこと」にまとめており、そこで使った検証コードは
