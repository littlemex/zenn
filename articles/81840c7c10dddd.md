...
- `aliases` の各エントリ（cache_k: 1..30、cache_v: 31..60）が return タプルの範囲内に収まることが確認できます
...
```python
# compile 時: config.checkpoint_path は "" のまま -> ゼロ初期化 state dict を返す
app.compile("compiled_path")

# load_weights 時: 呼び出し前にパスを注入する
config.checkpoint_path = "model.pth"
...
def get_state_dict(cls, model_name_or_path, config):
    ckpt_path = getattr(config, "checkpoint_path", "")
    if ckpt_path:
...
Section 2 で説明した 2 点を確認します。(1) `getattr(config, "xttsv2_checkpoint_path", "")` が `None` ではなく `""` を返すこと。(2) `NeuronConfig.seq_len` / `InferenceConfig.max_seq_len` / `input_generator()` が生成する入力形状の 3 値がすべて 1081 で一致していること。

期待される結果と理由。

- compile 時は `xttsv2_checkpoint_path = ""` → `bool("")` が `False` → `get_state_dict()` はゼロ初期化の state dict を返す
- load_weights 時は属性にパスが注入され `bool(path)` が `True` → 実際の重みが読み込まれる
- `max_seq_len = 605 + 2 + 402 + 2 + 70 = 1081`（音声・テキスト・プロンプトトークンの上限と区切り）が `NeuronConfig.seq_len` と一致していることを assert で検証。不一致はコンパイル時の形状不一致エラーとして現れます
...
aliases 機構は「forward() の return タプルの出力インデックス」と「`nn.Parameter` の Python オブジェクト ID」を紐付け、コンパイラが forward の return 値を自動的に対応するパラメータに書き戻す仕組みです（概要セクションの図および Section 4 のシーケンス図を参照）。キャッシュテンソルを aliases に登録するためには `nn.Parameter` として定義する必要があります（通常の `torch.Tensor` 属性は Python オブジェクト ID での識別が保証されないため不可）。

```python
self.cache_k = nn.Parameter(
    torch.zeros(batch_size, n_kv_heads, seq_len, head_dim, dtype=dtype),
    requires_grad=False
)
```
...
`.contiguous()` は必須。`expand()` は non-contiguous テンソルを返すため、scatter の index に渡すとコンパイルエラーになります。aliases が「この出力テンソルを `cache_k` パラメータに書き戻せ」とコンパイラに伝え、次ステップでは自動的に最新のキャッシュ値が `self.cache_k` に入っています。

::::details [動作確認] torch.scatter による KV-Cache 更新と expand().contiguous() の挙動を確認する（CPU 実行）

**参照ファイル**: [`src/neuron_xttsv2/modeling_gpt.py`](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/modeling_gpt.py)

このコードが確認すること。

`torch.scatter` によるキャッシュ更新パターン（Decode ステップ 1 と 2 の連続更新）、`expand().contiguous()` の必要性、Prefill での複数位置への一括書き込みを確認します。

期待される結果と理由。

- `expand()` は `is_contiguous() = False` を返す（stride=0 の非連続テンソル）。`.contiguous()` を呼ぶと `True` になる。scatter の index に非連続テンソルを渡すとコンパイルエラーになるため `.contiguous()` が必須です
- `torch.scatter` は元テンソルを変更せず新規テンソルを返す。aliases 機構が「この新規テンソルを `cache_k` パラメータに書き戻す」仕組みの前提になっています
- Decode ステップ 2 では、ステップ 1 の出力（`updated_cache`）を入力として更新することで累積的なキャッシュが実現できます
...
> **TP 並列を使う場合**: KV-Cache の形状は `[B, n_kv_heads, seq_len, head_dim]` ではなく `[B, n_kv_heads/tp_degree, seq_len, head_dim]` になります。aliases の構築は同じパターンだが、`n_kv_heads` がシャードされている点に注意してください。

### return 文でのアンパック構文

多層モデルで KV-Cache を return する際、Python の拡張アンパックは `torch.jit.script` に非対応。
...
> 実装全体: [→ modeling_gpt.py](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/modeling_gpt.py)

## 4. BaseModelInstance: aliases と forward の対応設計

[`BaseModelInstance`](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/model_wrapper.py) に対する aliases の設計は「`nn.Module.forward()` の return タプルの設計」と表裏一体です。先に forward の return タプルを決めてから aliases を構築する順序が安全です:
...
aliases のインデックスと return タプルが 1 つでもずれると、コンパイルは通るが KV-Cache の書き戻し先が狂い、推論結果が繰り返しまたは無音になります。

::::details [動作確認] aliases の仕組みと return タプルとの対応を小規模モデルで確認する（CPU 実行）

**参照ファイル**: [`src/neuron_xttsv2/modeling_gpt.py`](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/modeling_gpt.py)

このコードが確認すること。

2 層のミニトランスフォーマーで `aliases` の構造（nn.Parameter オブジェクト → return タプルのインデックス）と return タプルとの対応を確認します。aliases のカウントを 1 つズラした NG パターンも検証します。

期待される結果と理由。

- aliases の辞書キーは `nn.Parameter` の Python オブジェクト（ハッシュ可能）
- return タプルの長さは `1 + N_LAYER * 2 = 5`（hidden_states + cache_k 全層 + cache_v 全層）
- aliases と return タプルの形状が一致することを確認。NG パターン（カウントを 2 から始める）では最後のキーが `index=5` になり `len(outputs)=5` を超えてインデックス範囲外になります。aliases のカウントを 1 つでもズラすと KV-Cache の書き戻し先が狂います
...
Python 側では「古いキャッシュを受け取って新しいキャッシュを返す」という受け渡しが不要になります。aliases を正しく設定すると、コンパイラがハードウェアレベルで書き戻し処理を埋め込みます。
...
### `super().__init__()` を呼ぶかどうかの判断

`BaseModelInstance.__init__` は `(module_cls, input_output_aliases)` を引数に取ります。`module_cls` と aliases は `__init__` 時点では確定できないため（モジュールの初期化を `load_module()` まで遅延させる必要があります）、`super().__init__()` を呼ばず属性を直接設定します。

```python
def __init__(self, config):
    self.module = None
    # [{}] = 「バケット 0 については aliases なし」のプレースホルダー。
    # NeuronApplicationBase は self.models[0].input_output_aliases を参照するが、
    # 実際の aliases は get() が構築して返すため、この初期値が推論に使われることはない。
    self.input_output_aliases = [{}]
    self.config = config

def load_module(self):
    # load_module() は get() より必ず先に呼ばれる（フレームワーク保証）。
    # ここでモデルをインスタンス化する遅延初期化パターンを使う。
    self.module = YourNeuronTransformer(self.config)
```

`load_module()` の実装は必須。`get()` 呼び出し時点では `self.module` が初期化済みであることが前提となります。

> 実装全体: [→ modeling_gpt.py](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/modeling_gpt.py)

::::details [動作確認] load_module() が get() より先に呼ばれることを確認するシンプルな実装パターン（CPU 実行）

**参照ファイル**: [`src/neuron_xttsv2/modeling_gpt.py`](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/modeling_gpt.py)

このコードが確認すること。

フレームワーク（NeuronApplicationBase）が `load_module()` を `get()` より先に呼ぶ順序保証を CPU シミュレーションで再現します。逆順（`get()` → `load_module()`）を試みた場合のエラーも確認します。

期待される結果と理由。

- 正しい順序（`load_module()` → `get()`）: `self.module` が初期化済みになるため aliases が構築でき、`(module, aliases)` のタプルが返ります
- NG パターン（`get()` を先に呼ぶ）: `self.module is None` のため `RuntimeError: get() が load_module() より先に呼ばれた` が発生します
- `super().__init__()` を呼ばず `self.module = None` で初期化する遅延パターンはこのフレームワーク保証に依存しています
...
flowchart LR
    subgraph "compile() 呼び出しフロー"
        ...
        BMI["NeuronGPTInstance<br/>(BaseModelInstance)"]
        IG[input_generator]
        Sample[サンプル入力]
        Trace[torch.neuronx.trace]
    end
...
::::details [動作確認] input_generator() の形状と推論時入力の不一致がどんなエラーになるか確認する（CPU 実行）
...
`input_generator()` が生成した形状と異なる形状を推論時に渡した場合のエラーを CPU モックで再現します。Prefill モデル（seq_len=1081 固定）と Decode モデル（seq_len=1 固定）の両方で正常ケースと異常ケースを確認します。

期待される結果と理由。

- 正常ケース: Prefill 入力 `(1, 1081, 1024)` → OK、Decode 入力 `(1, 1, 1024)` → OK
- NG ケース: Decode モデルに Prefill 入力（seq_len=1081）を渡すと `RuntimeError: コンパイル時の seq_len=1 に対し 実行時 seq_len=1081` が発生。逆も同様
- 実際の Neuron ランタイムでは同様のエラーが XLA レベルで発生します。`NeuronConfig.seq_len` / `InferenceConfig.max_seq_len` / `input_generator()` の 3 値を一致させることでこのエラーを防げます
...
**方法 A: 2 つの ModelWrapper を 1 つの Application に登録（ScriptModule が共通の場合）**

Whisper の Decoder はこのパターンを使います（NxD Inference の [Whisper 実装](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/whisper/modeling_whisper.py) を参照）。Prefill と Decode でそれぞれ異なる入力形状を `bucket_config` で複数形状としてコンパイルしておくことで、1 つの ScriptModule が両方の形状を処理できます。KV-Cache パラメータはその ScriptModule 内で共有されるため、Prefill 実行後のキャッシュが Decode でそのまま参照でき、**Decode Application への明示的な転送は不要**です。

**方法 B: 2 つの独立した Application に分離（ScriptModule が別の場合）**

XTTS v2 の GPT はこのパターンを使います。Prefill と Decode でそれぞれ独立した `ScriptModule` を持ち、KV-Cache も別々のデバイスメモリに保持します。
...
2 Application に分離した場合、Prefill 後の KV-Cache を Decode Application に転送する仕組みが別途必要になります（Section 7 参照）。**KV-Cache を持たないモデル**（BERT 等の Encoder-only）は Decode ループ自体が不要なため、通常は単一の Application/ModelWrapper で構成できます（aliases を空 `{}` で実装）。

### バケット設定

バケットとは、異なる入力長に対応するためにコンパイル時に生成する形状バリアントのことです。固定長のシーケンスを扱う場合は `bucket_config=None`（デフォルト: `[seq_len]` の 1 バケット）で十分です。`super().__init__()` の引数として渡します。

```python
class ModelWrapperPrefill(ModelWrapper):
    def __init__(self, config, model_cls, **kwargs):
        super().__init__(config, model_cls, bucket_config=None, **kwargs)
        # bucket_config=None: 固定長シーケンスの場合は 1 バケットで十分
```

不要なバケットを定義するとコンパイル時間が乗算で増えるため、可変長入力が不要な場合は `bucket_config=None` を維持してください。

::::details [動作確認] bucket_config=None が 1 バケットになることをモックで確認する（CPU 実行）

**参照ファイル**: [`src/neuron_xttsv2/model_wrapper_gpt.py`](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/model_wrapper_gpt.py)

このコードが確認すること。

`bucket_config=None` を渡すと ModelWrapper が seq_len の 1 バケットだけをコンパイルすること、複数バケットを指定した場合とのコンパイル回数の違いを確認します。

期待される結果と理由。

- `bucket_config=None`: Prefill では `[1081]` の 1 バケット（1 回コンパイル）、Decode では `[1]` の 1 バケット（1 回コンパイル）
- `bucket_config=[256, 512, 1024, 1081]`: 4 バケット（4 回コンパイル）
- XTTS v2 GPT は入力長が常に `max_seq_len=1081`（Prefill）または `1`（Decode）に固定されるため、`bucket_config=None` で十分です。不要なバケットを追加するとコンパイル時間が乗算で増えます
...
> 実装全体: [→ model_wrapper_gpt.py](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/model_wrapper_gpt.py)
...
### `get_state_dict()` の 2 つの重大な制約

#### 制約 1: aliases に登録した KV-Cache パラメータを含めてはいけない

`cache_k`・`cache_v` は aliases 機構がゼロ初期化してランタイムが管理します。これを `get_state_dict()` の戻り値に含めると、`shard_checkpoint()` が通常の重みとして扱い形状不一致エラーになります。
...
::::details [動作確認] state_dict() に cache_k / cache_v が含まれる場合と除外した場合のキー一覧を比較する（CPU 実行）
...
`get_state_dict()` の戻り値に KV-Cache パラメータを含める場合（NG）と除外する場合（OK）のキー一覧を、2 層のミニモデルで比較します。

期待される結果と理由。

- NG パターン（全キー）: 12 キー（各層 query/key/value/out + cache_k/cache_v の 6 個 × 2 層）
- OK パターン（除外後）: 8 キー（KV-Cache の 4 個を除外）
- KV-Cache パラメータは aliases 機構がゼロ初期化して管理します。`shard_checkpoint()` に渡すと「通常の重み」として TP 分割しようとしますが、KV-Cache の形状はすでに TP 分割済みのため double-split になり形状不一致エラーになります
...
::::details [動作確認] ColumnParallelLinear の TP 分割前後の weight 形状を比較し、zero state dict を full-shape で手動構築する（CPU 実行）

**参照ファイル**: [`src/neuron_xttsv2/application_gpt.py`](https://github.com/littlemex/samples/blob/main/ml_distributed_experiment_collection/xttsv2-nxd-inference/src/neuron_xttsv2/application_gpt.py)

このコードが確認すること。

TP=2 環境で `ColumnParallelLinear` の `state_dict()` が分割済み形状を返す挙動と、`_make_zero_state_dict()` が full-shape で手動構築する理由を比較します。

期待される結果と理由。

- NG パターン（`state_dict()` 直接）: TP=2 で `query.weight = (8, 16)` という分割済み形状が返ります
- OK パターン（full-shape 手動構築）: `query.weight = (16, 16)` という元の形状になります
- `shard_checkpoint()` は full-shape の state dict を受け取り、内部で `tp_degree` に合わせて TP 分割します。`model.state_dict()` を渡すと既に分割済みの形状がさらに分割される double-split になります。`application_gpt.py` の `_make_zero_state_dict()` がこのパターンで全レイヤーの full-shape zero テンソルを手動構築しています
