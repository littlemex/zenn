...
return _make_zero_state_dict(config)  # compile フェーズ（読者が実装する関数）
```
...
**`load()` 後の `traced_model` は Python オブジェクトではない**

[`NeuronApplicationBase.load()`](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/application_base.py) が `torch.jit.load()` でコンパイル済みモデルをロードすると、`traced_model` は TorchScript のコンパイル済みオブジェクトになります。**ここで重要な制約があります**。このオブジェクトには `@torch.jit.export` デコレーターで明示的に公開されたメソッド・属性しかアクセスできません。通常の Python オブジェクトのようにメソッドを呼び出せると思って実装すると `AttributeError` になります。

`NxDModel` の `read_from_neuron_buffer` / `write_to_neuron_buffer` メソッドはこのデコレーターがないため、`load()` 後は呼び出せません。
