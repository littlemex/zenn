---
title: "NxD Inference にカスタムモデルを統合するための事前知識"
emoji: "🧙"
type: "tech"
...
それぞれ **数メソッドを実装するだけ**でよい設計で、残りは NxD Inference が引き受けます。
...
|---|---|---|
| `BaseModelInstance` | `load_module()` — nn.Module をインスタンス化 | `load_module()` 完了後に `get()` を呼び出す |
| | `get()` — aliases（KV-Cache の書き戻し対応表）を構築して返す | `(module, aliases)` を受け取り `torch.neuronx.trace()` へ渡す・KV-Cache の自動書き戻し |
| `ModelWrapper` | `get_model_instance()` — `BaseModelInstance` を生成して返す（1 行） | bucket 管理、`compile()` 呼び出しループ |
| | `input_generator()` — compile 時のサンプル入力テンソルを返す | |
| | `forward()` — `self.model(*args, **kwargs)` を呼ぶ（1 行） | |
| `NeuronApplicationBase` | `get_config_cls()` — 使用する Config クラスを返す（1 行）| `compile()` / `load()` / `load_weights()` / `warmup()` の実装全体 |
| | `forward()` — `self.traced_model()` を呼んで結果を返す | TP 分割・neff 保存・ロード・エラーハンドリング |
| | `load_hf_model()` — HF Hub 非使用時は `return None`（1 行） | |
| | `get_state_dict()` — Neuron 用 state dict を返す | |
| | `convert_hf_to_neuron_state_dict()` — 変換不要なら `return state_dict`（1 行） | |
...
`ColumnParallelLinear` は Tensor Parallelism で重み行列を列方向に分割して複数チップに分散させる標準的なプリミティブ（Megatron-LM 由来）で、NxD に限らず大規模モデルの分散推論で広く使われています。スタブでは `nn.Linear` に差し替え、`parallel_state` は TP degree=1 として動作します。
...
| GPT Transformer (30 層) | テキスト → mel トークンの自己回帰生成 | **Yes** |
...
`max_seq_len`（`InferenceConfig` サブクラスで定義）と [`ModelWrapper.input_generator()`](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/model_wrapper.py) が生成するサンプル入力の形状の 2 つが完全に一致していなければなりません。`NeuronConfig` に `seq_len` パラメータはなく、シーケンス長は `InferenceConfig` サブクラス（`XTTSv2InferenceConfig.max_seq_len`）で管理します。いずれかがずれるとコンパイル時に形状不一致エラーになります。
...
# load_weights 時: コーディネーターの load_weights() が config.xttsv2_checkpoint_path に明示的に注入する
# （NxD の基底クラスが自動注入するわけではない）
...
def get_state_dict(cls, model_name_or_path, config):
    ...
    if ckpt_path:
        return load_gpt_weights_from_xttsv2(ckpt_path, config, tp_degree=1)  # load_weights フェーズ（state_dict.py に実装）
    return _make_zero_state_dict(config)                                   # compile フェーズ（application_gpt.py に実装）
```
...
Section 2 で説明した 2 点を確認します。

(1) `XTTSv2InferenceConfig.__init__` で `self.xttsv2_checkpoint_path = ""` と初期化されており、compile 時は `bool("")` が `False` → `_make_zero_state_dict()` がゼロ初期化 state dict を返すこと。(2) クラス階層（`NeuronApplicationXTTSv2GPT` → prefill/decode アプリ）が正しく組み上がり、`_make_zero_state_dict()` が compile フェーズ用のゼロ初期化 state dict を返すこと。
...
- `state_dict keys: 32` — `_make_zero_state_dict()` が `gpt_layers=2` × 16 キー = 32 エントリのゼロ初期化 state dict を返していることを確認。これが compile フェーズで NxD に渡す重みプレースホルダーです

::::

## 3. nn.Module: Neuron コンパイル対象の実装規則

`NeuronApplicationBase` の裏で `torch.neuronx.trace()` を通して Neuron コンパイラによりコンパイルされる `nn.Module` には、通常の PyTorch とは異なる制約があります。
...
> **TP を使う場合**: `__init__` 内で `self.n_kv_heads = ceil_div(n_head, tp_degree)` として計算されるため、KV-Cache の形状はつねに `[B, self.n_kv_heads, seq_len, head_dim]`（TP シャード後のヘッド数）です。aliases の構築は同じパターンです。
...
複数リストの `*` 展開（`*a, *b`）は CPython でも `SyntaxError` になります。単一リストの展開（`return x, *lst`）は CPython 3.5+ では動きますが、`torch.neuronx.trace()` による XLA 静的グラフ化の際に動的な展開を解釈できません。いずれの場合もタプルの結合演算子（`+`）で明示的に組み立てる必要があります。
...
本番の NxD SDK では `BaseModelInstance.__init__` は `(module_cls, input_output_aliases)` を引数として要求しますが、この 2 つは `__init__` 時点では渡せません。`module_cls`（モデルインスタンス）は `load_module()` 内で生成し、`aliases` は `get()` 内で `self.module.blocks` を直接走査して構築するため、どちらも後のタイミングに依存しているからです。

そのため `.__init__()` は呼ばず、属性を直接設定してモジュール初期化を `load_module()` まで遅延させます。`load_module()` の実装は必須で、`get()` が呼ばれる時点では `self.module` が初期化済みであることが前提となります。
...
1 つの `NeuronApplicationBase` が保持する `self.traced_model`（Application が直接参照する ScriptModule）は 1 つだけで、形状はコンパイル時に固定されます。

`NeuronApplicationBase` は `self.models` リストも持ち、複数の `ModelWrapper` を登録できます（方法 A で後述）。そのため `bucket_config` で複数形状をコンパイルしていない場合、同じ Application で Prefill (`[B, 1081, 1024]`) と Decode (`[B, 1, 1024]`) の両方を処理することはできません。

解決策は 2 通りあります。

**方法 A: 2 つの ModelWrapper を 1 つの Application に登録（nn.Module を共有する場合）**

Whisper の Decoder はこのパターンを使います（NxD Inference の [Whisper 実装](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/whisper/modeling_whisper.py) を参照）。Prefill 用と Decode 用の 2 つの `ModelWrapper` をそれぞれ `self.models` リストに登録します。それぞれの `input_generator()` が異なる入力形状（Prefill: `[B, n_text_ctx, n_state]`、Decode: `[B, 1, n_state]`）を返すことで、`compile()` 時に `torch.neuronx.trace()` が 2 回独立して実行され、2 つの ScriptModule が生成されます。`nn.Module` 内部の `if seq_len > 1` 分岐はトレース実行時の入力形状によってパスが自動的に決まるため、コンパイラが 2 グラフを生成するトリガーではありません。KV-Cache パラメータは同一 `nn.Module` インスタンス内で共有されるため、Prefill 実行後のキャッシュが Decode でそのまま参照でき、**Decode Application への明示的な KV 転送は不要**です。
...
| メソッド | 必須度 | デコレーター | 役割 |
|---------|--------|------------|------|
| `get_config_cls()` | 必須 | `@classmethod` | `config=None` 時にコンフィグクラスを返す |
| `forward()` | 必須 | — | 推論エントリポイント |
| `load_hf_model()` | 必須（HF Hub 未使用時は `return None`） | `@staticmethod` | HF Hub からロードする場合は実装、使わない場合は `return None` を返す 1 行実装 |
| `get_state_dict()` | 必須 | `@classmethod` | Neuron 用 state dict を返す（独自 ckpt 形式の場合はチェックポイント読み込みを実装）。第 1 引数が `cls` であることに注意 |
| `convert_hf_to_neuron_state_dict()` | 必須（変換不要なら `return state_dict`） | `@staticmethod` | HF キー名を NxD キー名に変換する。変換不要な場合は `return state_dict` の 1 行実装 |
...
# OK: model.state_dict() を呼ばず手動で構築し KV-Cache を含めない
return _make_zero_state_dict(config)  # compile フェーズ（Application 内のプライベート関数）
```
...
`forward()` の引数は通常の Python メソッドと同様に自由に定義できます。ただし `self.traced_model()` に渡す引数は `input_generator()` が生成するサンプル入力の形状・順序と一致させること（型・形状の不一致はランタイムエラーになります）が必要です。
...
HF Hub から直接ロードする場合は `load_hf_model()` と組み合わせることで `get_state_dict()` のオーバーライドが不要になるため、標準的な HF モデルへの組み込みではそちらが推奨です。

## 7. コーディネーターパターン: 複数 Application の束ね方

:::message
このセクションは **Section 5 の方法 B を採用した場合のみ必要**。単一 Application 構成ではコーディネーター層は不要で、`NeuronApplicationBase` のサブクラスを直接使えばよいです。
:::
...
class NeuronApplicationXTTSv2GPT:           # 継承なし
    def __init__(self, model_path, config):
        self.config = config          # compile() / load_weights() / sync_kv_cache_prefill_to_decode() で参照
        self.model_path = model_path  # load() のデフォルトパスに使用
...
**Decode ループ内で複数の Application を協調させる場合も同じパターンが使えます。** 

例えば「1 ステップ内で Model-A の出力を受け取り Model-B に渡す」構成は、コーディネーターの `forward()` 内で 2 つの Application を順番に呼ぶだけです。ただし各 Application 呼び出しは同期的であり、呼び出しのたびに Neuron デバイス→CPU→Neuron デバイスのラウンドトリップが発生します。生成ステップ数が多いモデルでは Application の分割粒度がレイテンシに直結するため注意が必要です。
...
**`load()` 後の `traced_model` は通常の `nn.Module` として扱えない**

[`NeuronApplicationBase.load()`](https://github.com/aws-neuron/neuronx-distributed-inference/blob/v0.7.14366/src/neuronx_distributed_inference/models/application_base.py) が `torch.jit.load()` でコンパイル済みモデルをロードすると、`traced_model` は `torch.jit.RecursiveScriptModule`（`nn.Module` を継承した TorchScript オブジェクト）になります。**ここで重要な制約があります**。`@torch.jit.export` デコレーターで明示的に公開されていない Python メソッドにはアクセスできず、呼び出すと `AttributeError` になります。`state_dict()` や `parameters()` などの `nn.Module` 固有メソッドおよびサブモジュール（`traced_model.nxd_model` 等）へのアクセスは引き続き可能です。

`NxDModel` の `read_from_neuron_buffer` / `write_to_neuron_buffer` メソッドは TorchScript 経由で公開されていないため、`load()` 後は呼び出せません。
...
どのメソッド・属性が TorchScript 経由でアクセス可能かを確認するには、ロード後に `dir(traced_model.nxd_model)` で一覧を取得するかなどで判断します。

**TorchScript 属性として公開されている `.state` を使う**

TorchScript を通じて公開されている属性を確認すると、`.state` が `List[Dict[str, Tensor]]` 形式で使用可能です。
...
Neuron デバイステンソルへの非連続スライス代入は動作しない（XLA 制約）ため、CPU で処理してから `copy_()` で戻します。

```python
# self.config から取得（実装では sync_kv_cache_prefill_to_decode(prefix_len) 内で参照）
tp_degree = self.config.neuron_config.tp_degree  # 例: 2
n_layer   = self.config.gpt_layers               # 例: 30
max_seq   = self.config.max_seq_len              # 例: 1081
# prefix_len: Prefill 実行後の有効トークン数（呼び出し元から渡す）
...
カスタムモデルをコンパイルする際、dtype の選択が精度に大きく影響します。

深い Transformer では、FP16 の後半の層で attention スコアが overflow/underflow し、同じトークンを繰り返し生成する・出力品質が著しく低下するという問題が起きることがあります。実際に 30 層規模の GPT モデルでこの問題が再現し、BF16 への切り替えで品質が大幅に改善した実績があります。

### dtype 変換の境界

CPU（float32）と Neuron（bfloat16）の境界でテンソルの dtype 変換が必要です。
...
```python
# [compile フェーズ（初回のみ）]
config = YourInferenceConfig(neuron_config=NeuronConfig(tp_degree=2, torch_dtype=torch.bfloat16))
...
# Section 2 の 2 フェーズ注入: coordinator.load_weights() が内部で
# config.xttsv2_checkpoint_path にチェックポイントパスを設定した上で
# 各 Application の load_weights(model_path/prefill|decode) を呼ぶ
# ※ NeuronApplicationBase.load_weights() とは異なるカスタム実装
#   コーディネーターの引数は .pth ファイルパス、各 Application への引数は compiled model ディレクトリパス
...
カスタムモデルを NxD Inference に統合するためのインタフェースを整理しました！次のブログでは XTTSv2 を NxD Inference に実際に適用する際の個別考慮が必要な事項やパフォーマンス、精度の結果についてまとめます。

これで皆さんも NxD Inference にカスタムモデルをゴリゴリに載せまくって、あわよくば OSS Contribution できると良いですね！Let's try AWS Neuron!!!!!