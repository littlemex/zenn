---
title: "GPUDirect RDMA 入門: NIC が GPU メモリに直接読み書きする仕組み"
emoji: "📨"
type: "tech"
...
:::message
**この記事のゴール**: 「GPU 同士がネットワーク越しに、CPU をほとんど介さず直接データをやり取りする」仕組みである **GPUDirect RDMA** を、初学者向けに、しかし**実装レベルの正確さ**で解説します。
...
この記事は単体で完結します。NVSHMEM など分散 GPU 通信ライブラリの「下回りの仕組みを知りたい」という動機があると読み進めやすいですが、前提知識は必須ではありません。前回の NVSHMEM 入門で「GPUDirect RDMA = 低レベルの土台技術」と書いたものを、今回はその土台そのものを掘り下げます。
...
::::details 補足: TCP も最適化はしている
Linux も `MSG_ZEROCOPY` や `sendfile()`（大きい転送向け）、送受信のバッチ化（GSO/GRO）でコピーや処理を削減しています。なので「TCP は毎回必ず何度もコピーする」は厳密には正しくありません。ただし**小さいメッセージを高頻度でやり取りする**ワークロードでは、これらの最適化が効きにくく、カーネル処理の固定オーバーヘッドが支配的になります。RDMA が効くのはまさにこの領域です。
::::
...
```mermaid
flowchart TB
    subgraph TCP["TCP/IP の RTT（大きい）"]
        direction LR
        T1["アプリ"] --> T2["カーネル"] --> T3["NIC"] -->|往復| T4["相手カーネル"] --> T5["相手アプリ"]
    end
    subgraph RDMA2["RDMA の RTT（小さい）"]
        direction LR
        R1["アプリ"] --> R2["NIC"] -->|往復| R3["相手 NIC が直接メモリへ"]
    end
```
...
:::message
**数値はあくまで目安**です。ベンダが公表する「アダプタ遅延 0.5 µs」のような数字は、多くが ping-pong テストの**往復時間を 2 で割って片道遅延として報告した値（RTT/2）**で、両端の NIC 処理時間を含みます。アプリから実際に見えるのは往復遅延（RTT）なので、体感値はこれより大きくなります。重要なのは絶対値より、**TCP と RDMA の間に一桁以上の差が出うる**という構造です。
:::

なぜ下がるのか。RDMA は、(1) データ転送時の system call を消し、(2) コピーを消し、(3) トランスポート処理（順序制御・ACK・再送）を**NIC のハードウェアにオフロード**します。CPU とカーネルがクリティカルパスから外れる、これがレイテンシ低減の正体です。「速いハードだから」ではなく「**アーキテクチャ的にソフト処理を省いているから**」が本質です。ひとことで言えば、TCP/IP は「**OS という郵便局を毎回経由する**」のに対し、RDMA は「**相手の郵便受けに自分で直接投函する**」イメージです。仕分け（カーネル処理）がない分、速くて軽い。
...
流れは三段階です。(1) アプリが「宛先・データの場所」を書いた**記述子をキューに積む**、(2) NIC のレジスタに直接書き込んで「**ドアベル**を鳴らす」（処理を始めろと知らせる）、(3) NIC が非同期に送信し、終わると**完了キュー（CQ）**に通知するので、アプリはそれを読み取る。
...
- `rkey` は暗号的な秘密ではなく、NIC ハードウェアがテーブル照合する 32 bit のトークンです。安全性は「接続が認証済みであること」と「**保護ドメイン（PD）**による分離」に依存します（PD は、メモリ登録（MR）や QP を論理的に束ね、別グループからのアクセスを NIC レベルで遮断する管理単位。どの部品が PD に属すかは上級編で詳説）。
...
| 位置づけ | 古くからの方式。今も多くの本番環境で稼働 | 新しく推奨される方向。クリーンで標準的（Blackwell 以降は dma-buf へ移行が進む） |

:::message
**よくある誤解の整理**:
- 「GPUDirect RDMA はシステム RAM を経由する」→ **逆です**。経由するのは GPUDirect が**ない**ときの話。GPUDirect RDMA の主眼はまさにそのバウンスコピーを消すこと。
- 「`gdrcopy` と GPUDirect RDMA は同じ」→ **違います**。両方とも BAR1 窓を使いますが、**GPUDirect RDMA = NIC が GPU メモリを DMA**、**gdrcopy = CPU が GPU メモリを mmap して直接読み書き**。主体（誰が動くか）が違います。gdrcopy は小さいデータを低レイテンシで GPU へ送るのに向きます（書きは write-combining で速いが、BAR 領域はバースト読み出しに向かず読みは遅い、という非対称性があります）。
:::

### 動かすための物理的な条件（ハマりどころ）

GPUDirect RDMA は「GPU と NIC が PCIe 上で直接やり取りできること」が前提です。

- **GPU と NIC が同じ PCIe ルートコンプレックスを共有**している必要がある。理想は同じ PCIe スイッチ配下。2 ソケットサーバで GPU と NIC が別 CPU 側にあると、CPU 間インターコネクト（UPI 等）を経由して**性能が大きく落ちる/動かない**ことがある
- **IOMMU は 1:1（パススルー）**である必要がある。IOMMU（Input-Output Memory Management Unit）は DMA の宛先アドレスを変換する機構で、仮想化環境ではこの変換が入って GPU と NIC の見るアドレスがずれ、直通が壊れる
- **PCIe ACS**（Access Control Services）はデバイス間の直接転送（P2P DMA）を制限するセキュリティ機能。有効だと、ピアツーピア転送が**黙ってルートコンプレックス経由に迂回**され、直通が壊れることがある（エラーは出ないのに遅い、という形で現れる）

---

## 5. AWS EFA はどうやって RDMA を実現しているか

クラウドで RDMA を使う代表格が、AWS の **EFA（Elastic Fabric Adapter）** です。EFA は GPUDirect RDMA に対応しており、NVSHMEM や NCCL の下回りとして使われます。ここは仕組みが面白いので少し掘り下げます。
...
InfiniBand では伝統的に **RC（Reliable Connection）** という「1 対 1 の信頼できる接続」を張ります。しかし N ノードが全員と通信すると **O(N²) 本の接続**が要り、巨大クラスタでスケールしません。

そこで EFA は **SRD（Scalable Reliable Datagram）** を使います。

- **信頼性は EFA デバイス（ハードウェア）が担保**（届くことは保証する）
- 複数経路にパケットをばらまく**マルチパス**で、巨大ネットワークでも詰まりにくい
- **データグラム型**なので、1 つの QP で全ピアと通信できる（接続を張らない＝スケールする）
- ただし **EFA デバイス層の SRD は順序を保証しない**（パケットは順不同で届く）。これは手抜きではなく、マルチパスで複数経路に並走させると経路ごとに遅延が違って自然に順序が乱れるため。順序保証をハードから切り離すことで、マルチパスの自由度を最大化している

```mermaid
flowchart LR
    subgraph rc["InfiniBand RC"]
        N0["ノード"] --- P1["ピア1 への専用接続"]
        N0 --- P2["ピア2 への専用接続"]
        N0 --- P3["… N 個の接続 (O(N²))"]
    end
    subgraph srd["EFA SRD"]
        S0["ノード<br/>1 つの SRD QP"] -.->|"宛先を指定して誰へでも"| ALL["全ピア"]
    end
```

「デバイス層では順序保証しない」のが SRD の割り切りです。では順序が必要なアプリ（MPI/NCCL）はどうするか。**順序の再構成は上位のソフトウェア（libfabric の efa provider）が担い**、アプリには **SAS（Send-After-Send）** 保証を提供します。これは TCP の recv と同じで「送った順にデータを受け取れる」という意味です。「ハードは順不同・速さ優先、ソフトが順序を後付け」という二層構造になっています。
...
::::details 実装詳細: SRD QP の作り方
SRD は標準の verbs QP 種別に無いため、`ibv_qp_type` に `IBV_QPT_DRIVER`（ドライバ拡張の口）を指定し、`efadv_create_qp_ex()` に `EFADV_QP_DRIVER_TYPE_SRD` を渡して作ります。QP のセットアップ自体は InfiniBand と同じく RESET → INIT → RTR → RTS と状態遷移しますが、設定するパラメータの中身は RC とは異なります。
::::
...
```mermaid
flowchart TB
    APP["アプリ / NCCL / NVSHMEM / MPI"] --> LF["libfabric efa provider [OSS]<br/>順序の再構成・大メッセージ分割・タグマッチ"]
    LF --> EFADV["rdma-core + efadv [OSS]<br/>SRD QP を作る"]
    EFADV --> DRV["efa カーネルドライバ [AWS]"]
    DRV --> HW["EFA デバイス [AWS Nitro HW]"]
    HW == "SRD: マルチパス・信頼・順不同" ==> HW2["相手の EFA デバイス"]
```
