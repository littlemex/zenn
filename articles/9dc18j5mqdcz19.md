...
This is a companion to the [Neuron Agentic Development setup article](https://zenn.dev/tosshi/articles/f3f678f4b6531c). That article covered deploying a single Trainium workstation with Claude Code integration. This article goes further: we deploy **Voice Image Edit**, a full multi-model inference demo that runs four ML models simultaneously on a single `trn2.48xlarge` instance (64 NeuronCores), orchestrated by a single `deploy-all.sh` script.

Voice Image Edit takes a voice instruction, transcribes it (ASR), interprets the instruction against an image (VLM), edits the image (diffusion), and speaks a confirmation (TTS) — all on Trainium, without any GPU or external API dependency for inference. Unlike GPUs where models share a pool of CUDA cores dynamically, NeuronCore allocation is static per process — packing four models onto one instance requires explicit core partitioning, which section 5 covers in detail.

This article assumes you have completed the [setup article](https://zenn.dev/tosshi/articles/f3f678f4b6531c). Reading time: ~15 minutes. A `trn2.48xlarge` Capacity Block costs approximately $36/hour.
