...
GIN、EFA GDA、RAS、cost model の背景は以前の記事で解説しました。

https://zenn.dev/tosshi/articles/8d7fea5568e638

[NCCL v2.32.3-1](https://github.com/NVIDIA/nccl/releases/tag/v2.32.3-1) で、上記記事に並べた項目がどこまで実装に降りたのかを 4 つ整理します。背景の解説は繰り返しません。
...
拒否された後の挙動も読み取れます。`ncclGinDevCommSetup` は候補のバックエンドを順に試し、失敗すると次の候補に移ります。低い番号で再試行するのではなく、別のバックエンドを試します。すべて失敗すると「GIN: DevComm setup failed on all available backends」で終わります。EFA GDA を狙っていても、候補に他のバックエンドが残る構成なら気付かないままその候補に切り替わり、残らなければ GIN 自体が立ち上がらない、という形で表に出ます。実機で再現したわけではないので断定はしませんが、コード上はこう読めます。
