---
title: "Amazon SageMaker HyperPod Health Monitoring Agent 徹底解説"
emoji: "🏥"
type: "tech"
...
- HMA のアーキテクチャ（Slurm / EKS 環境）
- ヘルスチェックの分類（パッシブチェックとアクティブチェック）
- 障害検出から復旧までの対応フロー
- Slurm との統合（手動操作・API 操作）
- Auto-Resume によるジョブの自動再開
- ログとモニタリング
...
| テスト | 対象 | 説明 |
|---|---|---|
| [DCGM](https://docs.nvidia.com/datacenter/dcgm/latest/user-guide/index.html) Diagnostics | GPU | メモリテストを含む包括的な GPU 診断（DCGM の Level 3 診断を使用） |
...
### DCGM Level 3 診断のプラグインアーキテクチャ

**NVIDIA Data Center GPU Manager（DCGM）** は、GPU の健全性診断を提供するデーモンです。HyperPod では特に **Level 3 診断**（最も包括的な診断レベル）を使用しています（[DCGM ユーザーガイド](https://docs.nvidia.com/datacenter/dcgm/latest/user-guide/index.html)、[DCGM GitHub](https://github.com/NVIDIA/DCGM)参照）。

Level 3 診断は、複数の**プラグイン**を組み合わせて GPU のハードウェア・ソフトウェア全体を検証します。
...
:::message
**実環境での効果**: NVIDIA の検証では、Summit スーパーコンピューター（24,576 GPU）において、Tree アルゴリズムが Ring に比べて **最大 180 倍のレイテンシ改善**を達成しました。この改善は、ResNet-50 などの実際の訓練ワークロードにおいても、スケール時の訓練スループット向上として観測されています。
:::
...
| 強制交換 | `scontrol update ... state=down reason="Action: Replace"` | -- |
...
:::message alert
`--auto-resume=1` を使用する場合は `--exclusive` フラグの指定が必須です。また、`$SLURM_JOB_NODELIST` と `$SLURM_NNODES` は Auto-Resume 後に更新されないため、上記スクリプトのように `scontrol` で動的にノードリストとノード数を取得してください。
:::
...
本記事では、SageMaker HyperPod の Health Monitoring Agent（HMA）について解説しました。

- **アーキテクチャ**: Slurm 環境ではシステムデーモン、EKS 環境では DaemonSet として各ノードに常駐し、GPU / Trainium / EFA を継続的に監視
- **2 種類のヘルスチェック**: パッシブチェックで障害を即座に検出し、アクティブチェック（Deep Health Checks）でクラスター作成・更新時に包括的な診断を実施
- **自動障害対応**: ノードのスケジューリング除外から交換まで自動で対応し、Auto-Resume との組み合わせでジョブの自動再開も実現
- **誤検知防止**: 段階的な対応、複合的な診断、閾値ベースの判定を組み合わせて誤検知を最小化
- **運用ツール**: Slurm コマンドと AWS API による手動操作、CloudWatch Logs によるモニタリングが利用可能

大規模訓練においてハードウェア障害は「起こるかどうか」ではなく「いつ起こるか」の問題です。HMA の仕組みを理解し適切に活用することで、障害発生時のダウンタイムを最小化し、訓練全体の効率を高めることができます。
...
- [NVIDIA DCGM User Guide](https://docs.nvidia.com/datacenter/dcgm/latest/user-guide/index.html) -- Level 3 診断とプラグインアーキテクチャの詳細
...
本記事で解説した HMA は、HyperPod が提供する耐障害性機能の 1 つです。他の機能についても別記事で解説しています。
