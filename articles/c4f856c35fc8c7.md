...
① AgentCore Evaluations がどういうもので他の AWS サービスとどのような関係があるのか、② Tool use と Memory に関する組み込み評価にどのようなものがあるのか、③ 実際に AgentCore Evaluations を動かしてみる、の 3 点に取り組む。
...
:::details Context relevance（コンテキスト関連性）
**評価 ID:** `Builtin.ContextRelevance`

**取得される情報:**
- `context`: ユーザークエリ
- `retrieved_passages`: 取得されたパッセージ

**評価内容:**
提供されたコンテキストが、与えられた質問に適切に答えるために必要な情報を含んでいるかを評価する。RAG（Retrieval-Augmented Generation）システムでの検索品質の評価に有用。

**スコア:**
- `Perfectly Relevant`: パッセージは明確に質問に関連している
- `Partially Relevant`: パッセージは明確に無関係でも明確に関連でもない
- `Not Relevant`: パッセージは明確に質問に無関係
...
① AgentCore Evaluations がどういうもので他の AWS サービスとどのような関係があるのか、② Tool use と Memory に関する組み込み評価にどのようなものがあるのか、③ 実際に AgentCore Evaluations を動かしてみる、の 3 点について整理した。
