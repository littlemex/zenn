...
**Fig 8: 測定の実行順序（Step 1 環境確認 → Step 2-3 E2E 測定 → Step 4-5 分析・詳細測定）**
...
flowchart LR
    ...
    subgraph STEP2["Step 2: ベースライン"]
        L1["<b>L1-Unified</b><br/>単一ノード E2E<br/>KV 転送なし<br/>25 パターン"]
    end

    subgraph STEP3["Step 3: 分散推論"]
        L2["<b>L2-EFA</b><br/>EFA Disaggregated<br/>24 パターン"]
        L3["<b>L3-TCP</b><br/>TCP Disaggregated<br/>24 パターン"]
    end

    subgraph STEP4["Step 4: 自動分析"]
        L4["<b>L4-Analysis</b><br/>bimodality detection<br/>proxy overhead<br/>TPOT separation"]
    end

    subgraph STEP5["Step 5: 詳細測定・検証"]
        L5["<b>L5-LowLevel</b><br/>fi_pingpong (4+2)<br/>NIXLBench (22)<br/>KVBench (6)<br/>ucx_perftest (10)"]
        CMP["<b>手動比較</b><br/>CMP-01 ~ CMP-10<br/>クロスバリデーション"]
    end

    STEP1 -->|"HW 正常確認後"| STEP2
    STEP2 -->|"KV 転送なし基準確定後"| STEP3
    STEP3 -->|"全 E2E 測定完了後"| STEP4
    STEP4 -->|"自動分析完了後"| STEP5
    ...
    style CMP fill: #f5f5f5,stroke: #424242
```
...
**Step 2: L1-Unified -- KV 転送なしの基準値を取得する**

単一ノードで KV-Cache 転送のない状態の TTFT/TPOT を測定します。この値は「KV-Cache 転送が存在しない場合の理想値」であり、Step 3 の分散推論測定との差分が「KV-Cache 転送のオーバーヘッド」に対応します。1K から 128K tokens の 7 種類のプロンプト長について、並行度 c=1,4,8（64K 以上）または c=1,4,8,16（32K 以下）で測定します。

**Step 3: L2-EFA + L3-TCP -- 分散推論の E2E 測定**

EFA と TCP の両方で分散推論の E2E 測定を実施します。Step 2 の基準値があるため、KV-Cache 転送のオーバーヘッドを分離して評価できます。128K tokens では分散推論モードで GPU VRAM 制約があるため、c=1,4 のみ測定します。

**Step 4: L4-Analysis -- 自動分析**

E2E 測定データを自動分析します。bimodality detection では TTFT の二峰性（Phase A: MR 登録コスト、Phase B: 定常状態）を検出し、proxy overhead では Proxy サーバーの内部タイムスタンプ（X-Proxy-* ヘッダー）から Prefill/Decode の転送オーバーヘッドを測定します。TPOT separation では ITL（Inter-Token Latency）を GMM でクラスタリングし、GPU 計算時間とバッチスケジューリング待機時間を分離します。

**Step 5: L5-LowLevel + 手動比較 -- 詳細測定とクロスバリデーション**

E2E 測定結果を踏まえて、Transport Layer と Library Layer の詳細測定を実施します。fi_pingpong/fi_rdm_pingpong で EFA のレイテンシープロファイルを、ucx_perftest で UCX レイヤの帯域幅（cuda vs host）を、NIXLBench/KVBench で NIXL の転送性能をそれぞれ測定します。これらの結果と E2E メトリクスを用いて、10 個のクロスバリデーション比較（CMP-01 から CMP-10）を手動で実施し、各レイヤの測定結果が整合しているかを検証します。
