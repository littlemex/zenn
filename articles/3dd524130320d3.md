...
本記事では、vLLM Disaggregated Inference の性能を理解するために、**レイヤードアプローチ**による体系的な測定手法を提案しました。通信スタックを Application、Library、Transport、Hardware の 4 層に分離し、測定も同様に L0（Baseline）、L5（LowLevel）、L1-L3（E2E）の階層で実施することで、**各レイヤの性能を独立に測定し、依存関係と値を分離**できます。さらに、複数の測定ツールを組み合わせた **10 個のクロスバリデーション比較（CMP-01 ~ CMP-10）**により、ハードウェアレイヤの帯域差が上位レイヤに伝播し、最終的に vLLM の TTFT に影響することを段階的に検証します。このアプローチにより、E2E のメトリクスだけでは見えない性能差の因果関係を明確にし、ボトルネックの所在を特定できます。
