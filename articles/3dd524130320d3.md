...
:::message
次回の実験結果考察編はインスタンス確保の都合上 g6e.12xlarge を利用していますが、このインスタンスは GPUDirect RDMA (GPUDirect Remote Direct Memory Access) に対応していないためゼロコピーの恩恵は受けられません。[g7e](https://aws.amazon.com/jp/blogs/news/announcing-amazon-ec2-g7e-instances-accelerated-by-nvidia-rtx-pro-6000-blackwell-server-edition-gpus/) は対応しています。
:::

:::message alert
**重要**: GPUDirect RDMA と `kv_buffer_device="cuda"` は密接に関連しています。
`kv_buffer_device="cuda"` + LIBFABRIC backend を使用する場合、GPU Direct RDMA が**必須**です。GPU Direct RDMA 非対応のインスタンスでは、`kv_buffer_device="cpu"` を使用する必要があります。NixlConnector は実装上 3 つの異なるパターンをサポートしています。
:::

| 項目 | パターン 1: GPU Direct RDMA | パターン 2: EFA (CPU buffer) | パターン 3: TCP (CPU buffer) |
|------|---------------------------|----------------------------|----------------------------|
| **GPU Direct RDMA 対応** | あり（ハードウェア要件） | 不要 | 不要 |
| **対象インスタンス** | P5/P5en/P4d/g7e など GPU Direct RDMA 対応 | 全インスタンス（g6e、g5 を含む） | 全インスタンス（g6e、g5、TPU、XPU を含む） |
...
| **理論帯域** | EFA 100 Gbps (g6e.12xlarge) | EFA 100 Gbps (g6e.12xlarge) | TCP ~9.5 Gbps |
| **性能** | 最高 | 高 | 最低 |
...
次回の Phase 1 実験結果考察編では、g6e.12xlarge でパターン 2（EFA、CPU buffer）とパターン 3（TCP、CPU buffer）の性能差を測定します。パターン 2 は EFA RDMA（OS-Bypass、ゼロコピーDMA）を使用するため、パターン 3 の TCP/IP（カーネル経由、メモリコピーあり）より高い性能が期待されます。理論的には、パターン 2 は EFA の 100 Gbps 帯域幅を活用できるのに対し、パターン 3 は TCP の実効帯域（~9.5 Gbps）に制限されます。
:::
...
sequenceDiagram
    ...
    EFA_P->>Network: データ送信<br/>(EFA RDMA, SRD Protocol)
    Network->>EFA_C: パケット転送
    EFA_C->>Consumer_CPU: DMA 転送
    Consumer_CPU->>Consumer_GPU: cudaMemcpy H2D<br/>(sync_recved_kv_to_device)
    Note over Consumer_GPU,Producer_GPU: メモリコピー 2 回（D2H + H2D）<br/>CPU DRAM 経由、EFA RDMA（100 Gbps）
```
