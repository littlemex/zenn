...
本測定では、vLLM DI の 3 つの重要な技術要素に焦点を当てます。

第一に **NIXL (Network Interconnect for XPU Linking)** です。NVIDIA の GPU 間通信ライブラリであり、P2P (peer-to-peer) 通信により Producer と Consumer が直接接続して KV-Cache を転送します。NIXL は UCX または libfabric を通信バックエンドとして使用し、環境変数だけでトランスポートレイヤ（EFA または TCP）を切り替えられる設計になっています。

第二に **CPU offloading** です。KV-Cache 転送時に CPU メモリ（DRAM）を経由するパス（`kv_buffer_device=cpu` ）と、GPU VRAM から直接転送するパス（`kv_buffer_device=cuda` ）の性能差を測定します。GPUDirect RDMA 対応インスタンスでは、cuda パスがゼロコピー転送を実現し、cpu パスより高速になることが期待されます。

第三に **P2P アーキテクチャ** です。Producer と Consumer が NIXL 経由で直接通信する構成を採用しており、将来的には Remote Storage（Valkey）を使った代替アーキテクチャとの性能比較も予定しています。
