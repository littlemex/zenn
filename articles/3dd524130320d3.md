...
:::message
**L0-Baseline と L5-LowLevel での libfabric ツールの違い**: L0-Baseline では `fi_rdm_pingpong` と `fi_rdm_bw` を使用してハードウェアの健全性を確認します。一方、L5-LowLevel では `fi_pingpong`（汎用版、標準エンドポイント使用）を使用して、より詳細なレイテンシープロファイル（64B, 1KB, 64KB, 1MB の 4 サイズ）を測定します。`fi_rdm_*` は RDM (Reliable Datagram) エンドポイント専用、`fi_pingpong` は汎用的なエンドポイント（MSG/RDM）をサポートする違いがありますが、どちらも libfabric の EFA プロバイダを通じて同じ EFA デバイスを測定します。L0 で大まかな帯域幅とレイテンシーを確認し、L5 でサイズごとの詳細なプロファイルを取得する設計です。
:::
::::
...
:::message
**bytes_per_token の計算根拠**: これらのサイズは NIXLBench のデフォルト値 `bytes_per_token = 262144`（256KB/token）に基づきます。この値は Qwen2.5-32B-Instruct (TP=4) のモデル全体の KV-Cache サイズ `2 x 64 layers x 8 kv_heads x 128 head_dim x 2 bytes(bf16) = 262144 bytes/token` に相当します。実際の分散推論では、各 TP ワーカーは自分の担当する kv_heads 分（TP=4 の場合、8 kv_heads / 4 = 2 kv_heads）の KV-Cache を個別に転送しますが、NIXLBench では測定の便宜上、この大きめの値をデフォルトとしています。
:::
...
**どうやって**: NIXLBench コマンドで、1K tokens（約 268MB）から 128K tokens（約 32GB）のデータを GPU VRAM 間で転送します。バックエンドとして Libfabric（EFA）と UCX（TCP）の両方を測定し、転送方式として one_to_one（1 対 1）と many_to_one（多対 1）をテストします。
...
**どうやって**: KVBench コマンドで、モデル構成を指定し（例: `--num_layers 64 --num_kv_heads 8 --head_dim 128 --cache_dtype bf16`）、1K, 4K, 12K, 32K, 64K, 100K, 128K tokens の KV-Cache 転送を測定します。バックエンドとして Libfabric（EFA）と UCX（TCP）の両方を測定します。
