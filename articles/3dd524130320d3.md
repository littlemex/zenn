...
:::message
**Proxy 実装について**: vLLM リポジトリには複数インスタンス対応の [Toy Proxy Server](https://github.com/vllm-project/vllm/blob/v0.16.0/tests/v1/kv_connector/nixl_integration/toy_proxy_server.py) が参考実装として提供されています。本測定では、**1 Prefiller + 1 Decoder 構成に特化した独自実装** `disagg_proxy_server.py v3` を使用しています。この実装には以下の測定最適化が含まれます：
- **タイムスタンプ測定機能**: Prefill/Decode 各フェーズの時間を `X-Proxy-*` ヘッダーで記録
- **接続プーリング最適化**: HTTP/1.1 keep-alive による TCP handshake 削減（~50-100ms の Proxy オーバーヘッド削減）
- **1:1 構成最適化**: 単一 Prefiller/Decoder ペアでの測定精度向上

本セクションで説明する Proxy の一般的な動作フロー（Prefill → Decode のリクエスト転送、`kv_transfer_params` のパススルー）は、両実装で共通です。
:::
...
**Phase 3** が DI の核心です。Decode ノード (Consumer) が Prefill ノード (Producer) から、**NIXL ライブラリを使用して KV-Cache を直接 READ/PULL** します。
...
この設計の利点は、**並行処理**にあります。Prefill が first_token を返却した時点で、Proxy はすぐに Decode ノードにリクエストを送信できます。KV-Cache の転送と Decode 処理の準備が並行して進行するため、レイテンシーを最小化できます。また、Proxy が大容量の KV-Cache を中継しないことで、Proxy のネットワーク帯域幅とメモリ使用量を大幅に削減できます。

### レイヤ 2: Prefill/Decode Node と NIXL の役割

DI の内部実装は 3 つの主要コンポーネントで構成されます。**Prefill Node (Producer)** は Prefill 処理を実行し、生成した KV-Cache を GPU VRAM 上に保持します。[NixlConnector](https://github.com/vllm-project/vllm/blob/v0.16.0/vllm/distributed/kv_transfer/kv_connector/v1/nixl_connector.py) を通じて KV-Cache を NIXL Agent に登録し、**Consumer からの READ 要求を待機**します。
