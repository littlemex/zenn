...
:::message
**bytes_per_token の計算根拠**: これらのサイズは NIXLBench のデフォルト値 `bytes_per_token = 262144`（256KB/token）に基づきます。この値は Qwen2.5-32B-Instruct (TP=4) の全 TP ワーカー合計での KV-Cache サイズ `2 x 64 layers x 8 kv_heads x 128 head_dim x 2 bytes(bf16) = 262144 bytes/token` に相当します（各 TP ワーカーでは 65536 bytes/token ですが、NIXL 転送では全 TP ワーカーの KV-Cache を合計して転送します）。
:::
...
:::message alert
**128K tokens の測定制限**: 分散推論モード（L2-EFA, L3-TCP）では、128K tokens x c=8 以上で Consumer ノードの KV-Cache が GPU VRAM に収まらず OOM (Out Of Memory) が発生するため、c=1,4 のみ測定します（128K: 約 32GB/req x 8 並行 = 256GB > g6e.12xlarge の KV-Cache 利用可能容量（物理 VRAM 192GB からモデル重み等を除外））。一方、単一ノードモード（L1-Unified）では Prefill と Decode が同一 GPU で実行されるため、c=8 まで測定可能です。
:::
