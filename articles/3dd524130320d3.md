...
その下の **NIXL Library Layer** では、NIXL (Network Interconnect for XPU Linking) がデータ転送を抽象化しています。NIXL は複数のバックエンド（UCX、libfabric）をサポートしており、`backends` パラメータでどちらを使用するかを選択できます。これにより、アプリケーションレイヤのコードを変更せずに、異なる通信方式を試すことができます。

さらに下の **Communication Backend Layer** では、UCX (Unified Communication X) または libfabric (OpenFabrics Interfaces) がトランスポートプロトコルを選択する役割を担います。UCX の場合は `UCX_TLS` 環境変数で、libfabric の場合は `FI_PROVIDER` 環境変数でトランスポートを指定します。

その下の **Transport Layer** では、実際の通信プロトコルが動作します。TCP（標準的な TCP/IP スタック）、RDMA（Remote Direct Memory Access）、SRD Protocol（AWS の Scalable Reliable Datagram）のいずれかが使用されます。このレイヤがネットワークを介したデータ転送の方式を決定します。

最下位の **Hardware Layer** では、物理的なネットワークハードウェアが動作します。EFA Device（Elastic Fabric Adapter、高性能ネットワーク）、ENA Device（Elastic Network Adapter、標準ネットワーク）、Nitro Card（AWS のネットワークハードウェア）が実際のパケット送受信を行います。

### トランスポートレイヤの切り替え方法

vLLM DI の重要な設計上の特徴は、**アプリケーションレイヤのコードを変更せずに、環境変数だけでトランスポートレイヤを切り替えられる**ことです。

vLLM v0.16.0 の [KV Connector レジストリ](https://github.com/vllm-project/vllm/blob/v0.16.0/vllm/distributed/kv_transfer/kv_connector/factory.py#L146-L203)を確認すると、TCP 専用のコネクタ（TcpConnector）は存在しません。利用可能なコネクタは、NixlConnector、P2pNcclConnector、LMCacheConnectorV1、LMCacheMPConnector、MultiConnector、MoRIIOConnector、OffloadingConnector、DecodeBenchConnector、MooncakeConnector、ExampleConnector です。

TCP 通信を使用する場合も、同じ NixlConnector を使用し、UCX レイヤの環境変数 `UCX_TLS=tcp,self,sm` で TCP トランスポートを指定します。`kv_connector` の設定は変更せず、UCX レイヤの環境変数のみを変更することで、EFA と TCP の性能比較が可能になります。
...
1. **NixlConnector** → NIXL（`nixl_agent` API を使用）
2. **NIXL** → UCX または libfabric（`backends` パラメータで選択、デフォルト: `["UCX"]`）
3. **UCX/libfabric** → トランスポートレイヤ
   - UCX の場合: `UCX_TLS` 環境変数でトランスポートを制御（`tcp,self,sm` または `srd`）
   - libfabric の場合: `FI_PROVIDER=efa` で EFA を選択、`FI_EFA_USE_DEVICE_RDMA=1` で GPUDirect RDMA を有効化
4. **トランスポートレイヤ** → ハードウェア
   - EFA: SRD Protocol を使用し、Nitro Card のハードウェアで処理
   - TCP: ENA を使用

### 設定例とトランスポート確認

**EFA モード**:
- 環境変数: なし（デフォルト）、またはオプションで `FI_PROVIDER=efa` と `FI_EFA_USE_DEVICE_RDMA=1`
- 設定: `"backends": ["LIBFABRIC"]`

**TCP モード**:
- 環境変数: `UCX_TLS=tcp,self,sm` と `UCX_NET_DEVICES=all`
- 設定: `"backends": ["UCX"]`（UCX_TLS が効かない場合は `kv_transfer_config` に明示的に指定）

**トランスポート確認方法**:
- `UCX_LOG_LEVEL=info` を設定してログを確認
- TCP 使用: "using transport: tcp"
- RDMA 使用: "using transport: rc" or "ib"
- EFA 使用: "efa" or "libfabric"
...
そこで、通信スタックの最下レイヤ（Hardware Layer）から最上レイヤ（Application Layer）まで、**各レイヤを独立に測定するツールを用意**します。下のレイヤが正常であることを確認してから上のレイヤを測定することで、問題の原因を論理的に特定できます。

vLLM DI は、Application Layer（vLLM/NixlConnector）、NIXL Library Layer（NIXL）、Communication Backend Layer（UCX/libfabric）、Transport Layer（TCP/RDMA/SRD）、Hardware Layer（EFA/ENA）の 5 レイヤで構成されます。本セクションでは、各レイヤに対応する測定ツールと、それらを組み合わせて実装全体をカバーする方法を説明します。
...
**Fig 8: 測定の実行順序（Step 1 環境確認 → Step 2 下位レイヤ → Step 3 上位レイヤ）**
...
最初にハードウェアレイヤの健全性を確認します。EFA デバイスが正常に動作していなければ、上位レイヤの測定はすべて無意味です。iperf3 で TCP 帯域が期待値（9-10 Gbps）を満たしているか、fi_rdm_pingpong で EFA のレイテンシーが正常か、nvidia-smi で GPU の温度やメモリに異常がないかを確認します。この段階で問題が見つかれば、測定を開始する前にハードウェアの問題を解決できます。

**Step 2: L5-LowLevel -- 各レイヤの「基準値」を取得する**

ハードウェアが正常であることを確認した上で、Transport Layer と Library Layer の基準値を取得します。具体的には、fi_pingpong で EFA のレイテンシープロファイル（64B～100MB の 6 サイズ）を、ucx_perftest で UCX レイヤの帯域幅（cuda vs host）を、NIXLBench/KVBench で NIXL の転送性能をそれぞれ測定します。これらの基準値が、Step 4 の E2E 測定結果を解釈する「参照点」となります。
...
すべての測定データが揃った段階で、10 個のクロスバリデーション比較（CMP-01 から CMP-10）を実施します。各レイヤの測定結果が整合しているかを検証し、測定の信頼性を確保します。
...
第三に **再現性の確保** です。下位レイヤの測定が安定していることを確認することで、E2E 測定の信頼性を担保できます。たとえば iperf3 で TCP 帯域が 9 Gbps 未満の場合、ネットワーク問題を疑うことができます。
