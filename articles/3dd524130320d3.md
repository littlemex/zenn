...
:::message
**測定レイヤーの命名規則について**: レイヤー番号は通信スタックの深さではなく、**測定の実行フェーズ**を示します。L0 はハードウェア基盤の確認（Step 1）、L5 は低レベルツールによる基準値取得（Step 2）、L1-L3 は vLLM E2E 測定（Step 3-4）、L4 は分析フェーズ（Step 5）に対応します。実行順序は **L0 → L5 → L1 → L2 → L3 → L4** です。測定順序がおかしいのは試行錯誤の結果です。今後綺麗にします。
:::
...
| **L2-EFA** | EFA Disaggregated | 24 | EFA DI E2E 測定 | vLLM E2E 測定 (7 prompt × c=1,4,8,16) |
| **L3-TCP** | TCP Disaggregated | 24 | TCP DI E2E 測定 | vLLM E2E 測定 (7 prompt × c=1,4,8,16) |
...
:::message alert
**128K tokens の測定制限**: 128K tokens x c=8 以上では KV-Cache が GPU VRAM に収まらず OOM (Out Of Memory) が発生するため除外しています（128K: 約 32GB/req x 8 並行 = 256GB > g6e.12xlarge の 96GB VRAM）。
:::
