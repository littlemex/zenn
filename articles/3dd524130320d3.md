...
そこで、**まず E2E 測定で全体像を把握し、その結果を踏まえて低レベルツールで各レイヤを独立に測定**します。通信スタックの各レイヤ（Hardware、Transport、Backend、Library、Application）に対応する測定ツールを用意し、E2E の性能差がどのレイヤーに起因するかを段階的に特定します。
...
全体の実装解説を踏まえて、Phase 1 で測定する各メトリクスがどのように振る舞うかを理論的に予測します。これらは仮説であり、[測定編](0de1b80d35ccd0.md)で実測により検証する予定です。
...
複数の測定ツールを組み合わせて性能差の因果関係を段階的に検証するクロスバリデーション比較を実施します。これらの詳細については[測定編](0de1b80d35ccd0.md)で解説します。

## まとめ

本記事では、vLLM Disaggregated Inference の性能を理解するために、**レイヤードアプローチ**による体系的な独自の測定手法を提案しました。通信スタックを Application、Library、Backend、Transport、Hardware の 5 レイヤーに分離し、測定も同様に L0（Baseline）、L5（LowLevel）、L1-L3（E2E）の階層で実施することで、**各レイヤーの性能を独立に測定し、依存関係と値を分離**できます。さらに、複数の測定ツールを組み合わせた **10 個のクロスバリデーション比較（CMP-01 ~ CMP-10）**により、ハードウェアレイヤーの帯域差が上位レイヤーに伝播し、最終的に vLLM の TTFT に影響することを段階的に検証します。このアプローチにより、E2E のメトリクスだけでは見えない性能差の因果関係を明確にし、ボトルネックの所在を特定できます。

### vLLM v0.16.0 での試行錯誤と知見

vLLM v0.16.0 は pre-release 段階であり、それまでのバージョンと実装変更があった箇所も多く、試行錯誤が必要でした。NixlConnector の `kv_buffer_device` 設定による 3 つの転送パターン（GPUDirect RDMA、cuda buffer、cpu buffer）の違いや、NIXL バックエンドの選択（UCX/libfabric）が性能に与える影響を理解するために、低レベルツールを組み合わせた測定が不可欠でした。本記事で提案した階層的測定アーキテクチャは、このような実装の詳細を理解し、最適な設定を見つけるための体系的な手法として有効です。

### 将来の展望: Valkey による N: M スケーリング

Phase 1 では P2P (NIXL + EFA) アーキテクチャに焦点を当てましたが、将来的には **Valkey（Redis 互換 KV ストア）を用いた Remote Storage アーキテクチャ**の測定も予定しています。
...
P2P は低レイテンシー（1 ホップ、50-200ms）だが 1:1 の固定ペアに制限されます。一方、Valkey は 2 ホップ（100-400ms）のレイテンシーと引き換えに、N: M の動的スケーリングと Prefix Caching による KV-Cache 再利用が可能になります。
...
**Phase 2 以降の測定計画**: L6-Valkey レイヤーを追加し、P2P モードと Valkey モードの TTFT を比較します（CMP-11, CMP-12）。NIXLBench による直接転送と Valkey 経由転送の性能差を測定し（CMP-13）、Multi-Consumer スケーラビリティを評価します（CMP-14）。GPUDirect RDMA 対応インスタンス（P5/P5en）での測定により、P2P のゼロコピー転送と Valkey のスケーラビリティのトレードオフを定量化します。
