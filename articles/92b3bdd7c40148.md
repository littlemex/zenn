...
Neuron Distributed Inference (NxD Inference) が Vision Language Models のサポートを大幅に拡充しました。
...
Neuron SDK 2.28.0 では、開発者の生産性向上とパフォーマンス最適化を支援する重要なアップデートが提供されました。

第一に、Neuron Explorer に System Profiling、Tensor Viewer、Database Viewer が追加され、包括的なプロファイリング環境が整いました。特に Database Viewer では自然言語クエリでプロファイリングデータを分析できるようになり、ボトルネック特定の効率が大幅に向上しています。

次に、Qwen2/Qwen3 VL と Flux.1 Inpainting のサポートにより、Vision Language Models の適用範囲が拡大しました。これにより、マルチモーダル AI アプリケーションの開発が容易になります。

第三に、NKI Library に RoPE や MoE Operations などの最適化カーネルが追加され、Transformer モデルの推論性能が向上しています。さらに、NKI Beta 2 では LNC Multi-core がサポートされ、複数の NeuronCore を協調させたカーネル実行が可能になりました。

第四に、Kubernetes の DRA (Dynamic Resource Allocation) API に対応した Neuron DRA Driver が導入され、Kubernetes ネイティブなリソース管理が実現しました。従来の Device Plugin と比較して、より柔軟かつ細粒度なリソース制御が可能です。

最後に、PyTorch 2.10 以降では PyTorch/XLA から Native PyTorch への移行が必須となります。PyTorch 2.9 が最終の PyTorch/XLA 対応版であり、既存のワークロードをお持ちの方は早期の移行計画策定が推奨されます。
