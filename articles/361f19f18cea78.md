...
最初、 vision encoder と text decoder を `NeuronApplicationBase` 直接継承で別々の Application として書き、 上位の VLM 制御は自前ループで書こうとしましたが `HuggingFaceGenerationAdapter.generate()` が要求する `forward()` シグネチャ (画像 + テキスト + KV cache + 位置情報をまとめて受ける) を素直に実装すると、 prefill bucket 切り替えや CTE/TKG 切替の制御が NxDI 内部実装と二重管理になりました。 NxDI 同梱の `NeuronQwen2VLForCausalLM` が **その制御を全部やってくれている**ことに気づき、 これを継承して `text_model_cls` / `vision_model_cls` だけ差し替える今の形に倒しました。

### 経緯 2: 「Qwen2-VL クラスを vision でもそのまま使う」→「Qwen2.5-VL 用に書き直す」

NxDI 同梱の `NeuronQwen2VLVisionModel` をそのまま `vision_model_cls` に指定して checkpoint をロードしたら、 大量の missing/unexpected key warning と共に **vision embedding が完全に壊れました**。 原因は Qwen2-VL → Qwen2.5-VL で vision tower が置き換わっていたことです。
