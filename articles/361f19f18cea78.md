...
### なぜ NxD Inference 直叩き経路なのか

vLLM Neuron plugin (v0.16) の `MODEL_TYPES` に `qwen2_5_vl` が登録されていません（`qwen2_vl`・`qwen3_vl` などは対応済みですが、`qwen2_5_vl` という model_type は未登録）。`vllm serve` 経由ではロード時に次のエラーで落ちます。

```text
ValueError: Model Qwen2_5_VLForConditionalGeneration is not supported on Neuron for now.
Supported models: ['gpt_oss', 'llama', 'llama4', 'mllama', 'mistral', 'mixtral',
                   'pixtral', 'dbrx', 'qwen2', 'qwen2_vl', 'qwen3', 'qwen3_moe',
                   'gemma3', 'qwen3_vl']
```

そこで本サンプルは vLLM を経由せず、**NxDI を直接叩き、HuggingFace の `GenerationMixin` 互換アダプタ (`HuggingFaceGenerationAdapter`) で `generate()` する**経路を採用します。

```mermaid
flowchart LR
    subgraph CPU["CPU"]
        ADAPTER["HuggingFaceGenerationAdapter<br>(generate ループ)"]
        PROC["AutoProcessor<br>(画像前処理)"]
    end
    subgraph NeuronChip["Neuron デバイス"]
        VISION["NEFF: vision encoder"]
        CTE["NEFF: text CTE (prefill)"]
        TKG["NEFF: text TKG (decode)"]
    end
    PROC --> ADAPTER
    ADAPTER -->|画像| VISION
    ADAPTER -->|prefill| CTE
    ADAPTER -->|decode 1 step| TKG
    VISION -->|vision embeds| CTE
```
