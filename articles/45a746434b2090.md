...
1. **In-Process Recovery**: 「どうやって復旧するか」
   - 障害発生時に、プロセスを終了せずに RCB (Re-Executable Code Block) を再実行することで、サブ秒レベルでの復旧を実現
...
::::details In-Process Recovery と Checkpointless Recovery の技術詳細
...
従来の障害復旧では、どのフェーズで障害が発生しても、常に前のステップの開始地点までロールバックしていました。これは、Optimizer Step の途中で障害が発生した場合、健全なレプリカが既に完了した計算結果を破棄することを意味します。例えば、8 つのレプリカのうち 1 つが Optimizer Step の 90% 完了時点で障害を起こすと、残り 7 つの健全なレプリカが完了した Optimizer Step をすべて捨てて、前のステップから再計算する必要があります。
...
Backward Pass が完了すると、各レプリカは Optimizer Step に入る前に**Atomic Lock を取得**します。このロックは、Optimizer Step が完了するまで保持されます。
...
Atomic Lock による保護がない場合、Optimizer Step 中の障害によって、健全なレプリカが完了した計算がすべて無駄になります。Optimizer Step は学習ステップ全体の 10-20% 程度の時間を占めますが、完了すると新しいモデル状態が確定するため、これを破棄して再計算するコスト（メモリアクセス、通信オーバーヘッド）は無視できません。

Atomic Lock を使用することで、健全なレプリカが完了した Optimizer Step の計算結果を保持でき、**計算リソースの無駄を最小限に抑えながら、サブ秒レベルでの障害復旧**を実現します。特に、大規模クラスターで障害頻度が高い環境では、この最適化によって実効学習時間（goodput）が大幅に向上します。
