...
Amazon SageMaker HyperPod の **Checkpointless Training** は、**チェックポイントをストレージに書き込むことなく、障害から復旧する**機能です。従来の大規模分散学習では、ハードウェア障害に備えて定期的にモデルの状態をストレージ（S3 や FSx）に保存する必要があり、これが学習のボトルネックとなっていました。

AWS の公式発表によれば、数千台の AI アクセラレータを持つクラスターにおいて **95% 以上の学習 goodput**（実効学習時間の割合）を実現し、障害からの自動復旧を**数分以内**に完了します[^1]。

本記事では、Checkpointless Training の技術的な仕組み、4 つのコアコンポーネント、導入方法について 2026 年 2 月時点の情報をもとに詳しく解説します。
...
**In-Process Recovery と Checkpointless Recovery の技術詳細**
...
:::message
以下の説明は、Optimizer Step のみに焦点を当てた概念的な説明です。実際には、ロックはバッチ開始時に取得されますが、Forward/Backward Pass 中の障害は前のステップからの再実行で対処できるため、Optimizer Step の保護が最も重要になります。
:::
...
4. **NeMo 統合**: NVIDIA NeMo Framework との統合により、コード変更最小で導入可能

**導入時の考慮事項**:

- **環境制約**: 2026 年 2 月時点では HyperPod EKS 環境のみで利用可能
- **フレームワーク要件**: NeMo Framework が必須（Megatron-Core の冗長インスタンス機能に依存）
- **Managed Tiered Checkpointing との併用**: カタストロフィックな障害に備えた永続バックアップとの併用を推奨
