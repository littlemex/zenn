...
1. **Megatron-Core の冗長インスタンス機能がコア技術**: Checkpointless Training は Megatron-Core の `num_distributed_optimizer_instances >= 2` という機能に依存します。この機能により、オプティマイザ状態の冗長コピーを GPU メモリ内に保持できます。
...
AWS 公式ドキュメントでは Checkpointless Training を [3 つの最適化トラック](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-eks-checkpointless-features.html) として説明していますが、本記事では理解を深めるため、3 番目の最適化トラックを 2 つのコンポーネントに分けて説明します。
...
::::details Optimized Collective Communication Initialization の技術詳細
...
- オリジナルと最適化パスの柔軟な切り替えをサポートし後方互換性を維持
...
**3. 分散学習層**
- PyTorch Distributed 層での TCPStore 作成をバイパス
- グローバルグループカウンターを通じて対称アドレスパターンを維持

### 実装例

GitHub 上の実装例:
[launch/pretrain_llama3_70b_checkpointless_p5.yaml#L111-L113]()
...
::::details Memory-Mapped Data Loading の技術詳細
...
実装例は [HyperPod Recipes の NeMo サンプル](https://github.com/aws/sagemaker-hyperpod-recipes/tree/main/recipes_collection/training-recipes/nemo)を参照してください。

MMAP の有効化は、既存の PyTorch Lightning DataModule を `MMAPDataModule` でラップするだけで完了します。

**ステップ 1: MMAP 設定の作成**

まず、キャッシュの動作を定義する設定オブジェクトを作成します。

```python
from hyperpod_checkpointless_training.dataloader.config import CacheResumeMMAPConfig

mmap_config = CacheResumeMMAPConfig(
    cache_dir='/dev/shm/pdl_cache',       # 高速な共有メモリにキャッシュを配置
    prefetch_length=10,                   # 10 バッチ先行してプリフェッチ
    lookback_length=2,                    # 過去 2 バッチを保持（再起動時の即座再開用）
    checkpoint_frequency=100              # 100 ステップごとにキャッシュを整理
)
```

**ステップ 2: 既存の DataModule をラップ**

次に、既存の DataModule を MMAP でラップします。

```python
from hyperpod_checkpointless_training.dataloader.mmap_data_module import MMAPDataModule

# 既存の DataModule（変更不要）
original_data_module = MyLightningDataModule(...)
...
**ステップ 3: PyTorch Lightning で使用**

ラップした DataModule を通常通り Trainer に渡すだけです。

```python
import pytorch_lightning as pl

trainer = pl.Trainer(max_epochs=10)
trainer.fit(model, datamodule=mmap_data_module)

# プロセス再起動時は、キャッシュから自動的に再開されます
```
::::
...
従来の障害復旧では、全プロセスを停止し、ディスクからチェックポイントをロードして再起動する必要があり、復旧に数分〜数十分を要していました。また、前回のチェックポイント以降の学習進行が失われるため、計算リソースが無駄になっていました。In-Process Recovery は、障害発生時に健全なレプリカのモデル・オプティマイザ状態を故障したレプリカに転送することで、プロセスを終了せずに復旧し、ディスク I/O を完全に排除してサブ秒レベルでの復旧を実現します。

::::details In-Process Recovery の技術詳細
...
| フェーズ | 障害発生時の動作 | ロック状態 | 結果 |
|---------|----------------|-----------|------|
| Forward Pass | ステップ開始にロールバック、モデル状態をブロードキャスト | ロック取得前 | 前のステップから再実行 |
| Backward Pass | ステップ開始にロールバック、モデル状態をブロードキャスト | ロック取得前 | 前のステップから再実行 |
| Optimizer Step | 健全なレプリカがロック保護下で完了後、更新状態をブロードキャスト | **ロック保護下** | オプティマイザ更新は保持され、最新状態で再開 |

**主要戦略**: 健全なレプリカがロック保護下でオプティマイザステップを完了し、その後更新されたモデル状態をブロードキャストできるようにします。これにより、完了したオプティマイザ更新が破棄されることがなくなり、計算リソースの無駄を防ぎながら障害復旧時間が短縮されます。
...
従来の分散学習では、障害復旧のためにディスクにチェックポイントを定期的に保存する必要があり、大規模モデルでは数十 GB〜数百 GB のディスク I/O が学習のボトルネックとなっていました。また、単一ノード障害時にそのノードが保持していたモデルシャードが失われ、チェックポイントからの復旧が必須でした。Checkpointless Recovery は、In-Process Recovery と組み合わせることで、ディスクチェックポイントを完全に排除した障害復旧を実現します。モデルレプリカを複数のノードグループに分散配置することで、ノード障害時でも健全なレプリカから状態を復旧できます。
...
**FSx for Lustre との併用について**: Managed Tiered Checkpointing と FSx for Lustre + DRA は技術的に併用可能です。ただし、どちらも S3 にチェックポイントを永続化するため、通常は併用不要です。FSx が構築済みの場合は、データセット保存に FSx を使用し、チェックポイント保存には Managed Tiered Checkpointing を使用する使い分けを推奨します（詳細はセクション 3.6 参照）。
:::
...
従来方式には、学習のブロック時間（5-30 秒）、FSx の高コスト（月額$140-$500/TB）、S3 エクスポート遅延、容量管理の課題がありました。Managed Tiered Checkpointing は、これらの課題を以下のように解決します。
...
従来方式ではチェックポイント保存中に学習がブロックされます（図の赤色部分）。一方、Tiered Checkpointing では Tier 1（RAM）へのステージング（1-2 秒）のみで学習を継続でき、S3 アップロードはバックグラウンドで非同期に実行されます。
...
`SageMakerCheckpointConfig` の主要パラメータを以下に示します。
...
| 学習ブロック時間の最小化 | 1-2 秒のステージングのみ | 5-30 秒 |
...
Slurm 環境では HyperPod 管理プレーンがシステムデーモンとして各ノードにインストールされ、EKS 環境では DaemonSet として Kubernetes クラスター内にデプロイされます。
