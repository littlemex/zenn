...
Llama、Qwen、DeepSeek のような標準的なアーキテクチャを使用している場合、すでに用意されているレシピを使えばコード変更ゼロで始めることができます。必要なのは、データを持ち込み、最小ノード数と最大ノード数を設定するだけなので、多くのケースでは特に裏側の実装を気にすることはないかもしれません。以下に Llama3 のサンプルがあります。
...
障害が検出されると、システムは**現在の学習ステップの先頭までロールバック**します。健全なレプリカは、前のステップで完了したモデル状態（既に Optimizer Step で更新済みの重み）をブロードキャストし、すべてのレプリカがその状態から新たに Forward Pass を再実行します。この場合、失われるのは現在のステップの計算途中の中間結果だけなので、チェックポイント間隔に依存する従来方式と比較して損失は最小限です。

**Case 2: Optimizer Step 中の障害（Atomic Lock 保持中）**

学習バッチの実行中、各レプリカは **Atomic Lock (`ParameterUpdateLock`)** を保持します。このロックはバッチ全体（Forward Pass、Backward Pass、Optimizer Step）を保護し、バッチ完了時（`on_train_batch_end`）に解放されます。公式ドキュメントでは `on_train_batch_end` でロックが解放されることが記載されています。取得タイミングの詳細は公式ドキュメントに明示されていませんが、バッチの開始から終了までの間保持されると考えられます。ロックが保持されている間、Checkpointless Recovery は無効化されます。
...
@HPWrapper(
    ...
    finalize=CheckpointlessFinalizeCleanup(), # 終了時のクリーンアップ
)
...
| `finalize` | 学習終了時または障害復旧後のクリーンアップ処理 |
...
**配置の特徴**:
- 各レプリカは物理的に異なるノードに配置
- 同一レプリカ内（Replica 1 や Replica 2）の各ランクは、reduce-scatter と all-gather を通じてオプティマイザ状態のシャードを共有・同期
- レプリカ間（Replica 1 ↔ Replica 2）では、通常時も all-reduce で同期が行われ、この同期が冗長性を維持して障害時の復旧を可能にします
...
2. 復旧実行可能性の評価（`CheckpointManager.checkpointless_recovery_feasible()` メソッドで検証）
...
- **影響範囲**: 高精度のマスター重み/勾配とオプティマイザ状態のみ（低精度の重み・勾配・アクティベーションメモリはモデル並列でシャードされたまま）
- **増加量**: `num_distributed_optimizer_instances=2` の場合、クラスター全体で約 1 DCP (Distributed Checkpoint) チェックポイントサイズ分のメモリが追加で必要
- **例**: DCP チェックポイントが 50 GB の場合、クラスター全体で追加 50 GB が GPU 間で分散して消費される

**トレードオフ**:
- **メリット**: ディスク I/O を完全に排除、復旧時間の大幅短縮
- **デメリット**: デバイスメモリ使用量の増加（ただし影響は高精度の状態のみ）
...
**結果**:
- 通信量は冗長レプリカの同期分だけ増加しますが、公式ドキュメントでは「overall communication volume remains roughly unchanged（おおよそ変わらない）」と記載されています
...
これにより、ディスク I/O を排除し、高速な障害復旧と学習進行の損失最小化を実現します。
...
3. **In-Process Recovery** + **Checkpointless Recovery** の統合
   - プロセス内復旧メカニズムとモデルレプリカからの状態転送を組み合わせることで、ディスク I/O を完全に排除し、高速な障害復旧を実現
...
Amazon SageMaker HyperPod の Checkpointless Training は、大規模分散学習における耐障害性の新しいパラダイムを提供します。本記事で解説した 3 つの最適化トラック（Optimized CC Initialization、MMAP、Program Restart Overhead Reduction。3 つ目は In-Process Recovery と Checkpointless Recovery で構成）が相互に連携することで、従来は数十分かかっていた障害復旧を数分以内に短縮し、95% 以上の goodput を実現します。
