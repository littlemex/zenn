---
title: "vllm-neuron プロファイリング練習で得た知見"
emoji: "🔍"
type: "tech"
...
カスタムカーネルを実装するようなケースでは必須と言えますが、最適な設定を探す際の初手で実施する必要があるものではありませんでした。そしてハードウェアレベルの改善をする前に vllm-neuron 全体の実行時間の内のどの程度をハードウェア側の推論処理が占めているのかによって改善の優先度が変わってくるので vllm-neuron 全体のプロファイリングをしないと意味ないぞ、と思いました。。

### 1.5 NEFF、NTFF、Perfetto の違いと使い分け

Phase 1 で登場した 3 つのファイル形式と分析ツールの違いを整理します。
...
**分かること（静的情報）**:
- ✅ グラフ構造: 373 ノード、458 テンソル
- ✅ テンソル形状とデータ型: `custom_call.17` は `float32 [256, 1, 1]`
- ✅ 演算量: HloMacCount 29.2B（理論値）
- ✅ メモリ使用量: IfmapSize 1.14GB、OfmapSize 448MB
- ✅ DMA キュー構成: 16 キュー × 複数エンジン

**分からないこと（動的情報）**:
- ❌ 実際の実行時間
- ❌ 実行回数
- ❌ 初期化遅延（skip_warmup の影響）
- ❌ 具体的な演算ロジック
...
#### NTFF (Neuron Trace File Format)

**役割**: 実行時の NeuronCore トレースファイル

```bash
# NTFF ファイルの例
profile_output_10queries/i-0049acfde6046f237_pid_520024/
...
**分かること**:
- ✅ グラフごとの実行時間: Graph 1 は 10.81ms、Graph 2 は 69.79ms
- ✅ NeuronCore ごとの内訳: vnc_0 と vnc_1 の実行時間
- ✅ 使用されたグラフの識別

**分からないこと**:
- ❌ オペレーションレベルの詳細（MATMUL、COPY など）
- ❌ Python レイヤーのオーバーヘッド
- ❌ グラフ間の遷移時間
...
- ❌ テンソル形状やデータ型
- ❌ グラフ構造
- ❌ 演算内容の意味（custom_call が何をしているか）

**活用シーン**:
- 実行時性能測定
- ボトルネック特定
- skip_warmup などのパラメータ効果測定
- 最適化前後の比較
...
%%{init: {'theme':'dark'}}%%
graph TD
    A[プロファイリング目的] --> B{何を知りたい？}

    B -->|グラフ構造<br/>テンソル形状| C[NEFF 分析]
    B -->|実行時間<br/>ボトルネック| D[Perfetto 分析]
    B -->|グラフ選択<br/>bucketing 効果| E[NTFF 分析]

    C --> F[unpacking]
    F --> G[tensor_map.json<br/>hlo_stats.json]

    D --> H[TraceProcessor]
    H --> I[SQL クエリ]

    E --> J[ntff_summary.json]
    J --> K[グラフ別実行時間]

    style A fill:#1a1a2e,stroke:#16213e,color:#fff
    style B fill:#1a1a2e,stroke:#16213e,color:#fff
    style C fill:#0f4c75,stroke:#16213e,color:#fff
    style D fill:#0f4c75,stroke:#16213e,color:#fff
    style E fill:#0f4c75,stroke:#16213e,color:#fff
    ...
    style J fill:#1a1a2e,stroke:#16213e,color:#fff
    style K fill:#1a1a2e,stroke:#16213e,color:#fff
```

| 目的 | NEFF | NTFF | Perfetto |
|------|------|------|----------|
| **カスタムカーネル検証** | ✅ 最適 | ❌ | ⚠️ 補助 |
| **実行時性能測定** | ❌ | ⚠️ グラフ単位 | ✅ 最適 |
| **bucketing 評価** | ⚠️ サイズ | ✅ 最適 | ⚠️ 補助 |
| **ボトルネック特定** | ❌ | ❌ | ✅ 最適 |
| **メモリ最適化** | ✅ 最適 | ❌ | ❌ |
| **skip_warmup 効果** | ❌ | ❌ | ✅ 最適 |

**推奨ワークフロー**:
1. **NEFF** で静的構造を理解（何が使われているか）
2. **Perfetto** で実行時挙動を測定（どう動いているか）
3. **NTFF** で bucketing の効果を評価（グラフ選択は適切か）

**今回のプロファイリングでの活用**:
- Phase 1: **Perfetto** で `custom_call.17` の実行時間を測定 → 7ms、36 回
- 補足調査: **NEFF** で `custom_call.17` の構造を確認 → `float32 [256, 1, 1]`、RoPE の可能性
- bucketing 分析: **NTFF** と **NEFF** で 11 グラフの存在を確認 → 合計 124MB

この 3 つを組み合わせることで、コンパイル時の設計から実行時の動作まで、完全な理解が得られます。
