---
title: "vllm-neuron プロファイリング練習から得られた知見"
emoji: "🔍"
type: "tech"
...
[前回の記事](https://zenn.dev/tosshi/articles/d68bd091d1934d) では Reranker ベンチマークツールの開発に注力しましたが、今回は AWS Inferentia2 上で vllm-neuron を使用した実際のパフォーマンスをプロファイリング分析します。

本記事では、3 つの調査フェーズを実施しました。まず Phase 1 では AWS Neuron Profiler によるハードウェアレベルのプロファイリングを試行錯誤しました。次に Phase 2 では line_profiler による Python レベルのプロファイリングを実施しました。最後に Phase 3 では NxD Inference を直接使用した測定を行い、vLLM との詳細比較を実施しました。
...
AWS のブログ記事 [「Unlocking Machine Learning Model Performance」](https://builder.aws.com/content/2x3ldAuia9gjjUixpOt162ttkK2/unlocking-machine-learning-model-performance-from-bottlenecks-to-breakthroughs) は AWS Neuron Profiler の理解にとても有用です。
...
詳細に試行錯誤しながら実行したクエリを全て紹介していると膨大になってしまうため割愛しますが Phase 1 の時系列の命令実行に関する調査結果から、custom_call が初回実行時に大きな遅延を起こしていることが分かったため、neuronx-distributed-inference(以降、NxDI) のパラメータである `skip_warmup=False` を試しました。デフォルト値は `False` なのですが以前の実験の試行錯誤で `True` にしていました。このパラメータはモデルロード後に 1 回の forward 実行を行い、遅延初期化を完了させるというものです。
...
ウォームアップすれば速くなると予想していましたが、実際には約 4% 遅くなりました。（測定のたびに結果は多少変動します）
...
Warmup (skip_warmup=False):
  ...
  しかし、ACTIVATEとCOPYが大幅に増加し、トータルでは遅化
```

:::

  skip_warmup=False で性能が悪化する理由

  記事で観察された「skip_warmup=False で約 4% 遅くなった」現象は、以下が原因です：

  1. Warmup 自体がオーバーヘッド
    - モデルロード時に追加で数秒の warmup 実行
    - ベンチマークでは頻繁にモデルをロード/アンロードするため、この時間が累積
  2. MATMUL/LDWEIGHTS の実行回数減少の理由
    - Warmup で初期化が完了すると、一部の演算がキャッシュされたり最適化される
    - しかし、ACTIVATE と COPY が大幅に増加し、全体では遅化
  3. Neuron Profiler の自動 warmup
    - Profiler は自動的に 2 回実行し、1 回目を warmup として扱う（--num-exec 2 --profile-nth-exec 2）
    - そのため、skip_warmup=True でも正確な測定が可能

### 1.4 Neuron Profiler の測定範囲の限界

:::message alert
ここで重要な気づき：Neuron Profiler のトレース時間は16-17msなのに、ベンチマーク全体は約3秒かかっている。**この16-17msってどこからどこまで？**
:::

調査の結果、Neuron Profiler の測定範囲に関する重要な特性が判明しました。各 NTFF ファイルは 1 つのコンパイル済みグラフを表しており、異なるバッチサイズや sequence length 用の複数グラフが存在します。このため、実際のバッチ処理時間を直接測定することができません。
...
**結論**: Neuron Profiler はハードウェアレベルの分析には有用だが、アプリケーションレベルの処理時間測定には向かない。

---

## Phase 3-5: line_profiler による Python プロファイリング

### 3.1 測定対象の理解

**測定対象**: 1クエリ（20候補文書のリランキング）を処理する時間

```yaml
reranker:
  search_num: 20        # 1クエリあたり20候補文書
  batch_size: 8         # 8ペアずつバッチ処理

vllm:
  max_num_seqs: 4       # vLLMの同時処理数
```

**処理フロー**:
```
1クエリ = 20ペア ÷ batch_size=8 = 3バッチ
10クエリ = 30バッチ
合計時間 = 2,992ms → 1クエリあたり約300ms
```

### 3.2 line_profiler 測定結果
...
### 3.3 マルチプロセスアーキテクチャの壁

**衝撃的な発見**: `get_output()` の **100% が `outputs_queue.get()`** です！
...
**重要な制約**:
- line_profiler は**メインプロセスのコードのみ**測定できる
- Workerプロセスで実行される関数（`execute_model()` など）は測定できない
- `outputs_queue.get()` の13.962msには以下のすべてが含まれている：
 1. **Neuron 実行時間** (Worker プロセス内)
 2. **シリアライゼーション** (Worker プロセス内)
 3. **Ray IPC (ZMQ 通信)** (プロセス間)
 4. **デシリアライゼーション** (メインプロセス内)

### 3.4 測定の限界

**Phase 3-5時点で分かっていたこと**:
```
llm.generate() = 126.052 ms/batch
├─ LLMEngine.step() × 7.6 = 106.1 ms (84.2%)
│  └─ engine_core.get_output() = 13.962 ms/step × 7.6
│     └─ outputs_queue.get() = 13.950 ms/step (100%)
│        ├─ Neuron 実行: ??? ms          ← Phase 3-5時点では不明
│        ├─ シリアライゼーション: ??? ms  ← Phase 3-5時点では不明
│        └─ Ray IPC: ??? ms              ← Phase 3-5時点では不明
└─ その他: 19.9 ms (15.8%)
```

:::message
**Phase 6で解決！**: NxD Inference 直接測定により、Neuron 実行時間は **12.06 ms/batch** と確定しました。これにより、vLLM オーバーヘッドは **52.27 ms (81%)** であることが判明しました。詳細は次の Phase 6をご覧ください。
:::

**教訓**

Phase 3-5 を通じて、測定ツールの限界が明確になりました。Neuron Profiler はバッチ処理時間を直接測定できず、line_profiler は Worker プロセスの処理が見えません。この経験から、複雑なシステムでは単一のツールでは全体像を把握できないという測定の本質が明らかになりました。

**次のアプローチ**

この状況を打開するため、NxD Inference（neuronx-distributed-inference）を直接使った測定でベースラインを取得する必要があります。

---

## Phase 6: NxD Inference 直接測定と徹底比較

### 6.1 測定戦略の転換

Workerプロセスの直接測定が困難なため、**vLLM を使わずに Neuron SDK を直接使用**して純粋な Neuron 実行時間を測定することにしました。
...
Neuron SDK: 2.27.x
NxD Inference: 0.7.0
vLLM: 0.13.0 (Neuron対応版)
Python: 3.12

# モデル
Model: Qwen3-0.6B-Reranker
Prompt length: 97 tokens (固定長)
```

### 6.3 vLLM 設定（Zenn 最適値）

```yaml
vllm:
  tensor_parallel_size: 2           # 2 NeuronCores
  max_num_seqs: 4                   # Batch size
  block_size: 32                    # KV cache block size
  max_model_len: 2048
  max_num_batched_tokens: 256
  num_gpu_blocks_override: 512
  enable_prefix_caching: false      # 今回は無効
  dtype: "bfloat16"

  # Neuron-specific overrides (Zenn最適値)
  additional_config:
    override_neuron_config:
      enable_bucketing: true        # 重要！
      pa_num_blocks: 512
      pa_block_size: 32
```

参考: [Zenn 記事 - Inf2で vLLM を動かす際のパフォーマンスチューニング](https://zenn.dev/tosshi/articles/ef61e14fe73399)
...
graph LR
    subgraph NxD["NxD Inference (12.05 ms)"]
        A[Neuron 実行<br/>12.05 ms<br/>100%]
    end

    subgraph vLLM["vLLM (64.33 ms)"]
        B[Neuron 実行<br/>12.05 ms<br/>19%]
        C[Python オーバーヘッド<br/>52.28 ms<br/>81%]
        B -.-> C
    end
...
%%{init: {'theme':'dark'}}%%
graph TD
    subgraph vLLM["vLLM での結果"]
        A1["bucketing=False<br/>126.05 ms"] -.->|2倍高速化| A2["bucketing=True<br/>64.33 ms ✅"]
    end

    subgraph NxD["NxD Inference 直接"]
        B1["bucketing=OFF<br/>12.06 ms ✅"] -.->|1.96倍低速化| B2["bucketing=ON<br/>23.60 ms ⚠️"]
    end
...
%%{init: {'theme':'dark'}}%%
graph TD
    subgraph GPU["通常の vLLM (GPU など)"]
        A1[Scheduler<br/>スケジューリング] --> A2[KVCacheCoordinator]
        A2 --> A3[Hash-based prefix caching<br/>PagedAttention<br/>Block allocation/eviction<br/>LRU eviction policy]
        A3 --> A4[GPU Worker<br/>実行]
    end

    subgraph Neuron["vllm-neuron"]
        B1[Scheduler<br/>full_context_lens 計算<br/>computed_context_lens 計算] --> B2["❌ KVCacheCoordinator<br/>(使用されない)"]
        B2 -.->|bypass| B3[vllm-neuron Plugin]
        B3 --> B4[is_prefix_caching 設定<br/>メタデータを Neuron に渡す]
        B4 --> B5[NxD Inference<br/>Neuron レベル]
        B5 --> B6[Prefix caching 実装<br/>KV キャッシュ管理<br/>ハードウェアアクセラレーション]
    end
