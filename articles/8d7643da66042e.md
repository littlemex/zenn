...
**パターンA: bucketing=OFF, prefix-caching=OFF（最速）**
...
python src/neuronx_distributed_inference/inference_demo.py \
  ...
  --max-length 129 \
  --benchmark \
  --benchmark-report-path /tmp/benchmark_pattern_a.json

# 結果確認
cat /tmp/benchmark_pattern_a.json | jq
```
...
**パターンB: bucketing=ON, prefix-caching=OFF（1.96倍遅い）**
...
python src/neuronx_distributed_inference/inference_demo.py \
  ...
  --enable-bucketing \
  --benchmark \
  --benchmark-report-path /tmp/benchmark_pattern_b.json

# 結果確認
cat /tmp/benchmark_pattern_b.json | jq
```
...
**パターンC: bucketing=OFF, prefix-caching=ON（エラー）**
...
python src/neuronx_distributed_inference/inference_demo.py \
  ...
  --max-context-length 128 \
  --max-length 129 \
  --enable-variable-length-prefill \
  --benchmark \
  --benchmark-report-path /tmp/benchmark_pattern_c.json

# エラーが発生
# ValueError: enable_bucketing must be True when enable_variable_length_prefill is True
```

**パターンD: bucketing=ON, prefix-caching=ON（エラー）**
...
python src/neuronx_distributed_inference/inference_demo.py \
  ...
  --enable-variable-length-prefill \
  --benchmark \
  --benchmark-report-path /tmp/benchmark_pattern_d.json

# エラーが発生
# prefix caching 関連のエラー
```
...
|---------|-----------|----------------|------|---------------------|
| **A** | OFF | OFF | ✅ 成功 | **12.06** |
| **B** | ON | OFF | ✅ 成功 | **23.60** (1.96倍遅い) |
| **C** | OFF | ON | ❌ エラー | - |
| **D** | ON | ON | ❌ エラー | - |

:::message alert
**重要な発見**: bucketing を有効にすると、同一長プロンプト（97トークン）の連続処理では **1.96倍遅くなる** という予想外の結果が得られました。これは Phase 1 で vLLM 経由で測定したときの結果（bucketing で高速化）とは逆の傾向です。
:::
...
1. **vLLM オーバーヘッド**（reranker ワークロード、前回の Zenn 最適値設定）
 - vLLM: 64.33 ms/batch
 - NxD: 12.06 ms/batch
 - オーバーヘッド: +52 ms (81%)
