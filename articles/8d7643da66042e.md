...
[前回の記事](https://zenn.dev/tosshi/articles/d68bd091d1934d) ではプロファイリングツールの開発にほぼ趣味で集中してしましたが、今回は（真面目に） Inferentia2 上で vllm-neuron を使用したパフォーマンスのプロファイリング分析にチャレンジします。
...
**ソフトウェアバージョン**:
- Neuron SDK: 2.27.0
- vllm-neuron: 0.13.0 対応（vLLM の Neuron プラグイン）
- neuronx-distributed-inference (NxD Inference): 0.7.14366
- Python: 3.12
...
試行錯誤しながら実行したクエリを全て紹介していると膨大になってしまうため割愛しますが custom_call が初回実行時に大きな遅延を起こしていることが分かったため、NxD Inference のパラメータである `skip_warmup` の効果を検証しました。デフォルト値は `False`（ウォームアップ実行）なのですが以前の実験の試行錯誤で `True`（ウォームアップスキップ）にしていました。`skip_warmup=False` の場合、モデルロード後に 1 回の forward 実行を行い、遅延初期化を完了させます。
...
**影響を受けるケース:**
- NxD Inference 0.7.14366（SDK 2.27.0）の `HuggingFaceGenerationAdapter` を使用
