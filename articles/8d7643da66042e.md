...
このベンチマークのワークロード:
- **固定プロンプト**（長い・共通）+ **多様なクエリ**（短い・異なる）
- プロンプト部分はキャッシュヒットするが、クエリ部分は毎回異なる
...
2. **オーバーヘッドは全ブロックで発生**
   - ハッシュ計算: 全ブロック（プロンプト + クエリ）
   - キャッシュ lookup: 全ブロック
   - LRU 管理: 全キャッシュエントリ

3. **NeuronCore のメモリアクセスパターン変化**
   - 連続レイアウト → ブロックレイアウト
   - DMA 転送パターンの変化による潜在的なオーバーヘッド

### 測定条件の影響

今回の測定では：
- **固定長ワークロード**: 全く同じプロンプト4つを100回繰り返し
- **可変長ワークロード**: 同じ4つの異なるプロンプトを100回繰り返し

つまり、**2 回目以降は完全にキャッシュヒットするはず**の理想的な条件でした。それにも関わらずレイテンシが悪化したことから、**Prefix Caching に伴うオーバーヘッド（ソフトウェア管理処理 + NeuronCore の KV cache レイアウト変更）が、キャッシュヒットの恩恵を打ち消している**と考えられます。

::::

根本原因の分析と今後の調査方向は記事末尾の「まとめ」に記載しています。以下では今後の改善に向けた具体的な調査手法を示します。

**1. line_profiler や カスタムインストゥルメンテーションによる関数レベル分析**

今回の調査で、Prefix Caching の管理オーバーヘッドが原因であることは特定できましたが、具体的にどの関数がどれだけ時間を消費しているかは推定に留まっています。

```python
# vLLM ソースコードに直接タイマーを追加
import time
def compute_hash(block_content):
    start = time.perf_counter()
    result = hash(tuple(block_content))
    elapsed = (time.perf_counter() - start) * 1000
    logger.debug(f"Hash computation: {elapsed:.3f} ms")
    return result
```

line_profiler や カスタムインストゥルメンテーションにより、以下を正確に測定：
- ハッシュ計算の実行時間
- ハッシュテーブル lookup の頻度と時間
- LRU 管理の時間
- 各操作の呼び出し回数

**2. 異なるワークロードでの測定**

- **高キャッシュヒット率ワークロード**: チャットボット（長い system prompt を共有）
- **低キャッシュヒット率ワークロード**: 完全にランダムなプロンプト
- **実世界ワークロード**: 実際のアプリケーションからのトレース

これにより、Prefix Caching がどのようなワークロードで有効かを特定できます。

**3. vLLM のメトリクス詳細分析**

vLLM のログレベルを上げて、以下を測定：
- 実際のキャッシュヒット率
- ブロック割り当て/解放の頻度
- Scheduler の待機時間

**4. Neuron Profiler によるハードウェアレベル分析**

今回はソフトウェアレベルの調査でしたが、Neuron Profiler を使用してハードウェアレベルで：
- KV キャッシュのメモリアクセスパターン
- NeuronCore の利用率
- メモリ帯域幅の使用状況

を分析することで、より深い洞察が得られる可能性があります。

**5. NxD Inference での直接測定**

現在、NxD Inference では Prefix Caching が vLLM 経由でのみ自動サポートされています。将来のSDKバージョンで直接 API に引数計算機能が追加された場合、vLLM の Scheduler を経由しない純粋な NxD Inference での性能を測定できます。
...
### プロファイリング手法の知見（Phase 1-2）

**1. Neuron Profiler の限界と活用**

- Perfetto トレースはハードウェアレベルの分析に有用だが、Worker プロセスの詳細は取得困難
- `skip_warmup` 設定により、モデルコンパイル時間（数分）を省略可能
- NEFF (Neuron Executable File Format) の理解がパフォーマンスチューニングに重要

**2. line_profiler による Python レベルプロファイリング**

- Worker プロセスに直接 `@profile` デコレーターを適用することで、推論パイプラインの詳細を測定可能
- ただし、プロファイリングオーバーヘッド込みの測定となるため、絶対値の解釈には注意が必要
- 全体の 84.2% の時間が `_run_workers` 内部で消費されており、Worker プロセスの最適化が鍵

### Bucketing と Prefix Caching の性能特性（Phase 3）

**1. Bucketing の効果**

- **NxD Inference**: 可変長ワークロードで 89.5% 改善（194.73 ms → 20.42 ms）
- **vLLM**: 固定長・可変長ともに 76% 改善（252 ms → 60 ms）
- **重要**: `--benchmark` フラグは常に 2048 トークン入力を生成するため、Bucketing の効果を正しく測定できない

**2. Prefix Caching の効果**

- **Bucketing OFF の場合**: 約 6% 改善（軽微な効果）
- **Bucketing ON の場合**: **約 45% 悪化**（予想外の結果）

**3. Bucketing ON + Prefix Caching ON でのレイテンシ悪化の根本原因**

今回の最も重要な発見は、**Prefix Caching に伴うオーバーヘッドがキャッシュヒットの恩恵を打ち消す** ケースがあることです。当初の仮説（Maximum Concurrency 制約）は `num_gpu_blocks_override` を 2 倍にしても -1.0% しか改善せず棄却。真の原因として以下の 2 要因を特定しました。

1. **NxD Inference の KV cache レイアウト変更**: Prefix Caching ON で [`is_block_kv_layout`](https://github.com/vllm-project/vllm-neuron/blob/ed89ae8/vllm_neuron/worker/neuronx_distributed_model_loader.py#L782-L783) が `True` になり、連続メモリ → ブロックレイアウトに切り替わる。NeuronCore の DMA 転送パターンが変化
2. **vLLM Scheduler の管理処理オーバーヘッド**: [`hash_block_tokens()`](https://github.com/vllm-project/vllm/blob/v0.13.0/vllm/v1/core/kv_cache_utils.py#L525-L552) によるハッシュ計算、lookup、LRU 管理がリクエスト毎に全ブロックで発生

各要因の寄与度を定量的に切り分けることはできませんでした。NxD Inference では `is_block_kv_layout=True` に `is_prefix_caching=True` または `chunked_prefill=True` が必須であり（[`_validate_neuron_config()`](https://github.com/vllm-project/vllm-neuron/blob/ed89ae8/vllm_neuron/worker/neuronx_distributed_model_loader.py#L840-L846) で強制）、分離実験は 3 パターン試みたがいずれも Neuron コンパイル時にクラッシュしました。詳細は Phase 3.2 の details を参照してください。

### NxD Inference の実装上の発見

**1. `tensor_capture_hook` バグ（SDK 2.27.0）**

`HuggingFaceGenerationAdapter` が `tensor_capture_hook` を誤って `forward()` メソッドに渡すバグを発見しました。これにより、`--benchmark` フラグを使わない実プロンプト測定時にエラーが発生します。

**2. Prefix Caching のサポート状況**

NxD Inference のモデル自体は Prefix Caching をサポートしていますが、`inference_demo.py` や `HuggingFaceGenerationAdapter` は必要な引数（`full_context_lens`, `computed_context_lens`）を計算・提供する機能を実装していません。vLLM は Scheduler でこれを自動化しているため、"out of the box" で動作します。

### プロファイリングから学んだ教訓

1. **複数のプロファイリング手法を組み合わせる**: Neuron Profiler（ハードウェアレベル）+ line_profiler（Python レベル）+ ベンチマーク測定（エンドツーエンド）
2. **測定条件を慎重に設計する**: `--benchmark` フラグのような隠れた制約に注意
3. **予想外の結果を深掘りする**: Prefix Caching の悪化という予想外の結果から、`platform.py` の block_size 上書きや KV cache レイアウト変更といった重要な知見が得られた
4. **実装の詳細を理解する**: vLLM と NxD Inference の統合方式の違いが、Prefix Caching のサポート状況に影響
5. **ソースコードレベルでの検証**: ログやメトリクスの背後にある計算ロジックをソースコードから確認することで、現象の根本原因を正確に理解できる
6. **実際のデータ特性に近い条件で測定する**: `--benchmark` フラグが 2048 トークン固定入力を生成して Bucketing の効果を隠蔽していたように、合成データと実データではプロンプト長の分布やキャッシュヒット率が大きく異なり、性能特性が変わる。Bucketing はプロンプト長とバケットサイズの差が効果に直結し、Prefix Caching はキャッシュヒット率とオーバーヘッドのバランスがワークロードに依存するため、実際のデータに近い条件でプロファイリングしないと正しい最適化判断ができない

### 次のステップ

今回の調査で、vllm-neuron のプロファイリング手法と、Bucketing/Prefix Caching の性能特性について深い理解が得られました。次は：

1. 実世界のワークロード（長い system prompt を共有するチャットボットなど）で Prefix Caching の効果を検証（プロンプト長が長くキャッシュヒット率が高いケースでは恩恵がオーバーヘッドを上回る可能性がある）
2. Neuron Profiler によるブロックレイアウト時の NeuronCore メモリアクセスパターン分析
3. line_profiler 等による Prefix Caching 管理処理（ハッシュ計算、lookup、LRU 管理）の関数レベルのコスト計測

この知見が、AWS Inferentia2 上での推論最適化に取り組む皆さんの参考になれば幸いです。
