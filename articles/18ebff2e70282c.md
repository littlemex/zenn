...
[Ambiguity: 最新の FPGA Developer AMI のバージョンと AMI ID については、AWS Marketplace または AWS コンソールで最新情報を確認してください。本記事執筆時点での情報として、Rocky Linux 8.10 版と Ubuntu 24.04 版が提供されていますが、AMI ID は定期的に更新される可能性があります]
...
HDK (Hardware Development Kit) 環境をセットアップするには、`hdk_setup.sh` スクリプトを実行します。初回実行時は 2〜3 分かかり、Vivado インストールの確認、環境変数の設定、IP シミュレーションモデルの生成、シェルファイルの S3 からのダウンロードが自動で実行されます。

```bash
source hdk_setup.sh
```

成功すると `AWS HDK setup PASSED.` というメッセージが表示されます。なお、新しいターミナルセッションを開くたびに `source hdk_setup.sh` を実行する必要があります。
...
Vivado が見つからないというエラーが出た場合、`which vivado` コマンドでパスを確認します。パスが表示されない場合は、AMI が正しいか確認してください。

```bash
which vivado
# 出力例: /tools/Xilinx/Vivado/2025.1/bin/vivado
```

AMI が正しくインストールされているか確認：

```bash
ls -la /tools/Xilinx/Vivado/
```
...
- [AWS FPGA GitHub](https://github.com/aws/aws-fpga)：サンプルコードとスクリプト
...
3. **Vivado での設計**：IP Integrator を使った GUI ベースの設計
4. **シミュレーション**：実機なしでの動作確認
