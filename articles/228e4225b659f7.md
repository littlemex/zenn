...
手元でリング通信を体感したい場合は、**NVLink でつながった GPU を持つマシン**（NVIDIA HGX/DGX 系、`p3.8xlarge` / `p4d` / `p5` などの NVLink 搭載インスタンス）を使ってください。以降のコマンドは p4d（A100 ×8）で実際に動作を確認したものです。
:::
...
### 補足: pip でライブラリだけ入れる方法

コンテナを使わず、既存の CUDA 環境にライブラリだけ追加したい場合は、pip wheel も利用できます（CUDA 12 系の環境が前提）。

```bash
pip install nvidia-nvshmem-cu12
# インストール先を確認（環境により出力が異なるので Location を見る）
pip show nvidia-nvshmem-cu12 | grep Location
```

表示されたディレクトリ配下の `nvidia/nvshmem/` にある `include` / `lib` を、上のコンパイルコマンドの `-I` / `-L` に指定すれば同様にビルドできます。
...
次に試すなら、`nvshmem_int_g`（get）で「相手のメモリから読み出す」動作を確認するのが、最もシンプルな一歩です。その後、broadcast や all-reduce などの集団通信に進むと、NVSHMEM の全体像が見えてきます。
