...
本記事は、Physical Intelligence の **openpi (Pi0.5)** を、**JAX で AWS Trainium2 上で動かす**までの調査記録です。最終的に **公開重み (`pi05_base` checkpoint) で CPU 実装と数値整合する actions を Trainium2 上で出力する**ところまで到達しました。

対象モデルは `pi05_aloha_pen_uncap` (Pi0.5 + Aloha 系入力)。Gemma 2B + Action Expert backbone を持ち、続編 (Phase 5) で split-head NKI Flash Attention を投入して warm **0.82 s/step** まで詰めることになります。本記事はその下準備として、**「Pi0.5 を素直に jit すると Trainium で何が起こるか」「どう分解すれば NEFF (Neuron Executable File Format) に乗るか」** を Phase 1 から Phase 4 まで順を追って記録します。
...
- openpi の Pi0.5 forward は **巨大すぎて** Neuron Compiler の instruction 上限 (5,000,000) を素直に踏む
...
| ckpt | `gs://openpi-assets/checkpoints/pi05_base` |
| バッチサイズ | **B = 1** |
...
| Flow Matching ステップ数 | 本記事 (Phase 4-C) は **1 step**、続編の Phase 5-E §6.2 で **10 step** |
...
## Phase 4: Pi0.5 staged inference を完成させる
...
| 資源 | trn2.3xlarge の容量 | Phase 4-C の実使用 | 使用率 |
|---|---:|---:|---:|
...
- 解は **構造で解く** ─ 1 つの forward を 1 つの NEFF に押し込めず、CPU で lazy_init / Neuron で forward / Python ループで Flow Matching、と切り分けて 1 NEFF あたりの命令数を 5M 以下に圧縮する (本記事の Phase 3-4 がそれ)。
...
| Pi0.5(dummy) prefix forward Phase 3-C | trn2 | 27.0s | OK |
| Pi0.5(dummy) staged 1 step Phase 4-A | trn2 | 75.3s/step | OK, diff=0.034 |
| Pi0.5(2B/300M) staged 1 step Phase 4-B | trn2 | 82.2s/step | NaN (random init) |
| **`pi05_aloha_pen_uncap` staged 1 step Phase 4-C** | **trn2** | **92.4s/step** | **OK, diff=0.112** |
| `pi05_aloha_pen_uncap` CPU JAX (reference) | CPU | 8.5s | OK |
| `pi05_aloha_pen_uncap` 一括 jit Phase 1 | trn2 | — | NG (HLO overflow) |
| Pi0.5(dummy) 一括 jit Phase 2 | trn2 | 30 分超 stuck | NG (NCC 5M 上限) |
...
ap-southeast-4 (メルボルン) で本記事の Phase 4-C を再ランしたところ、CPU init は **policy ready 31.0 s + CPU infer 9.9 s + pickle 6.71 GB** で再現、Neuron 側 staged forward は元ランと整合する結果でした (warm step の数字は続編の Phase 5-E 章で扱います)。
...
`policy.infer` の `actions.shape=(50, 14)`、CPU 側 stats は実機ランと bit 一致。Phase 5 (NKI) も同じ pickle / Observation 経路で動きました (warm 0.82 s/step、続編で詳述)。
