---
title: "vllm-neuron を用いた LLM 推論とその測定"
emoji: "⚡"
type: "tech"
topics: ["AWS", "Inferentia2", "vLLM", "Neuron", "機械学習"]
published: false
---
...
vLLM は継続的なバッチ処理などの高度な推論機能を活用したLLM推論およびサービス提供のための人気ライブラリです。vllm-neuron は、高速な LLM 推論エンジンである vLLM を AWS Neuron SDK と統合したものです。通常の vLLM が NVIDIA GPU 上で CUDA を使用するのに対し、vllm-neuron は AWS Inferentia と Trainium チップ上で動作します。
...
sequenceDiagram
    ...
    App->>vLLM: LLM インスタンス化
    vLLM->>Compiler: モデルコンパイル要求
    Compiler->>Compiler: PyTorch → NEFF 変換
    Compiler-->>vLLM: コンパイル済みグラフ
    vLLM->>Runtime: グラフロード
    Runtime->>Chip: グラフ配置

    App->>vLLM: 推論リクエスト
    vLLM->>Runtime: バケットサイズに丸めて実行
    Runtime->>Chip: 推論実行
    Chip-->>Runtime: 結果
    Runtime-->>vLLM: 結果
    vLLM-->>App: 推論結果

    Note over Compiler,Runtime: 初回コンパイルは数分～数十分<br/>2回目以降はキャッシュ利用で高速

    rect rgb(230, 245, 255)
        Note over vLLM,Chip: CUDA グラフと同様の概念:<br/>事前コンパイル済みグラフで<br/>実行時オーバーヘッド削減
    end
```
...
本セクションでは、vllm-neuron のパフォーマンスを評価するための比較的汎用なベンチマークスクリプトの実装を解説します。このスクリプトは、JSON 設定ファイルによる柔軟なテストパターン定義をサポートし、複数の設定を自動的にテストして結果を比較できます。解説の容易さを重視するためにファイル数、メソッド数などを極力削減する形としています。計測で追加すべきパラメータがあれば AI にお願いして追加実装してもらいましょう！
...
vLLM 公式のベンチマーク CLI (`vllm bench`) は **vLLM-Neuron 環境では動作しません**。CLI として 3 つのコマンド（`serve`、`throughput`、`latency`）が存在しますが、今回利用したかった `throughput` を実際に実行すると初期化時にハングし、正常に動作しませんでした。`startup`、`mm-processor`、`sweep` は実装されてなさそうです。
...
#### 指定可能な Neuron 固有パラメータ

`override_neuron_config` 内で以下のパラメータが指定可能です（本プロジェクトの実装から推測）。

- `enable_bucketing` - bucketing の有効化（推奨: `true`）
- `pa_num_blocks` - PagedAttention のブロック数
- `pa_block_size` - PagedAttention のブロックサイズ
- `is_prefix_caching` - prefix caching の有効化
- `is_block_kv_layout` - block KV layout の有効化（prefix caching ON 時のみ）
- `skip_warmup` - warmup のスキップ
- `kv_cache_quant` - KV cache quantization
- `sequence_parallel_enabled` - sequence parallel
- `flash_decoding_enabled` - flash decoding

---

## 3. vLLM-Neuron で利用不可能な機能

以下のコマンドは vLLM-Neuron でサポートされていません。

### 3.1 `vllm bench startup`

**機能**: モデルのスタートアップ時間（コールド/ウォーム）を測定

**理由**: vLLM-Neuron には実装されていない可能性が高い

**代替手段**: 手動で時間を計測する

```bash
time vllm serve --model <model> --tensor-parallel-size 2
```

### 3.2 `vllm bench mm-processor`

**機能**: マルチモーダルプロセッサーのレイテンシー測定

**理由**: vLLM-Neuron のマルチモーダル対応状況に依存

**代替手段**: 現時点では代替手段なし

### 3.3 `vllm bench sweep`

**機能**: パラメータスイープによる最適設定の探索

**理由**: vLLM-Neuron には実装されていない

**代替手段**: 本プロジェクトの JSON 設定ベースのベンチマークシステムが代替として機能
...
本プロジェクトでは、独自の JSON 設定ベースのベンチマークシステムを実装しています。

**ファイル**:
- `run_offline_benchmark.sh` - メインスクリプト
- `offline_benchmark.py` - Python 実装
- JSON 設定ファイル（`test_config_small.json` など）
...
### 4.2 vLLM bench との違い（理論上）

⚠️ **注意**: vLLM bench は vLLM-Neuron 環境では動作しないため、以下は理論上の比較です。

| 項目 | 本プロジェクトの実装 | vLLM bench（動作せず） |
|------|---------------------|---------------------|
| **動作状況** | ✅ 正常に動作 | ❌ 初期化時にハング |
| **設定方法** | JSON ファイル | CLI パラメータ |
| **バッチ実行** | 複数設定を自動実行 | 手動で複数回実行 |
| **結果集約** | 自動的にサマリー生成 | 手動で集約 |
| **Neuron 設定** | JSON 内で明示的に定義 | `--additional-config` で指定（未検証） |
| **データセット** | Reranking タスク専用 | 汎用データセット対応 |
| **実行方法** | LLM クラス直接 | LLM クラスまたはサーバー |

### 4.3 本プロジェクトの実装の利点

- ✅ **vLLM-Neuron 環境で実際に動作する**
- ✅ 複数設定の一括実行が容易
- ✅ 結果の自動集約とランキング
- ✅ Reranking タスクに最適化
- ✅ JSON で設定を versioning 可能
- ✅ 実験の再現性が高い
- ✅ 実証済みの最適化結果（prefix caching OFF + bucketing ON で 33.4% 高速化）

### 4.4 vLLM bench の理論上の利点（実際には使用不可）

仮に vLLM bench が動作した場合の利点:
- 公式サポートで将来的な互換性が保証
- 豊富なデータセット対応
- 詳細なレイテンシーメトリクス（TTFT、TPOT など）
- オンラインサービングのベンチマークが可能
- 標準的なベンチマーク手法

**ただし**: 現時点では vLLM-Neuron で動作しないため、これらの利点は享受できません。

---

## 5. 推奨される使い方

### 5.1 vLLM-Neuron 環境での現実的な選択

**結論: 本プロジェクトの実装のみが実用可能**

vLLM bench は vLLM-Neuron 環境では動作しないため、本プロジェクトの JSON 設定ベースのベンチマークシステムを使用してください。
...
- ✅ 結果の自動集約が必要
- ✅ オフライン推論のみ（サーバー不要）
- ✅ vLLM-Neuron 固有の設定（bucketing、prefix caching など）のテスト
...
### 5.2 vLLM bench が使えない理由（再確認）

❌ **vLLM bench は以下の理由で使用不可**:
1. LLM 初期化時にプロセスがハングする
2. 循環 import エラーが発生
3. 実行テストで JSON 出力ファイルが生成されない
4. vLLM-Neuron との互換性に問題がある

### 5.3 将来的な可能性

vLLM bench が vLLM-Neuron で修正された場合、以下の用途で併用を検討できます:

**Phase 1**: 本プロジェクトの実装で Reranking タスクの最適化
```bash
CONFIG_FILE=benchmark_config_example.json bash run_offline_benchmark.sh
```

**Phase 2**: vLLM bench で汎用スループット測定（将来的に動作した場合）
```bash
# ※現時点では動作しません
vllm bench throughput \
  --model ./models/Qwen3-0.6B-Reranker \
  --tensor-parallel-size 2 \
  --additional-config '{"override_neuron_config": {"enable_bucketing": true}}'
```
...
❌ **vLLM bench は vLLM-Neuron 環境では使用不可**

- LLM 初期化時にプロセスがハングする
- 実行テストで動作しないことが確認済み
- `--additional-config` パラメータの有効性も未検証

詳細はセクション 7 の実行テスト結果を参照してください。

### 6.2 本プロジェクトの実装の制限事項

#### メトリクスの制約

- TTFT、TPOT、ITL などの詳細メトリクスは取得できません
- 測定できるのはクエリ全体の処理時間のみです

#### データセット

- Reranking タスク専用のデータフォーマットです
- 汎用的なデータセット（ShareGPT など）は使用できません

#### オンライン評価

- サーバーベースのベンチマークはサポートしていません
- Request rate の調整による最適化は実施できません
...
- **Date**: 2026-01-24
- **Environment**: `/opt/aws_neuronx_venv_pytorch_inference_vllm`
- **vLLM-Neuron Version**: 0.2.2+lts
- **Model**: `./models/Qwen3-0.6B-Reranker`
- **Tensor Parallel Size**: 2

### 7.2 `vllm bench throughput` テスト結果

#### テスト 1: 基本実行（minimal parameters）
...
#### 7.3.2 `--additional-config` パラメータの検証

`EngineArgs.add_cli_args()` を調査した結果、`--additional-config` パラメータが CLI オプションとして認識されているかどうか不明です。Documentation には記載されていますが、実装が最新版と同期していない可能性があります。

#### 7.3.3 vLLM bench の互換性問題

vLLM bench は主に CUDA GPU 環境を想定して設計されており、vLLM-Neuron への移植が不完全である可能性があります。特に:

- LLM クラスの初期化プロセスが Neuron backend で正常に動作していない
- Neuron 固有の warmup や compilation が適切に処理されていない
- Neuron 環境特有の initialization sequence に対応していない

### 7.4 `offline_benchmark.py` との機能比較

#### 7.4.1 本プロジェクトの `offline_benchmark.py` の特性

`offline_benchmark.py` を詳細に分析した結果、以下の特徴が判明しました:

**専用の Reranking ロジック**:
```python
# Lines 101-104: カスタムプロンプトテンプレート
self.prefix = "<|im_start|>system\nJudge whether the Document meets the requirements..."
self.suffix = "<|im_end|>\n<|im_start|>assitant\n<think>\n\n</think>\n\n"

# Lines 133-154: yes/no トークン確率からスコア計算
sp = SamplingParams(
    max_tokens=1,
    temperature=0.0,
    logprobs=20,
...
**データ処理パイプライン**:
- `Speed_measurement.csv` から query-document ペアを読み込み
- カスタムフォーマットでプロンプトを構築
- vLLM で推論実行
- Yes/No トークンの logprobs から relevance score を計算
- ランク付けされた結果を返す

#### 7.4.2 vLLM bench との違い

| 項目 | offline_benchmark.py | vLLM bench throughput |
|------|---------------------|---------------------|
| **目的** | Reranking タスクの実行 | 汎用スループット測定 |
| **入力データ** | Speed_measurement.csv (query-doc pairs) | ShareGPT/Random/HF datasets |
| **プロンプト** | Reranking 専用テンプレート | 汎用テキスト生成 |
| **出力** | Relevance scores (logprobs-based) | Generated text |
| **メトリクス** | Task latency + reranking accuracy | Throughput (requests/s, tokens/s) |
| **実装** | Task-specific logic | Generic benchmark |

#### 7.4.3 結論: 完全な置き換えは不可能

**vLLM bench では `offline_benchmark.py` を置き換えられません**。理由:

1. **タスクの性質が異なる**
   - `offline_benchmark.py`: Reranking という特定のタスクを実行
   - vLLM bench: 汎用的なテキスト生成のスループット測定

2. **出力形式が異なる**
   - `offline_benchmark.py`: Yes/No トークンの確率スコア
   - vLLM bench: 生成されたテキスト

3. **評価指標が異なる**
   - `offline_benchmark.py`: タスク完了時間 + 関連性スコア精度
   - vLLM bench: スループット（requests/s, tokens/s）

### 7.5 推奨される使い分け

#### `offline_benchmark.py` を使うべき場合

✅ **Reranking タスクの実行とパフォーマンス測定**
- 実際のタスクロジックを実行
- Relevance scoring が必要
- Speed_measurement.csv データを使用

```bash
CONFIG_FILE=test_cache_bucketing_comparison.json bash run_offline_benchmark.sh
```

#### vLLM bench を使うべき場合（理論上）

✅ **汎用的なスループットベンチマーク**
- モデルの純粋な生成能力を測定
- 標準的なデータセット（ShareGPT など）で比較
- オンラインサービングの性能評価

**ただし**: 現時点では vLLM-Neuron で `vllm bench throughput` が正常に動作しないため、実用性は低い。

#### ハイブリッドアプローチ（将来的）

vLLM bench が修正された場合:

1. **Phase 1**: vLLM bench で汎用スループットを測定
   - モデルの基本性能を把握
   - 様々なバッチサイズ・シーケンス長をテスト

2. **Phase 2**: offline_benchmark.py で Reranking タスク性能を測定
   - 実際のユースケースでの性能評価
   - タスク固有の最適化を検証
...
vLLM 公式のベンチマーク CLI (`vllm bench`) は **理論上は vLLM-Neuron でサポートされていますが、実際には動作しません**。

#### テスト結果のサマリー

| コマンド | 理論上の対応 | 実際の動作 | 推奨度 |
|---------|------------|-----------|-------|
| `vllm bench throughput` | ✅ 対応 | ❌ 初期化時にハング | 使用不可 |
| `vllm bench serve` | ✅ 対応 | ⚠️ 未テスト | 不明 |
| `vllm bench latency` | ✅ 対応 | ⚠️ 未テスト | 不明 |

#### 主な問題点

1. **初期化エラー**: LLM クラスの初期化時に循環 import エラーが発生
2. **プロセスハング**: 初期化後にプロセスが無応答状態になる
3. **CLI 互換性**: `vllm bench` CLI が正しく動作せず、Python module 呼び出しが必要
4. **`--additional-config` の検証不可**: ハングにより、Neuron 固有設定の有効性を検証できず

### 8.2 `offline_benchmark.py` の代替可能性

**結論: 代替不可能**

`offline_benchmark.py` は vLLM bench で置き換えることはできません。理由:

1. **目的が異なる**
   - `offline_benchmark.py`: Reranking タスクの実行と評価
   - vLLM bench: 汎用スループット測定

2. **実装が異なる**
   - `offline_benchmark.py`: タスク固有のロジック（プロンプトテンプレート、logprobs スコアリング）
   - vLLM bench: 汎用ベンチマークフレームワーク

3. **vLLM bench が動作しない**
   - 現時点で vLLM-Neuron 環境では実行不可能
...
**理由**:
- ✅ Reranking タスクに特化した実装
- ✅ 複数設定の一括実行と結果集約
- ✅ 実証済みの動作実績（prefix caching 33.4% 高速化の発見など）
- ✅ JSON ベースの設定管理で再現性が高い
...
**理由**:
- ❌ vLLM-Neuron 環境で動作しない
- ❌ Reranking タスクには不適切（汎用スループット測定のみ）
- ❌ タスク固有ロジックの再実装が必要
- ❌ メンテナンスコストが増加

#### 将来的な検討事項

vLLM bench が vLLM-Neuron で修正された場合、以下の用途で併用を検討:

1. **モデルの基本性能評価**: vLLM bench で汎用スループットを測定
2. **タスク固有評価**: `offline_benchmark.py` で Reranking 性能を測定
3. **オンラインサービング**: `vllm bench serve` でレイテンシーメトリクスを取得（TTFT/TPOT）

---

**作成日**: 2026-01-24
**最終更新日**: 2026-01-24 (テスト結果追加)
**調査者**: Claude Code
**ステータス**: ✅ 調査完了 - 本プロジェクトの実装継続を推奨


## まとめ

本記事では、vllm-neuron を使用したベンチマークについて実装例の解説と実際に結果を確認しました。
