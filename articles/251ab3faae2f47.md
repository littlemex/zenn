...
公式リポジトリ: https://github.com/verl-project/verl

Verl（ByteDance Seed チーム開発）は、強化学習ベースのポストトレーニングアルゴリズムを大規模クラスタで実行するための分散訓練フレームワークです。

### Hybrid-Controller アーキテクチャ（HybridFlow）

Verl は **Hybrid-Controller アーキテクチャ（HybridFlow とも呼ばれます）** を採用しています。この設計は、Google Pathways などの非同期シャーディングデータフローシステムと設計原理を共有しており、PPO、GRPO、DAPO などの強化学習アルゴリズムを、**マルチステージ、マルチモデル、並列化可能なデータフローグラフ**としてモデル化します。

柔軟性とパフォーマンスのバランスをとるために、Verl は **2 つの異なるプログラミングモデル**を統合しています。

#### 高レベル Single-Controller（MPMD: Multiple Program Multiple Data）

オーケストレーションレベルでは、**単一プロセスの RLTrainer がグローバル計算グラフを管理**します。以下のようなマクロタスクを処理します：

- Rollout（推論）のスケジューリング
- Reward スコアリングのトリガー
- 分散訓練ジョブのディスパッチ
- Experience Buffer の管理
- パイプライン全体の制御フロー

このレイヤーでは、異なるコンポーネント（Actor、Rollout、Critic、Reward）が異なるプログラムを実行し、Controller が Ray のような分散フレームワークを介して協調動作を制御します。

#### 内部 Multi-Controller（SPMD: Single Program Multiple Data）

各コンポーネント（Model Engine / Rollout Engine）は、**内部的に標準的な分散訓練/推論モードで動作**します。全 Worker が同一のプログラムを実行し、Collective Communication を介して同期しながら、大量の分散計算を実行します：

- **Model Engine**: FSDP、Megatron、VeOmni などのトレーナーバックエンドを使用
  - 全 Worker が同一の訓練コードを実行
  - AllGather、ReduceScatter などの Collective で勾配・パラメータを同期

- **Rollout Engine**: vLLM/SGLang/TensorRT-LLM などのロールアウトエグゼキューター（Rollout Server ではない）を使用
  - 全 Worker が同一の推論コードを実行
  - Tensor Parallel では AllReduce で中間結果を同期

この 2 層構造により、Verl は**上位のアルゴリズムロジックの柔軟性**と**下位の分散計算の高効率性**を両立しています。

```mermaid
graph TB
    subgraph "MPMD Layer: Single-Controller (RLTrainer)"
        Controller[Single-Process Controller<br/>グローバル計算グラフ管理]
    end

    subgraph "SPMD Layer 1: Model Engine"
        subgraph "Actor Training (FSDP)"
            A1[Worker 0<br/>同一コード]
            A2[Worker 1<br/>同一コード]
            A3[Worker N<br/>同一コード]
            A1 <-->|AllGather/ReduceScatter| A2
            A2 <-->|AllGather/ReduceScatter| A3
        end
    end

    subgraph "SPMD Layer 2: Rollout Engine"
        subgraph "Rollout Generation (vLLM)"
            R1[Worker 0<br/>同一コード]
            R2[Worker 1<br/>同一コード]
            R3[Worker N<br/>同一コード]
            R1 <-->|AllReduce| R2
            R2 <-->|AllReduce| R3
        end
    end

    subgraph "SPMD Layer 3: Reward Engine"
        subgraph "Reward Model (FSDP)"
            W1[Worker 0<br/>同一コード]
            W2[Worker 1<br/>同一コード]
            W3[Worker N<br/>同一コード]
            W1 <-->|AllGather/ReduceScatter| W2
            W2 <-->|AllGather/ReduceScatter| W3
        end
    end

    Controller -->|dispatch| A1
    Controller -->|dispatch| R1
    Controller -->|dispatch| W1
    R1 -->|sequences| W1
    W1 -->|reward scores| Controller
    Controller -->|training data| A1
...
class ModelEngine(ABC):
    def initialize(self):
        """モデル、オプティマイザ、スケジューラをインスタンス化"""

    def forward_backward_batch(self, data):
        """Forward と Backward Pass を実行"""
...
- **FSDP**: PyTorch ネイティブの FSDP+Sequence Parallel。全 Transformer モデルをサポートし、実装が最も簡単
- **Megatron-Core**: NVIDIA の高性能訓練フレームワーク。5D 並列化（DP+TP+PP+EP+CP）で最高性能だが、統合に時間がかかる
- **VeOmni**: Verl チーム開発の中性能バックエンド。VLM と MoE に最適化され、約 1 週間で統合可能

この抽象化により、**ユーザーは RL アルゴリズムの実装に集中でき、分散訓練の詳細に煩わされることがありません**。

#### Rollout Engine の詳細

v0.7 では、レガシーの SPMD rollout モードが削除され、**server mode がデフォルト**になりました。server mode は、オフライン・バッチ推論ではなく、**オンライン・サービングとして動作**します。

**主要な改善点**:
- クライアントがサンプル単位でリクエストを送信し、動的バッチングを実現
- LLM エンジンへの侵入的な変更が不要
- vLLM、SGLang、TensorRT-LLM バックエンドをサポート

**AgentLoop 抽象化**:

カスタム・エージェント・タスク用の拡張可能なインターフェース。

```python
class AgentLoopBase(ABC):
    @abstractmethod
    async def run(self, sampling_params: dict[str, Any],
                  **kwargs) -> AgentLoopOutput:
        """LLM サーバーと環境の対話を実行するエージェントループ"""
        pass
```
...
この抽象化により、SWEAgent（コード編集）、GUIAgent（UI 操作）などの複雑なエージェント行動を訓練できます。

#### TransferQueue の詳細

TransferQueue は、**制御フローとデータフローを分離**し、大量データ転送時のボトルネックを解決します（マルチモーダル訓練、ルーター・リプレイなど）。

**主要機能**:
- RLTrainer は指示とメタデータのみをディスパッチ
- PyTorch テンソル向けにゼロコピーと RDMA をサポート
- バックエンド拡張: ZeroMQ、NIXL、Ray RDT
- v0.8 でデフォルトの転送メソッドになる予定
...
このパターンにより、Controller は軽量なメタデータのみを扱い、実際の大容量テンソルは専用経路で転送されます。
...
**トランスポート・バックエンド**:
- **NCCL**: Collective broadcast（同報通信）
- **NIXL**: P2P 通信（ポイント・ツー・ポイント）

「効率的なクロスノード・パラメータ同期」を Off-policy 非同期訓練シナリオで実現します。
