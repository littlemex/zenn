...
:::message
この記事は、Verl v0.7 のリリースノートブログを ML インフラストラクチャ視点で整理・解説したものです。重要なアップデートに焦点を当て、自分のキャッチアップのためにまとめています。強化学習の理論面は専門外ですが、インフラ設計の観点から整理しています。
:::

Verl は、GRPO、DPO、CISPO、SAPO などの強化学習ベースの LLM ポストトレーニング手法を大規模クラスタで実行するための分散訓練フレームワークです。本記事では v0.7 の重要なアップデートに絞って解説します。
...
## 前提知識: GRPO と Generation/Training フェーズ

GRPO（Group Relative Policy Optimization）や DPO（Direct Preference Optimization）をはじめとする強化学習ベースのポストトレーニング手法には、共通する特徴があります。これらの手法では、**推論（Generation Phase）が訓練時間の約 80% を占める**ことです（[OpenRLHF の公式データ](https://github.com/OpenRLHF/OpenRLHF)、[DeepSpeed-Chat 分析](https://github.com/deepspeedai/DeepSpeed/blob/master/blogs/deepspeed-chat/README.md)）。GRPO は PPO から Critic モデルを削減してグループ内の相対評価で学習し、DPO は報酬モデルを介さず人間の選好ペアから直接学習します。v0.7 で追加された CISPO と SAPO は、これらの課題をさらに改善する発展的手法です。

:::message alert
**重要: 計算量 vs 実時間、そしてコスト構造**

[DeepSpeed-Chat の分析](https://github.com/deepspeedai/DeepSpeed/blob/master/blogs/deepspeed-chat/README.md)によると、**計算量（FLOPs）**では Generation が 20%、Training が 80% ですが、**実時間（wall-clock time）**では逆転します。Generation は各トークン生成ごとにモデル重み（例: 140 GB）をメモリから読み込む必要があり、**メモリバウンド**で GPU の計算性能を活かせません。一方、Training は Forward + Backward Pass で大量の演算を実行し、**計算バウンド**で GPU をフル活用できます。この違いにより、計算量は少ないが実時間では Generation が約 80% を占めます。

**コスト構造への影響**:

同じ GPU を使用する場合、実行時間がそのままコストに比例するため、**Generation Phase（推論）がコストの約 80% を占めます**。例えば 8 GPU で $100/h/GPU の場合：
- 総コスト: $800/h
- Generation Phase: $640/h（80%）
- Training Phase: $160/h（20%）

これが、v0.7 で推論エンジンの最適化（Rollout Engine の server mode 移行、vLLM/SGLang 統合）が重視される理由です。**推論の最適化こそが、RLHF/GRPO におけるコスト削減の本質**であり、Verl の設計思想もこの特性を深く理解した上で構築されています。
:::

### GRPO の Generation Phase と Training Phase

GRPO の訓練プロセスは、**Generation Phase**（推論）と **Training Phase**（学習）の 2 つのフェーズから構成されます。[VERL の公式ドキュメント](https://verl.readthedocs.io/en/latest/algo/grpo.html)によると、GRPO は各プロンプトに対して複数の応答（例: 8 個）を生成し、それらの報酬を相対的に評価することで、Critic ネットワークなしでポリシーを更新します。
...
#### Generation Phase の詳細

**Generation Phase** では、各プロンプトに対して複数の応答を生成します。VERL では、**ActorRolloutRefWorker** が Actor、Rollout（推論エンジン）、Reference Model を統合しており、以下の処理を実行します：

1. **推論エンジンによる生成**: vLLM または SGLang を使用して、各プロンプトに対して `rollout.n` 個（例: 8 個）の応答を生成します。v0.7 では server mode に移行し、Continuous Batching により GPU 利用率が向上しました

2. **Reference Model の処理**: 訓練開始時の初期ポリシー（Reference Model）を使用して、生成された応答のログ確率を計算します。これは KL divergence 計算に使用され、ポリシーの急激な変化を防ぎます

3. **Reward Model の評価**: RewardManager が各応答にスコアを付与します。Function-based（ルールベース）または Model-based（学習済みモデル）の報酬計算がサポートされています

**重要なポイント**: Generation Phase は自己回帰生成（各トークンが前のトークンに依存）のため逐次処理が必要です。前述の通り、この Phase がメモリバウンドであるため訓練時間の大半を占めます。v0.7 の server mode 移行により、Continuous Batching によるスループット向上が実現しました。

#### Training Phase の詳細

**Training Phase** では、生成された応答と報酬を使用して Actor モデルのポリシーを更新します：

1. **Advantage 計算**: GRPO は Critic ネットワークを使わず、グループ内の平均報酬をベースラインとして使用します：
   ```python
   group_baseline = mean(rewards_in_group)  # 8 個の応答の平均
   advantages = rewards - group_baseline
   ```

2. **Forward Pass**: Actor Model が生成済みの応答に対して新しいログ確率を再計算します。この処理は Teacher Forcing により**全トークンを並列処理**できるため、Generation Phase より高速です

3. **Loss 計算と更新**: PPO Clipping Loss と KL Loss を使用してポリシーを更新します。VERL では、KL divergence を報酬ではなく損失関数に直接追加する方式を採用しています

**重要なポイント**: Training Phase は Forward + Backward Pass を含みますが、生成済みの完全なシーケンスに対して並列処理が可能なため、Generation Phase よりはるかに高速です（全体の約 20%）。

---

このように、Generation Phase（メモリバウンド、逐次処理、長時間）と Training Phase（計算バウンド、並列処理、短時間）は本質的に異なる特性を持ちます。これら 2 つのフェーズを効率的に管理するインフラが、GRPO のような手法では極めて重要になります。Verl は、この課題に対応するために設計された分散訓練フレームワークです。

#### VERL における役割分離の設計

VERL は、Generation Phase と Training Phase の特性の違いを考慮した設計になっています：

**Generation Phase（推論に特化）**:
- **推論エンジン**: vLLM または SGLang を使用
- **最適化**: Continuous Batching、KV-Cache 管理、Tensor Parallel（TP）
- **特性**: メモリバウンド、逐次処理、長時間実行（約 80%）
- **設定例**: `actor_rollout_ref.rollout.name=vllm`、`tensor_model_parallel_size=8`

**Training Phase（学習に特化）**:
- **訓練フレームワーク**: FSDP（Fully Sharded Data Parallel）
- **最適化**: Gradient Checkpointing、Mixed Precision、Mini-batch 処理
- **特性**: 計算バウンド、並列処理可能、短時間実行（約 20%）
- **設定例**: `actor_rollout_ref.actor.fsdp_config`、`ppo_mini_batch_size=32`

この役割分離により、各フェーズに最適化された処理が可能になります。v0.7 の server mode 移行は、この設計思想をさらに推し進めたものです。

## Verl のアーキテクチャ概要

Verl（ByteDance Seed チーム開発）は、強化学習ベースのポストトレーニングアルゴリズムを大規模クラスタで実行するための分散訓練フレームワークです。
