...
AWS VP の James Hamilton 氏によると、Nitro Hypervisor は「最小化された Linux カーネルと KVM サブシステムで構成されており、I/O 処理を一切担わない」との説明があります（参考: [AWS re:Invent 2018 Keynote](https://www.youtube.com/watch?v=ZOIkOnW640A)）。この設計により、Nitro Hypervisor の役割は CPU とメモリの仮想化に限定されています。
...
特筆すべきは **P6-B300 インスタンス**で、**6.4 Tbps** の EFA 帯域幅を実現しており、第 1 世代の P4d.24xlarge（400 Gbps）と比較して **16 倍**の帯域幅です。P5/P5e/P5en インスタンスは 3.2 Tbps（8 倍）、G7e は 1.6 Tbps（4 倍）を実現しています。バケモノマシンで草。にゃ〜〜ん。
...
AWS 公式論文によると、RDMA over Converged Ethernet（RoCE）も検討されましたが、AWS のマルチテナント大規模データセンター環境には適していませんでした。RoCE は InfiniBand のトランスポートプロトコルを Ethernet 上で利用する技術（InfiniBand over Ethernet とも呼ばれる）であり、専用ファブリック環境では有効に機能しますが、AWS のスケール要件では以下の課題がありました。

**AWS 公式論文で明示された課題**

1. **Priority Flow Control (PFC) の必要性**
   - RoCEv2 は PFC を必要としますが、これは大規模ネットワークでは実行不可能（not feasible on large-scale networks）と論文で明記されています
   - PFC は head-of-line blocking、輻輳の伝播（congestion spreading）、デッドロックを引き起こします
   - 論文では、PFC 問題の解決策（Guo et al., 2016）が「AWS データセンターよりも大幅に小さいデータセンター」に依存していると指摘されています

2. **ECMP 衝突問題**
   - PFC を使用しても、RoCE は TCP と同様に ECMP ハッシュ衝突による輻輳の影響を受けます
   - 静的な Flow ハッシュによるパスマッピングは、現在のネットワーク使用率やフロー速度を考慮しないため、ハッシュ衝突が「ホットスポット」を作り出します

3. **輻輳制御の最適性**
   - RoCE の輻輳制御は、AWS データセンター規模のマルチテナント環境には最適化されていません（suboptimal congestion control）
   - 論文では Mittal et al. (2018) の研究を引用し、RDMA の輻輳制御の課題を指摘しています
...
:::message
**SRD 設計の核心的 trade-off**（AWS 公式論文より）

従来の TCP や InfiniBand RC（Reliable Connection）は厳密なパケット順序保証を提供しますが、論文では以下の理由で out-of-order delivery を選択したと説明されています：

**In-order delivery の問題点**:
- **Head-of-line blocking**: 1 つのパケットロスが後続の全パケットをブロック
- **リオーダリングバッファの制約**: NIC のメモリ帯域幅、バッファ容量、並行コンテキスト数に限界
- **平均レイテンシの増加**: 大規模ネットワークでは out-of-order 到着が頻発し、多数の無関係なパケットが遅延

論文の引用：
> "We decided to deliver packets to the host software even when they might be out-of-order... Most or all of these packets are likely to be unrelated to the lost packet, so such delay is unnecessary."

**SRD のアプローチ**:
- **パケットレベルで out-of-order delivery を許容**し、複数経路を同時に使用
- **メッセージ境界を保持**（TCP のようなバイトストリームではない）
- **メッセージング層が順序を管理**（例: MPI タグマッチング）
- **ネットワークの並列性を最大限に活用**しながら、アプリケーション要求に応じた順序保証を実現
:::
...
| EFA 対応インスタンスでのみ利用可能 | EFA は Nitro Card for VPC 上のハードウェア機能であるため、EFA に対応したインスタンスタイプでのみ利用できます。対応するインスタンスは P6、P5、Trn2、G7e、hpc8a などです（詳細は後述の「Nitro System と EFA の進化」の表を参照）。 |
...
**実験設定**（AWS 公式論文より）:
- 4 台のサーバー（各 12 プロセス）から単一の宛先サーバーへ 48 個の独立したフローを送信
- 最終ネットワークホップ（last-hop switch）でボトルネックを作成
- 合計帯域幅: 100 Gbps、各フローの理想的な公平シェア: 約 2 Gbps

**測定結果**（IEEE Micro 2020 論文 Figure 2, 3 より）:

| メトリック | SRD | TCP | 改善率 |
|-----------|-----|-----|-------|
| **2MB 転送の最大 FCT** | 理想値に近い（~50ms） | **150-1000ms** | **3〜20 倍改善** |
...
AWS 公式論文（IEEE Micro 2020, Figure 2, 3）によると、48-way incast という厳しい条件下でも、SRD は以下を実現しています：
- **最大 FCT が TCP の 1/3 〜 1/20**（転送サイズにより異なる）
- **FCT のばらつきが極めて小さい**（予測可能な性能）
- **タイムアウトによる遅延がゼロ**（TCP は最小 50ms の再送タイムアウトが頻発）
...
|----------|-----------|---------|---------|
| **SRD** | 理想値 + 15% | - | **理想値 + 15%** |
| **TCP** | 理想値 + 50% | 理想値 + 50% | **理想値の 10〜100 倍** |

:::message
**ECMP ハッシュ衝突の影響**

AWS 公式論文（IEEE Micro 2020, Figure 6）によると、理想的な負荷分散が期待される条件下でも、TCP は以下の問題を示しています：
- **中央値 FCT が理想値より 50% 高い**（ECMP の不均衡による輻輳）
- **最大 FCT が 1〜2 桁高い**（パケットドロップとタイムアウト）

一方、SRD は：
- **中央値 FCT が理想値 + 15%**（ほぼ最適）
- **最大 FCT も理想値 + 15% 程度**（テールレイテンシが低い）

論文では「Even though with ideal load balancing there would be no congestion at all, TCP clearly experienced congestion and even packet drops, because of nonuniform ECMP balancing」と明記されています。
:::

**原因分析**（論文より）:
- ECMP の静的フローハッシュは、インタースイッチリンクに均等に分散しない
- ハッシュ衝突により一部のリンクに複数のフローが集中
- TCP は輻輳したパスを回避できず、パケットドロップが発生
- SRD はパケット単位でマルチパス活用し、輻輳したパスを動的に回避

参考: [A Cloud-Optimized Transport Protocol for Elastic and Scalable HPC (IEEE Micro, 2020)](https://ieeexplore.ieee.org/document/9167399) -- 性能評価の詳細データは論文の Figure 10, 11, 12 を参照
...
- [A Cloud-Optimized Transport Protocol for Elastic and Scalable HPC (IEEE Micro, 2020)](https://ieeexplore.ieee.org/document/9167399) -- Leah Shalev, Hani Ayoub, Nafea Bshara, Erez Sabbag 著。SRD プロトコルの設計原則、マルチパスロードバランシング、out-of-order delivery の詳細、性能評価データ（48-way incast での TCP 比 3〜20 倍の改善）。DOI: 10.1109/MM.2020.3016891
