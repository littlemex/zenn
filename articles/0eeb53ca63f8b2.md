...
Nitro Card for VPC は EC2 インスタンスのネットワーキング機能の中枢を担います。PCIe 経由で ENA（Elastic Network Adapter）および EFA コントローラを OS に公開し、VPC データプレーンの処理（カプセル化、セキュリティグループの適用、帯域制限、ルーティング、フローログ、DHCP、DNS）をすべてハードウェア上で実行します。透過的なエンドツーエンドの 256 ビット AEAD 暗号化をネットワークパフォーマンスへの影響なしに処理できます（参考: [Data protection in Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/data-protection.html)）。
...
Nitro System では I/O 処理をハードウェアにオフロードすることで高いパフォーマンスを実現しています。[Brendan Gregg 氏のベンチマーク](https://www.brendangregg.com/blog/2017-11-29/aws-ec2-virtualization-2017.html)では、Nitro System 全体の I/O オーバーヘッドが 1% 未満と報告されています。
...
AWS 公式ドキュメントによると、Nitro Hypervisor は「軽量のハイパーバイザーであり、メモリと CPU の割り当てを管理し、ベアメタルに類似したパフォーマンスを実現する」と定義されています（参考: [AWS Nitro System](https://aws.amazon.com/ec2/nitro/)）。

AWS VP の James Hamilton 氏によると、Nitro Hypervisor は「最小化された Linux カーネルと KVM サブシステムで構成されており、I/O 処理を一切担わない」との説明があります（参考: [AWS re:Invent 2018 Keynote](https://www.youtube.com/watch?v=ZOIkOnW640A)）。この設計により、Nitro Hypervisor の役割は CPU とメモリの仮想化に限定され、Intel VT-x / AMD-V によるハードウェア支援の CPU 仮想化と、EPT（Extended Page Tables）/ NPT（Nested Page Tables）によるメモリ仮想化のみを実行します。

すべてのネットワーク、ストレージ、セキュリティの I/O 処理は Nitro Cards が担当するため、Hypervisor のアタックサーフェスが大幅に縮小されています。[Brendan Gregg 氏のベンチマーク](https://www.brendangregg.com/blog/2017-11-29/aws-ec2-virtualization-2017.html)では、Nitro System 全体の I/O オーバーヘッドが 1% 未満であると報告されています。AWS 公式ドキュメントでは「ベアメタルと区別がつかないパフォーマンス」と表現されています。

従来の仮想化基盤（Xen + QEMU）との比較を以下に示します：

| 項目 | 従来の仮想化基盤（Xen + QEMU） | Nitro Hypervisor |
|---|---|---|
| I/O パス | Dom0 / QEMU 経由（ソフトウェアエミュレーション） | Nitro Card への SR-IOV 直接アクセス |
| デバイスエミュレーション | QEMU で実装 | 最小限（Nitro Card がハードウェアで提供） |
| CPU/メモリ仮想化 | Xen Hypervisor | 最小化された Linux + KVM サブシステム |
| パフォーマンス | エミュレーション層のオーバーヘッドあり | ベアメタル相当（I/O オーバーヘッド 1% 未満*） |

\* Brendan Gregg 氏の測定による
