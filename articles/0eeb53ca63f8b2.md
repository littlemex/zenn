...
すべてのネットワーク、ストレージ、セキュリティの I/O 処理は Nitro Cards が担当するため、Hypervisor のアタックサーフェスが大幅に縮小されています。[Brendan Gregg 氏のブログ](https://www.brendangregg.com/blog/2017-11-29/aws-ec2-virtualization-2017.html)では、Nitro のオーバーヘッドは極めて小さく、多くの場合 1% 未満であると報告されています（ただし正確な測定は困難とも述べています）。AWS 公式ドキュメントでは「ベアメタルと区別がつかないパフォーマンス」と表現されています。
...
特筆すべきは **P6-B300 インスタンス**で、**6.4 Tbps** の EFA 帯域幅を実現しており、第 1 世代の P4d.24xlarge（400 Gbps）と比較して **16 倍**の帯域幅です。P5/P5e/P5en インスタンスは 3.2 Tbps（8 倍）、G7e は 1.6 Tbps（4 倍）を実現しています。
...
|-----------|--------------|----------|---------|
| **[libfabric](https://github.com/ofiwg/libfabric/releases/tag/v2.4.0)** | v2.4.0 | 2024-12-15 | EFA ダイレクトデータパスがデフォルト有効（v2.3.0〜）、RTW パケット送信最適化 |
...
|--------------|--------------|----------|---------|
| **[NCCL](https://github.com/NVIDIA/nccl/releases/tag/v2.29.3-1)** | v2.29.3-1 | 2025-02-03 | v2.29.2-1 で導入された GPU-Initiated Networking (GIN)、NCCL4Py を含む |
...
| 項目 | 内容 |
|---|---|
| UCX v1.18.0 でテスト | EFA SRD の RMA WRITE サポート（v1.20.0 で追加予定）、GPU-to-GPU 直接通信 API |
