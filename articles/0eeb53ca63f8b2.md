...
| 順序保証 | フロー内で保証（単一経路） | Out-of-order delivery を許容し、並べ直しは受信側の libfabric EFA Provider が担う |
...
**SRD の設計原則**: SRD は意図的に out-of-order delivery を許容し、届いたパケットを順序を揃えずに上位の層へ渡します。並べ直しを担うのは Nitro Card ではなく、受信側ホストのユーザー空間で動く libfabric の EFA Provider です。EFA Provider は送信元ごとにメッセージの番号を管理し、期待より先に届いたメッセージをリオーダーバッファに一時的に保持してから、番号順にアプリケーションへ渡します。このバッファの大きさは環境変数 `FI_EFA_RECVWIN_SIZE` で指定します（参考: [fi_efa(7)](https://ofiwg.github.io/libfabric/main/man/fi_efa.7.html)）。MPI や NCCL は libfabric を通して EFA を使うため、順序の扱いは主に EFA Provider の層で吸収されます。

この設計により、第一に、パケットごとに最適な経路を選択することでネットワークの並列性を最大限に活用し、head-of-line blocking を回避できます。第二に、NIC の上で順序待ちのためにパケットを保留しないため、ハードウェアのバッファ容量に縛られず、ロストパケットと無関係なパケットまで待たされることがありません。第三に、TCP/IP のようにアウトオブオーダーを「パケットロス」と誤検知した場合の不要な再送や輻輳制御の誤動作が回避されます。
