...
:::message
筆者のお気持ち: 自分たちのノード規模とそれによる冗長経路を持つトポロジーを逆に効率化に利用した発想が面白い！
:::
...
スプレイディングは、パケットが異なる経路を通るため、経路ごとの遅延差により out-of-order delivery が発生します。従来の TCP はこれを「パケットロス」と誤検知して性能低下を引き起こしますが、SRD は out-of-order を前提とした設計により、この問題を回避しています。順序管理はアプリケーション層（MPI タグマッチング、NCCL の通信 ID など）に委ねられ、各アプリケーションが必要とする順序保証レベルを実装します。
::::
...
SRD のパケットスプレイは Nitro Card のカスタム ASIC（ハードウェア）上で実装されています。各パケットの送信時、Nitro Card はまず利用可能な経路（複数の Spine スイッチへのパス）をスキャンし、リアルタイムで最適な経路を選択します。このとき、各経路の RTT（Round-Trip Time）と ACK タイミングを継続的に監視することで、輻輳している経路を検出します。輻輳が検出された経路は使用を抑制し、空いている経路により多くのパケットを振り向けることで、動的な負荷分散を実現します。さらに、経路で障害が発生した場合でも、ルーティングプロトコルの経路再収束を待つことなく、即座に他の健全な経路へスイッチします。

ハードウェア実装により、パケットごとの経路選択がワイヤースピード（100 Gbps 以上）で実行可能となり、マイクロ秒オーダーの応答速度を実現しています。ソフトウェア実装では避けられないカーネル処理やコンテキストスイッチのオーバーヘッドが完全に排除され、処理時間のばらつき（ジッター）も最小化されます。この予測可能な低レイテンシ特性により、マルチテナント環境の予測不可能なトラフィック変動に対しても、安定したスループットを維持できます。他のテナントのトラフィックバーストが発生した際でも、SRD は瞬時に空いている経路にパケットを振り向けることで、影響を最小限に抑えます。

### アウトオブオーダー配信の特性

複数経路を使用すると、各経路の遅延差によりパケットの到着順序が送信順序と異なる場合があります。TCP/IP ではアウトオブオーダーのパケット到着を損失と誤検知し、不必要な再送や輻輳ウィンドウの縮小を引き起こします。

**SRD の設計原則**: SRD は意図的に out-of-order delivery を許容し、パケットをアプリケーション層に直接配信します。順序管理はアプリケーション層（MPI タグマッチングなど）に委ねられ、これにより各アプリケーションが必要とする順序保証レベルを柔軟に実装できます。

この設計により、第一に、パケットごとに最適な経路を選択することでネットワークの並列性を最大限に活用し、head-of-line blocking を回避できます。第二に、TCP のように順序待ちでパケットを保留する必要がないため、不要なバッファリング遅延が排除されます。第三に、MPI や NCCL などの通信ライブラリが各々のセマンティクスに応じた順序管理を実装することで、アプリケーション要求に最適化された順序保証が実現されます。TCP/IP のようにアウトオブオーダーを「異常」として扱う必要がなく、パケットロスと誤認した場合の不要な再送や輻輳制御の誤動作も回避されます。
...
ソフトウェア実装では、マルチテナント環境で常に発生する他テナントのトラフィック変動に対して、マイクロ秒単位で応答することは困難です。カーネル処理やスケジューラの遅延により、応答時間がミリ秒オーダーになってしまうためです。また、各パケットの送信時にリアルタイムで最適経路を決定するには、ハードウェアレベルの高速処理が不可欠です。さらに、ネットワークの複数パスでの集約キューイングを最小限に保つためには、接続ごとの動的レート制限と輻輳検知をワイヤースピードで実行する必要があり、これも AWS データセンターのマルチパス特性を考慮した独自ハードウェア設計により実現されています。
...
| **タイムアウトによる遅延** | なし | 200ms 以上多数 | - |

:::message
**SRD の性能優位性**

AWS 公式論文（IEEE Micro 2020, Figure 2, 3）によると、48-way incast という厳しい条件下でも、SRD は優れた性能を実現しています。第一に、最大 FCT が TCP の 1/3 〜 1/20 となり、転送サイズにより大幅な改善を達成しています。第二に、FCT のばらつきが極めて小さく、予測可能な性能を提供します。第三に、タイムアウトによる遅延がゼロであり、TCP で頻発する最小 200ms の再送タイムアウトを完全に回避しています。
...
**TCP の問題点**（論文より）: TCP は FCT のばらつきが大きく、ノイジーな性能特性を示します。50ms 以上のテールレイテンシは再送タイムアウト（200ms）に起因し、タイムアウトに到達しないサンプルでも多数が理想値の 3 倍以上となります。パケットロスによる再送がボトルネックとなり、予測可能な性能を提供できません。
...
AWS 公式論文（IEEE Micro 2020, Figure 6）によると、理想的な負荷分散が期待される条件下でも、TCP は深刻な問題を示しています。中央値 FCT が理想値より 50% 高く、ECMP の不均衡による輻輳が発生します。さらに、最大 FCT が理想値の 1〜2 桁（10〜100 倍）に達し、パケットドロップとタイムアウトが頻発します。

一方、SRD は優れた性能を実現しています。中央値 FCT が理想値 + 15% とほぼ最適な値に収まり、最大 FCT も理想値 + 15% 程度に抑えられ、テールレイテンシが極めて低くなっています。
...
本記事では、AWS の EFA を中心に、Nitro System のハードウェアアーキテクチャ、SRD プロトコルの設計思想、EFA のソフトウェアスタック実装までを解説しました。この辺りの技術は久しぶりに触れたので楽しかったです。
...
- [AWS EC2 Virtualization 2017: Introducing Nitro - Brendan Gregg](https://www.brendangregg.com/blog/2017-11-29/aws-ec2-virtualization-2017.
