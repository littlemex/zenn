...
しかし、PFC には大規模ネットワークにおいて深刻な問題があります。第一に、**Head-of-line blocking** が発生します。1 つのフローが停止すると、同じリンクを共有する他の無関係なフローもブロックされてしまいます。第二に、**輻輳の伝播**が起こります。PAUSE フレームが連鎖的に上流スイッチに伝播し、輻輳がネットワーク全体に拡散します。第三に、**デッドロック**のリスクがあります。複数のスイッチ間で循環的な停止が発生すると、通信が完全に停止する可能性があります。理論的には、デッドロックフリーなルーティング設計や優先度クラスの適切な管理により回避可能ですが、AWS 公式論文では、既存の PFC デッドロック対策（Guo et al., 2016）が「AWS データセンターよりも大幅に小さいデータセンター」を前提としており、AWS スケールでは実用的でないと指摘されています。

SRD は、RoCE とは根本的に異なるアプローチを採用しています。SRD はパケットロスを許容する設計であり、信頼性は高速リトライで確保します。

輻輳制御については、SRD はエンドツーエンドで **BBR（Bottleneck Bandwidth and RTT）風のアルゴリズム**を採用しています。BBR は Google が開発した輻輳制御アルゴリズムで、従来の TCP とは異なるアプローチを取ります。従来の TCP はパケットロスを輻輳のシグナルとして送信レートを調整しますが、BBR はボトルネックの帯域幅と RTT（Round-Trip Time）を測定し、これらに基づいて送信レートを動的に調整します。この方式により、パケットロスが発生する前に送信レートを制御でき、ネットワークのキューイングを最小限に保ちながら高スループットを実現できます。

SRD の輻輳制御は、AWS 公式論文によると「受信 ACK のタイミングに基づくレート推定」と「最近の送信レートと RTT の変化」を考慮します。接続ごとの動的レート制限とインフライト制限を組み合わせ、複数パスでの集約キューイングを最小限に保ちます。この設計により、PFC のようなリンクレベルのバックプレッシャーは不要となります。
...
SRD は、この問題に対して革新的なアプローチを採用しています。SRD は**パケットレベルで out-of-order delivery を許容**し、複数経路を同時に使用します。ただし、TCP のようなバイトストリームではなく、**メッセージ境界を保持**します。パケットの順序管理は、アプリケーション層のメッセージング層（例: MPI タグマッチング）に委ねられます。この設計により、SRD は**ネットワークの並列性を最大限に活用**しながら、アプリケーション要求に応じた順序保証を実現します。
:::
