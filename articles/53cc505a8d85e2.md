...
print(f"  次のステップ: Step 2 をスキップして Step 3（推論実行）へ (03_inference.py)")
print("=" * 80)
PYTHON_EOF
...
================================================================================
  ...
  次のステップ: Step 2 をスキップして Step 3（推論実行）へ (03_inference.py)
================================================================================
```
::::
...
:::message
**重要な点**:
**GPT forward_wrapper での padding 処理**: Neuron の固定形状制約に対応するため、入力を固定長 (text_len=50, audio_len=100) に padding/truncate しました。詳細は以下のトラブルシューティングログを確認してください。
:::

いくつかトラブルシューティングがあったのでログを残しておきます。
...
# load_audio 関数を事前にpatch（TTS.apiのimport前に実行）
import TTS.tts.models.xtts

def load_audio_fixed(audiopath, sampling_rate):
    """soundfile を使用して音声ファイルをロード（torchcodec互換性問題を回避）"""
...
if gpt_neuron_path.exists():
    ...
    COMPILED_AUDIO_LEN = 100

    def forward_wrapper(self, text_inputs, text_lengths, audio_codes, wav_lengths,
                        cond_latents=None, return_attentions=False, return_latent=False):
        if hasattr(self, 'forward_neuron'):
            # Neuron の固定形状制約に対応するため、padding/truncate を実行
            batch_size = text_inputs.size(0)
            actual_text_len = text_inputs.size(1)
            actual_audio_len = audio_codes.size(1)

            # text_inputs を固定長にpadding/truncate（データ型も torch.int64 に変換）
            ...
            # audio_codes を固定長にpadding/truncate（データ型も torch.int64 に変換）
            ...
            wav_lengths_fixed = torch.tensor([COMPILED_AUDIO_LEN], dtype=wav_lengths.dtype, device=wav_lengths.device)

            return self.forward_neuron(text_inputs_padded, text_lengths_fixed, audio_codes_padded, wav_lengths_fixed, cond_latents)
        else:
            return self.forward_original(text_inputs, text_lengths, audio_codes, wav_lengths,
                cond_latents=cond_latents, return_attentions=return_attentions, return_latent=return_latent)
...
sequenceDiagram
    ...
    participant HifiDecoder as HifiDecoder.forward()
    participant HifiDecoder_Neuron as forward_neuron
    ...
    GPT->>HifiDecoder: latents
    HifiDecoder->>HifiDecoder: hasattr(self, 'forward_neuron')?
    alt forward_neuron が存在（--hifidecoder-device neuron）
        HifiDecoder->>HifiDecoder_Neuron: Neuron で実行
        HifiDecoder_Neuron-->>HifiDecoder: waveform
    else forward_neuron が存在しない（--hifidecoder-device cpu）
        HifiDecoder->>HifiDecoder: CPU で実行
...
今回は XTTS v2 という end-to-end ではないモデルを AWS Neuron でコンパイル、推論する仕組みを検証しました。Forward Override パターンは必須の技術ではありませんが主要ではないモデルを運用コストを下げ、CPU/GPU との環境互換性を維持しながら推論サービングする際には有用かもしれません。GPT については問題なくコンパイル・推論ができました。HifiDecoder の AWS Neuron コンパイルは未サポートでしたが実務上は CPU で全く問題がないです。CPU に向く演算をあえてカスタムチップで実施する必要性は薄いため適材適所で計算リソースを使うという判断が重要です！強化学習の推論は MIMD のようなので CPU 推論のニーズも増えていくことを考えると今後はより計算リソースの環境互換性や適材適所での利用、ヘテロジニアス環境での利用というのが重要になってくると思っています。
