...
6. **実行時デバイス選択**: HifiDecoder の実行デバイスを推論時に選択可能（CPU/Neuron）
...
```mermaid
graph TB
    subgraph "model.gpt インスタンス"
        A[forward]
        B[forward_original]
        C[forward_neuron]
    end

    A -->|"振り分けロジック<br/>(forward_wrapper)"| D{forward_neuron<br/>存在？}
    D -->|YES| C
    D -->|NO| B

    B -->|"元の forward メソッド<br/>(CPU/GPU 実行)"| E[GPT オリジナルの forward]
    C -->|"Neuron コンパイル済み<br/>(NeuronCore 実行)"| F[TorchScript モジュール]
...
TorchScript モジュールは PyTorch モデルの最適化された実行可能形式で、NeuronCore 実行ようにコンパイル済みであり、Python インタプリタを経由しない高速実行、`torch.jit.load()` で保存・復元可能、などの特徴があります。
...
"""
GPT モデルを torch_neuronx.trace() でコンパイルして保存
"""
import sys
import os
...
try:
    ...
    neuron_gpt.save(str(output_dir / "gpt_neuron.pt"))

    print(f"  [OK] コンパイル済みモデルを保存: models/compiled_gpt/gpt_neuron.pt")
...
print("Step 1 完了")
print("=" * 80)
print(f"  コンパイル済みモデル: models/compiled_gpt/gpt_neuron.pt")
print(f"  次のステップ: HifiDecoder のコンパイル (02_compile_hifidecoder.py)")
print("=" * 80)
PYTHON_EOF
...
Compiler status PASS
  ...
  [OK] コンパイル済みモデルを保存: models/compiled_gpt/gpt_neuron.pt

================================================================================
Step 1 完了
================================================================================
  コンパイル済みモデル: models/compiled_gpt/gpt_neuron.pt
  次のステップ: HifiDecoder のコンパイル (02_compile_hifidecoder.py)
================================================================================
```
::::
...
::::details ステップ 2 のコード
Step 1 で GPT をコンパイルしたのと同様に、HifiDecoder を Neuron にコンパイルします。

:::message
HifiDecoder は GPT の出力（latents）を音声波形に変換するボコーダーです。このステップは**オプション**です。HifiDecoder は軽量なモデル（約 12M パラメータ）のため、CPU でも高速に動作します。GPU/Neuron がない環境では、このステップをスキップして CPU で実行できます。
:::

```python
cat > 02_compile_hifidecoder.py << 'PYTHON_EOF'
"""
HifiDecoder を torch_neuronx.trace() でコンパイルして保存
"""
import sys
import os
os.environ['PATH'] = '/opt/aws_neuronx_venv_pytorch_2_9_nxd_inference/bin: ' + os.environ.get('PATH', '')
...
print("=" * 80)
print("Step 2: HifiDecoder のコンパイル")
print("=" * 80)
...
# HifiDecoder の入力は GPT の出力（latents）と speaker_embedding (g)
# 形状: (batch, seq_len, dim) - GPT の出力形状そのまま
# speaker_embedding は .unsqueeze(-1) された形状
batch_size = 1
latent_len = 100  # sequence length
latent_dim = 1024  # hidden dimension
d_vector_dim = 512  # speaker embedding dimension

latents = torch.randn(batch_size, latent_len, latent_dim)
speaker_embedding = torch.randn(batch_size, d_vector_dim, 1)  # speaker embedding (.unsqueeze(-1) 済み)

print(f"  latents: {latents.shape}  # (batch, seq_len, dim)")
print(f"  speaker_embedding: {speaker_embedding.shape}  # (batch, d_vector_dim, 1)")
...
with torch.no_grad():
    try:
        # HifiDecoder の forward メソッドを呼び出し
        output = model.hifigan_decoder.forward(latents, g=speaker_embedding)
        print(f"  [OK] CPU inference successful")
        print(f"  Output shape: {output.shape}")
    except Exception as e:
        print(f"  [ERROR] CPU inference failed: {e}")
...
# Neuron コンパイル
print("\n[4/4] Neuron コンパイル...")
print(f"  [NOTE] これには約 20 秒かかります...")
print(f"  [NOTE] コンパイラログは /tmp/neuron_cache_hifidecoder/ に保存されます")

try:
    import types

    # forward をそのまま使用（speaker_embedding を渡す）
    ...
    # コンパイル済みモデルを保存
    output_dir = Path("models/compiled_hifidecoder")
    output_dir.mkdir(parents=True, exist_ok=True)
    neuron_hifidecoder.save(str(output_dir / "hifidecoder_neuron.pt"))

    print(f"  [OK] コンパイル済みモデルを保存: models/compiled_hifidecoder/hifidecoder_neuron.pt")
...
print("Step 2 完了")
print("=" * 80)
print(f"  コンパイル済みモデル: models/compiled_hifidecoder/hifidecoder_neuron.pt")
print(f"  次のステップ: 推論実行 (03_inference.py)")
print(f"  推論時に --hifidecoder-device [cpu|neuron] で実行デバイスを選択可能")
print("=" * 80)
PYTHON_EOF

# 実行（Neuron 環境でのみ動作）
python 02_compile_hifidecoder.py
```
::::
...
[2/4] ダミー入力を作成...
  latents: torch.Size([1, 100, 1024])  # (batch, seq_len, dim)
  speaker_embedding: torch.Size([1, 512, 1])  # (batch, d_vector_dim, 1)

[3/4] CPU で動作確認...
  [OK] CPU inference successful
  Output shape: torch.Size([1, 1, 2304000])

[4/4] Neuron コンパイル...
  [NOTE] これには約 20 秒かかります...
  [NOTE] コンパイラログは /tmp/neuron_cache_hifidecoder/ に保存されます
  [OK] Neuron compilation successful
  [OK] コンパイル済みモデルを保存: models/compiled_hifidecoder/hifidecoder_neuron.pt

================================================================================
Step 2 完了
================================================================================
  コンパイル済みモデル: models/compiled_hifidecoder/hifidecoder_neuron.pt
  次のステップ: 推論実行 (03_inference.py)
  推論時に --hifidecoder-device [cpu|neuron] で実行デバイスを選択可能
================================================================================
```
::::

:::message
HifiDecoder は軽量モデル（約 12M パラメータ）のため、**CPU でも十分高速**に動作します（約 40ms）。Step 3 の推論実行時に `--hifidecoder-device cpu` または `--hifidecoder-device neuron` で実行デバイスを選択できます。デフォルトは `cpu` です（推奨）。
:::
...
HifiDecoder は `--hifidecoder-device` 引数で実行デバイスを選択できます（デフォルト: `cpu`）。

::::details ステップ 3 のコード
```python
cat > 03_inference.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
Step 3: XTTS v2 で音声合成を実行（Neuron コンパイル済みモデルを自動使用）
"""
import sys
import os
os.environ['COQUI_TOS_AGREED'] = '1'
...
# GPT Neuron モデルをロード
print("\n[2/6] GPT Neuron モデルをロード...")

gpt_neuron_path = Path("models/compiled_gpt/gpt_neuron.pt")
if gpt_neuron_path.exists():
    ...
    model.gpt.forward = types.MethodType(forward_wrapper, model.gpt)
    model.gpt.forward_neuron = neuron_gpt
    print(f"  [OK] GPT Neuron モデルロード完了")
else:
...
print(f"  GPT: Neuron で実行")
print(f"  HifiDecoder: {args.hifidecoder_device.upper()} で実行")
print(f"  Forward Override パターンにより、コンパイル済みモデルが自動的に使用されました")
print("=" * 80)
PYTHON_EOF

# 実行（デフォルト: HifiDecoder = CPU、推奨）
python3 03_inference.py

# または HifiDecoder も Neuron で実行する場合
# python3 03_inference.py --hifidecoder-device neuron
```
::::
...
Step 3 完了
================================================================================
  音声ファイル: /home/coder/data-science/investigations/coqui-model-to-neuron/outputs/xtts_neuron_gpt_cpu.wav
  GPT: Neuron で実行
  HifiDecoder: CPU で実行
  Forward Override パターンにより、コンパイル済みモデルが自動的に使用されました
================================================================================
```
::::

::::details HifiDecoder を Neuron で実行する場合の出力例

HifiDecoder も Neuron で実行する場合は、以下のようにコマンドを実行します:

```bash
python3 03_inference.py --hifidecoder-device neuron
```

このように、XTTS v2 の内部実装を変更せずに、外から Neuron 対応を追加できます。HifiDecoder は推論時に `--hifidecoder-device` 引数で実行デバイスを選択可能です（デフォルト: CPU、推奨）。
::::

XTTS v2 Neuron の実行手順は以下の通りです。HifiDecoder のデバイス選択に応じて、2 つの実行パターンがあります。

#### パターン 1: GPT = Neuron、HifiDecoder = CPU（推奨）

```bash
# Step 1: GPT をコンパイル（約 1-2 分）
python3 01_compile_gpt.py

# Step 2: スキップ可能（HifiDecoder は CPU で十分高速）

# Step 3: 推論実行
python3 03_inference.py

# 出力ファイル
# outputs/xtts_neuron_gpt_cpu.wav
```

:::message
**推奨パターン**: HifiDecoder は軽量モデル（約 12M パラメータ）のため、CPU でも十分高速に動作します。このパターンでは Step 2 をスキップできるため、コンパイル時間を短縮できます。
:::

#### パターン 2: GPT = Neuron、HifiDecoder = Neuron

```bash
# Step 1: GPT をコンパイル（約 1-2 分）
python3 01_compile_gpt.py

# Step 2: HifiDecoder をコンパイル（約 20 秒）
python3 02_compile_hifidecoder.py

# Step 3: 推論実行（HifiDecoder も Neuron で実行）
python3 03_inference.py --hifidecoder-device neuron

# 出力ファイル
# outputs/xtts_neuron_gpt_neuron.wav
```

:::message alert
HifiDecoder を Neuron で実行してもパフォーマンス向上は限定的です。**パターン 1（HifiDecoder = CPU）を推奨**します。
:::
...
```bash
# ファイル情報の確認（HifiDecoder = CPU の場合）
ls -lh outputs/xtts_neuron_gpt_cpu.wav

# または HifiDecoder = Neuron の場合
# ls -lh outputs/xtts_neuron_gpt_neuron.wav
...
# HifiDecoder = CPU の場合
audio_path = Path.cwd() / "outputs" / "xtts_neuron_gpt_cpu.wav"

# または HifiDecoder = Neuron の場合
# audio_path = Path.cwd() / "outputs" / "xtts_neuron_gpt_neuron.wav"
...
2. [OK] HifiDecoder (11.8M params) の Neuron コンパイル（20.5 秒、オプション）
3. [OK] Forward Override パターンでの統合
4. [OK] コンパイル済みモデルでの音声生成パイプライン
5. [OK] HifiDecoder のデバイス選択機能（`--hifidecoder-device` 引数）

**プリミティブな実装の利点**:
- `torch_neuronx.trace()` だけでシンプルに実装
- 既存の PyTorch コードに最小限の変更で統合
- コンパイル済みモデルを `.pt` ファイルとして保存・再利用可能
- HifiDecoder のデバイスを実行時に選択可能（CPU/Neuron）

**推奨構成**:
- **GPT**: Neuron で実行（231 倍高速化）
- **HifiDecoder**: CPU で実行（推奨、十分高速: 約 40ms）
- この構成により、コンパイル時間を短縮しつつ高いパフォーマンスを実現
