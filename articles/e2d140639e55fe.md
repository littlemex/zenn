...
ここで注意したいのは、`EndpointPickerConfig` は `kubectl apply` するリソースではなく、その内容 (schedulingProfiles 定義) を **ConfigMap に格納し、EPP コンテナへ `/config/profile.yaml` として mount** して使う点です。本記事で示す profile YAML はこの ConfigMap に入る中身です。
...
[`50-epp-deployment.yaml`](https://github.com/littlemex/distributed-inference/blob/main/2026-06-21-multitenant-lora-vs-bedrock-b300/llm-d/manifests/50-epp-deployment.yaml) は Envoy sidecar と EPP scheduler の 2 コンテナで構成されます (以下は要点抜粋。実ファイルには `--pool-group` などの引数も含みます)。
