...
| 項目 | 値 |
|---|---|
| インスタンス | AWS p6-b300 (8x B300, 各 275GB HBM) |
| GPU 割り当て | 1 Pod = 1 GPU (TP=1) × 8 Pod |
| モデル | Gemma 4 31B fp8 |
| LoRA テナント | 128 (adapter-0..127)、各 Pod に全 adapter 登録済み |
| テナント分布 | Zipf 1.1 (人気テナントに偏り) |
| concurrency | 8, 32, 64, 128, 256, 512 |
| requests / stage | 512 |
| max_tokens | 64 |
| SLO | TTFT ≤ 2000ms かつ TPOT ≤ 80ms |
| 計測ツール | `concurrency_sweep.py` (同一ノードの bench Pod から実行) |

3 条件の差は **routing ロジックの違いだけ**で、LoRA の有無や個数・モデル・GPU 構成は全条件で同一です。
