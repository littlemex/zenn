...
| 計測ツール | `concurrency_sweep.py` (同一ノードの bench Pod から実行) | - |
...
**`epp-aff` (lora-affinity 単独) だけが 72.4 req/s に落ち込み**、ベースラインの `epp-rr(round robin)` (119.9) を大きく下回ります。一方 **`epp-full` (全 scorer 合成) は 123.9 req/s で最高**です。
...
**epp-full が最良で**、`lora-affinity-scorer` に `queue-scorer` と `kv-cache-utilization-scorer` を加えて重み付き合成することで、「アダプタが既にロード済みの Pod を優先する (局所性)」と「キューが詰まっている Pod を避ける (負荷分散)」が同じスコア空間で自動的に調停されます。結果として、崩壊した epp-affinity (72.4) はもちろん、aware なしの epp-rr (119.9) も上回る 123.9 req/s を達成しました。**「LoRA-aware を足すこと」自体ではなく、「他の load-aware と合成すること」が効いている**ことが分かります。

## そもそも LoRA はコストが乗るのか？

ここまでで「LoRA-aware routing で swap-in を避ければキュー詰まりを防げる」ことを実測で確認しました。では、アダプタが GPU に載っている (swap が起きない) 状態でも、LoRA には固有のコストがあるのか。今回の実験上あります。「swap が遅い」だけではなく、**decode 中に毎トークン走る LoRA カーネルの計算コスト**です。

なお本節で使う **B / B0** は、上記 8 Pod 実験の前段として 1Pod×8 プロセス構成で取得した事前検証データです。**B = LoRA あり、B0 = LoRA なし** で、LoRA のコストだけを切り出すために両者を直接対比しています。
...
### prefill と decode で LoRA カーネルの効きが違う

LLM 推論には 2 つのフェーズがあり、上の 2 段カーネルが**フェーズごとに別の効き方**をします。ポイントは「**一度に何トークン処理するか (= 行列の M)**」です。

- **prefill**: 入力プロンプト全体をまとめて処理する → 一度に**数百トークン** (M が大きい)
- **decode**: 出力を 1 トークンずつ生成する → 1 ステップで処理するのは**バッチ分だけ** (M が小さい)

GPU の行列積は M が大きいほど効率がよく、小さいと固定コストが相対的に重くのしかかります。だから LoRA のオーバーヘッドは、M の大きい prefill では埋もれ、M の小さい decode で顕在化します。

| | prefill | decode |
|---|---|---|
| 処理単位 | 入力プロンプト全体を一括 | 出力 1 トークンずつ |
| 行列の M | 大きい (数百トークン) | 小さい (バッチ分のみ) |
| 使うカーネル | SGMV 相 (Segmented Gather Matrix-Vector) | BGMV 相 (Batched Gather Matrix-Vector) |
| LoRA オーバーヘッド | base 計算に対して相対的に小さい | 相対的に大きい |
| 効く指標 | TTFT (最初のトークンまで) | **TPOT (1 トークンあたりの生成時間)** |

つまり **TPOT を悪化させる主因は decode フェーズ (BGMV 相) の per-token LoRA 計算**です。出力が長いほど decode ステップが積み重なるので、この影響は効いてきます。
...
ちなみに CPU pool からのアダプタ swap-in が TTFT に上乗せする時間 (アダプタが既に GPU にある場合との差) は約 1.2ms です。CPU から GPU へのコピー自体は 3〜7ms かかりますが、vLLM が他の処理と重ねて実行するため、TTFT への実際の上乗せは 1.2ms 程度に収まります。いずれにせよ、数十 ms 規模の TPOT に毎トークン乗る BGMV のコストとは桁が違います。

:::message
この比較は厳密ではない点に注意してください。B と B0 は (1) トラフィックの偏り (B=Zipf 1.1 / B0=均等)、(2) 飽和しているかどうか、(3) プレフィックスキャッシュ (vLLM の Automatic Prefix Caching。同じプロンプト先頭の計算を使い回す機能) を B0 だけ有効にしている、の 3 点が違うため、「TPOT 比 = カーネルのオーバーヘッド比」とそのまま言い切ることはできません。これらの差の影響が最も小さい (飽和前の) concurrency 256 の 2.57 倍が、最も素直な推定値です。それでも全 concurrency で比が約 2.5〜2.9 倍と安定していることが、「カーネルが主因」という結論を支えています。
:::

ここから分かるのは、LoRA-aware routing の効果と限界です。routing で swap-in とキュー詰まりは避けられますが、**1 トークンごとに乗る BGMV カーネルのコストは routing では消せません**。これは LoRA を使うこと自体に付いてくるコストなので、設計では別に織り込む必要があります。

## まとめ

スヤァ、ちょっと疲れたので 1000 年ほど寝ます。

![](https://zukan.pokemon.co.jp/zukan-api/up/images/index/33b0d2c0d0048ff76f152860c2ab6e26.png)

> https://zukan.pokemon.co.jp/detail/0385 より
