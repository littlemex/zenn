---
title: "TwelveLabs Marengo on Amazon Bedrock で動画セマンティック検索を実装する"
emoji: "🎬"
type: "tech"
...
本記事では、Amazon Bedrock を通じて TwelveLabs Marengo を実際に動かしてみます。AWS 公式ブログの説明の方が多分わかりやすいです。にゃ〜ん。
...
動画処理は時間がかかるため、非同期 API（`start_async_invoke`）を使用します。処理結果は S3 に保存されます。
...
def embed_video(video_s3_uri: str, output_prefix: str = "output") -> list[dict]:
    # ステップ 1: 非同期ジョブ開始
...
```bash
# AWS Workshop Assets から MP4 をダウンロード（47 MB）
curl -L \
...
- AWS CLI、Python 3.8 以上、ffmpeg、curl がインストール済みであること
...
```bash
# Netflix Meridian をダウンロード（47 MB、Creative Commons 4.0）
curl -L \
  "https://ws-assets-prod-iad-r-pdx-f3b3f9f1a7d6a3d0.s3.us-west-2.amazonaws.com/335119c4-e170-43ad-b55c-76fa6bc33719/NetflixMeridian.mp4" \
  -o ~/marengo-demo/data/NetflixMeridian.mp4

# 先頭 60 秒に切り出し（6.5 MB 程度になる）
...
# sed -i '' 's/AWS_PROFILE = "default"/AWS_PROFILE = "your-profile"/' \
#   ~/marengo-demo/src/demo_search.py
...
```
=== Step 1: 動画埋め込み生成 ===
[OK] 非同期ジョブ開始: arn:aws:bedrock:us-east-1:xxxxxxxxxxxx:async-invoke/xxxxxxxx
...
`us.` プレフィックスはクロスリージョン推論（Cross-Region Inference）用のプロキシモデル ID です。非同期 API はプロキシ経由ではなく、直接モデルを指定する必要があります。
...
動画の処理には入力用と出力用の S3 バケットへのアクセス権限が必要です。

- 入力動画: `bedrock-runtime` が `s3:GetObject` できること
- 出力結果: `bedrock-runtime` が `s3:PutObject` できること

バケットポリシーで Bedrock サービスプリンシパル（`bedrock.amazonaws.com`）からのアクセスを許可する必要があります。
...
:::message
AI 君に考えさせただけなのでどのくらい各シナリオでちゃんと精度良く使えるのか検証してません！
:::
...
ユーザー: "夕日が海に沈む美しい映像"
  ...
  → OpenSearch / S3 Vectors で visual 埋め込みとコサイン類似度検索
...
視聴中の動画の asset 埋め込みを使い、コレクション内の類似動画をコサイン類似度でランキングして推薦できます。visual、audio、transcription を組み合わせたハイブリッドスコアリングにより、映像・音楽・内容それぞれの観点で類似度を計算することも可能です。
...
マルチモーダル埋め込みの活用により、テキストで動画を検索する、映像の雰囲気で類似動画を探す、発話内容で関連動画を見つけるといった高度な動画検索システムを Amazon Bedrock だけで構築できるのはよさげですね。Amazon S3 Vectors との組み合わせで、大規模なコレクションに対してもスケーラブルな動画検索インフラを実現でき、サンプルが公開されているのでぜひ試してみると良いです。
