...
**複数デバイスにまたがる場合**: リングを断ち切って複数のランクセットに分割すると、各セット間には直接接続がないため、セット間で Ring All-Reduce などの CC 演算を効率的に実行できません。これは以下を意味します：
...
デイジーチェーン構成により、各レイヤーの FSDP AllGather が前のレイヤーの計算中にプリフェッチされるよう、適切なタイミングで実行されると想定されます。これにより、TP 通信と FSDP 通信が同時に実行されてリソース競合を起こすことを防ぎます。
...
:::message
**注意**: トークンの受け渡し順序の具体的な決定方法や、トークンが実行時にどのように処理されるか（例: 最適化により削除されるか）については、元のドキュメントに明記されていません。上記の「より分かりやすい解釈」は、公式ドキュメントの記述と一般的な同期メカニズムの知識に基づく推測を含みます。
:::
