...
各 Neuron デバイスには 2 つの NeuronCore が含まれており、これらは "remote-vertical" と呼ばれる専用インターコネクトで接続されています。この接続は各方向で 200 GB/s の帯域幅を提供します。

:::message
この構成は Trainium1 (Trn1) 以降の世代に適用されます。第1世代の AWS Inferentia (Inf1) では、デバイスあたり 4 つの NeuronCore-v1 が含まれており、異なるアーキテクチャを採用しています。本記事では主に Trn1 以降の世代を対象としています。
:::
...
graph LR
    subgraph "Single Neuron Device"
        ...
        NC1 <-->|"Remote-Vertical<br/>200 GB/s each direction"| NC2
    end
```
...
各 Neuron デバイスは 4 つの PCIe コネクタを持ち、これらがチップ間通信に使用されます。（[Trn1 アーキテクチャドキュメント](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/general/arch/neuron-features/collective-communication.html)）。

trn1.32xlarge では、これらの PCIe リンクを使用してデバイスを 2D トーラストポロジーに接続します。
...
**単一デバイスに閉じる場合（2 ランク）**: リングを断ち切って分割しても、各セットが単一の Neuron デバイス内（remote-vertical 接続）で完結する場合、パフォーマンス影響はありません。

**複数デバイスにまたがる場合**: リングを断ち切って複数のランクセットに分割すると、各セット間には直接接続がないため、セット間で Ring All-Reduce などの CC 演算を実行できません。これは以下を意味します：

- 各ランクセットは独立した通信グループとして動作します
- セット間でデータを交換する必要がある場合、Ring All-Reduce の最適化が使えず、パフォーマンスが低下します
- 実質的には、同じモデルの複数コピーを並行実行する形になります

したがって、inf2.48xlarge で 3 セットの 8 ランクを実行する場合、8 ランクの Tensor Parallel 実行用にコンパイルされたモデルの 3 つの独立したコピーを並行実行することになります。これらのコピー間で効率的な Data Parallel 通信を行うことはできません（パフォーマンス劣化を許容すればできないわけではありません）。
...
各 Trainium2 チップは以下のコンポーネントで構成されています。

- **8 個の NeuronCore-v3**
- **96 GiB の HBM3/HBM3e メモリ**（2.9 TB/s の帯域幅）
- **NeuronLink-v3**: チップ間インターコネクト、1.28 TB/s の帯域幅
- **16 個の CC-Cores**: Collective Communication 専用の同期プロセッサ
...
- **LNC=8**: 全 8 つの NeuronCore を 1 つの論理ランクに統合（1 ランク/チップ）

:::message alert
**重要**: LNC は Trainium2 における重要な進化です。Inferentia2 とは異なり、Trainium2 では LNC 設定により NeuronCore を分割（LNC=1）しても統合（LNC=8）してもパフォーマンスへの影響を最小限に抑えながら、ワークロードに最適な構成を柔軟に選択できます。これにより、Tensor Parallel の粒度を自由に調整でき、より効率的な分散学習が可能になります。

Inf2 でリングを分割すると独立したセット間で効率的な通信ができなくなるのに対し、Trn2 の LNC は単一チップ内で論理的にランク数を制御するため、トポロジー全体の接続性を損なうことなく柔軟な構成が可能です。
:::
...
Trainium3 は AWS の第 4 世代 AI チップで、3nm プロセスを採用した最初の AWS AI チップです（[Trn3 アーキテクチャドキュメント](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/about-neuron/arch/neuron-hardware/trn3-arch.html)）。最も重要な変更は、トーラストポロジーから All-to-All スイッチファブリックへの移行です。
...
各 Trainium3 デバイスの主要な仕様は以下の通りです。

- **8 個の NeuronCore-v3**
- **144 GB の HBM3e メモリ**（4.9 TB/s の帯域幅、Hynix または Micron 製）
- **NeuronLink-v4**: 2.048 TB/s の帯域幅（PCIe Gen6、144 レーン）
- **16 個の CC-Cores**（Trn2 と同じ）
- **NeuronSwitch-v1**: All-to-All スイッチファブリック
...
NeuronSwitch-v1 は階層的なスイッチアーキテクチャを採用しており、サーバー内デバイスを接続する第 1 レベルスイッチと、サーバー間デバイスを接続する第2レベルスイッチで構成されています。この設計により、大規模な Trainium3 クラスタでも効率的な All-to-All 接続が可能になります。
...
DMA エンジンは計算サブシステムと通信サブシステムの間での**共有リソース**です。この共有により、リソース競合が発生する可能性があります。DMA トランザクションの遅延により、計算操作のスループットが低下する場合があります。
...
従来の Neuron Compiler の自動スケジューリングでは、FSDP AllGather 操作をできるだけ早いタイミングでまとめて実行しようとします。しかし、この方式では重要な TP 通信操作とのリソース競合を引き起こし、全体のパフォーマンス低下を招きます。
...
簡単に言うと、Token Threading は**デッドロックのようなリソース競合を回避するための順序制御**です。TP 通信と FSDP 通信の両方が同じ DMA リソースを必要とする場合、どちらを先に実行するか明確でないと競合が発生します。

Token Threading は「トークン」と呼ばれる小さなダミーデータ（単一要素のテンソル）を使って、CC 操作を強制的にシーケンシャル（順番）に実行させることで、順序関係を明確にします：

1. **CC 操作 A が完了** → トークンを出力
2. **トークンを CC 操作 B の入力として受け取る** → CC 操作 B はトークンが来るまで待つ
3. **CC 操作 B が実行される** → 次のトークンを出力

この「トークンの受け渡し」により、CC 操作は数珠つなぎのように順番に実行されます。Neuron Compiler はこの順序を守りながらスケジューリングを行いますが、トークン自体は実際のデータではないため、最終的な実行時には削除されます（実行順序だけが残る）。

**トークンの受け渡し順序はどう決まる？**

Neuron PJRT Compiler Pass（コンパイラの最適化パス）が、TP と FSDP 次元にまたがるオペレーションパターンを自動的に識別し、レイヤーの実行順序とデータ依存関係を解析します。この解析に基づいて、Compiler が最適なトークン受け渡し順序を決定します。

具体的には、各レイヤーの計算と次のレイヤーの FSDP AllGather が重なるように、レイヤーの順番に沿ってトークンが受け渡されます。開発者が手動で順序を指定する必要はなく、Compiler が自動的に最適化します。
