...
LLM 推論周りのプロファイリングをしたいのでそのための環境を作りました。プロファイルを取った後に数 GB の `.nsys-rep` などの解析バイナリをローカルにコピーして、手元で開いて、前回の結果と見比べて、次に何を変えるかを決める、などの一連の作業をしているといつ何をしたかぐちゃぐちゃになり散らかします（散らかしてます）。プロファイルが実験管理と紐づいていないと、それがどの条件で取ったものか、自分が何を試していたのかすら後から分からなくなります。AI 様のバフを得てプロファイリングする場合はさらに AI 様が勝手に謎ファイルを知らぬ間に量産していて、もはや美しいほど散らかっています。
...
どちらも小さなライブラリで、機能を増やすより先に一連の流れが通ることを優先しました。空のアカウントから新しいクラスタを立てて、基盤の導入からプロファイルの取得、MCP 越しの分析、完全撤去までを通しで確認しています。
...
run の記録には Amazon SageMaker のマネージド MLflow を使いました。ARN を渡すだけで、producer 側も分析側も追加のインフラなしに実験を記録・解決できます。MLflow サーバの永続化やバックアップを自分で面倒みる必要がなく、認証は SigV4 に寄せられます。`accelprof` の `[sagemaker]` extra を入れると、`MLFLOW_TRACKING_URI` に ARN を渡すだけでこの経路に乗ります。

マネージド MLflow には 2 つの形があり、基盤はどちらも選べるようにしました。インスタンスサイズを指定する MLflow Tracking Server と、指定しない Serverless の MLflow App です。導入時に `MLFLOW_BACKEND` で選び、あとから変更はできません。既定は App にしました。App には起動と停止の概念が無く、置いておくこと自体の課金要素がバケット以外に無いためです。裏返すと、しばらく使わないときに課金を止める「一時停止」ができるのは Tracking Server の側だけで、こちらは `aws sagemaker stop-mlflow-tracking-server` で止められます。長く空ける前提なら Server、放置する前提なら App という分かれ方です。
...
分析側は MCP クライアント (Claude Code など) から接続します。サーバはクラスタの中にいるので、手元からはポート転送越しに繋ぎます。そのポートを保持し、クライアントに渡す設定を組み、終わったら閉じるところまでを別のプラグインが引き受けます。対象はクラスタに登録した MCP すべてで、プロファイリング専用ではありません。
...
「テンソルコアが遊んでいて occupancy が低い」で引けば `gpu/tensor-cores-and-occupancy` が最上位に来ます。分析 MCP が返した事実と、知識 MCP が返した「次の一手」を突き合わせて次の実験のパラメータを決めます。この一往復を短くするのが、この基盤の目的です。

## まとめ

今回はぐちゃぐちゃになり散らかしていたローカルのディレクトリを見て発狂したことをきっかけにプロファイルの仕組みを作ってみました。新しいクラスタを立て直して、基盤の導入から GPU プロファイルの取得、MCP 越しの分析、データ層の完全撤去までを通しで確認したので、ここまでは動きます。導入は 1 コマンドで 16 分、`nsys` を含まない PyTorch イメージにプロファイラを注入して取った `.nsys-rep` から、支配的なカーネルとその平均時間がテキストで返るところまで繋がりました。S3 Files とマネージド MLflow は採用の必然性は別にありません、好きなものを使えば良いと思います。