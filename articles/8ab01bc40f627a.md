...
マネージド MLflow には 2 つの形があり、基盤はどちらも選べるようにしました。インスタンスサイズを指定する MLflow Tracking Server と、指定しない Serverless の MLflow App です。導入時に `MLFLOW_BACKEND` で選び、あとから変更はできません。既定は App にしました。App には起動と停止の概念が無く、置いておくこと自体の課金要素がバケット以外に無いためです。
