...
空いているポートを取って転送を張り、そのまま登録用のコマンドを出します。

```text
==> MCP servers are reachable from this machine
    analysis     http://127.0.0.1:60105/mcp
    knowledge    http://127.0.0.1:60126/mcp

    register them with a client that keeps a session open:
      claude mcp add --transport http analysis http://127.0.0.1:60105/mcp
      claude mcp add --transport http knowledge http://127.0.0.1:60126/mcp

    close them with:  kubectl distai-mcp down
```

ポートを固定したい場合は `--local-port analysis=18000` のように指定します。指定したポートが使用中なら別のポートへ黙って移らずに失敗するので、登録済みの URL と実際の転送先がずれることはありません。1 回だけコマンドを走らせたいなら `kubectl distai-mcp exec -- <コマンド>` で、転送を張ってから実行し、終了時に閉じます。
