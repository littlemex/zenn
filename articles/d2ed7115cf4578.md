...
:::message
この実装は v0.14.0 で導入された既存機能です。v0.16.0 の新機能については後述します。
:::
...
### Pipeline Parallel とは（既存技術）

:::message
Pipeline Parallel 自体は既存の並列化手法です。v0.16.0 では PP との併用が可能になりました。
:::
...
v0.16.0 の実装における 3 つの主要な変更は以下の通りです。
...
サンプリング後、即座にトークン ID をブロードキャストし、非最終ランクでは `self.input_batch.prev_sampled_token_ids` テンソルを受信します。
...
:::message
NVLink 4.0 の帯域幅 450GB/s（片方向）は H100/H200 構成における NVIDIA 公式スペックの 900GB/s（双方向）を基に算出しています。詳細は [NVIDIA NVLink 公式ページ](https://www.nvidia.com/en-us/data-center/nvlink/)を参照してください。
:::
...
前述の GPU 直接通信により CPU 往復が不要になったことによるレイテンシ削減が最大の要因です。PCIe Gen4 x16 では往復で数十マイクロ秒のオーバーヘッドが発生しますが、NVLink 4.0 ではこれが数マイクロ秒以下に削減されます。デコードフェーズでは 1 トークンごとにこの通信が発生するため、累積的な効果は大きくなります。
...
- **既存技術**: 非同期スケジューリング（v0.14.0 から）、Pipeline Parallel（既存の並列化手法）
- **v0.16.0 の新機能**: GPU 間直接通信によるトークン ID ブロードキャスト
- **性能改善**: 上記ベンチマーク結果の通り、大幅なスループット向上を実現
:::
