...
3. Disaggregated Inference 周りは新機能も試さなきゃ。。にゃーん。。

## はじめに

:::message
**記事の目的**: 本記事では、vLLM v0.16.0 の主要アップデートを解説します。特に私の興味範囲である、非同期スケジューリングと Pipeline Parallel の統合による性能向上、NixlConnector V2 による Large Scale Serving の改善に焦点を当てます。リリースノートを見ながら vLLM 実装を学ぶスタイルです！
:::
...
第一に、**CPU がブロードキャスト処理から解放される**ため、GPU 実行中に次のバッチをスケジュールできます。これにより、バッファリング付き Producer/Consumer パターンが依存関係なく実現されます。CPU（Producer）がバッチキュー（バッファ）にスケジュールし、GPU（Consumer）がキューから取り出して実行するという、完全に独立した並行動作が可能になります。v0.14.0 では CPU がブロードキャスト処理でブロックされていたため、独立した Producer/Consumer にはなっていませんでしたが、v0.16.0 でようやく実現されました。
...
### 転送ディスクリプタ増大の原因: PagedAttention の設計

:::message
ここまでで PagedAttention が転送ディスクリプタ増大の原因かもしれない、と思った方も多いと思いますが正解です！私もそう思って整理してみました。
:::

Cross-Layer KV Cache Layout は、vLLM の PagedAttention アーキテクチャの上に構築された最適化です。転送ディスクリプタが増大する原因は、PagedAttention の「レイヤーごとに独立したテンソルを確保する」という設計にあります。以下では、この問題がなぜ発生するのか、そして V2 がどのように既存実装と互換性を保ちながら効率を改善したかを説明します。
...
:::details permute() がデータコピーなしで次元を並び替える仕組み

PyTorch のテンソルは、実データ（GPU メモリ上の連続バイト列）と**メタデータ**（shape, stride, offset）の組み合わせで表現されます。`permute()` はメタデータのみを変更し、実データには一切触れません。

**例: (num_layers, num_blocks, ...) → (num_blocks, num_layers, ...) の並び替え**

```python
# 元のテンソル: shape=(32, 1000, 16, 8, 128)
original = torch.empty((32, 1000, 16, 8, 128))

# permute() で次元 0 と 1 を入れ替え
permuted = original.permute(1, 0, 2, 3, 4)  # shape=(1000, 32, 16, 8, 128)
```

**内部で何が起きているか**

| 操作 | shape | stride | 実データ |
|------|-------|--------|---------|
| original | (32, 1000, 16, 8, 128) | (163840000, 163840, 10240, 1280, 10) | GPU メモリ 0x7f8a2000... |
| permuted | (1000, 32, 16, 8, 128) | (163840, 163840000, 10240, 1280, 10) | **同じアドレス 0x7f8a2000...** |

stride（ストライド）は「次の要素に進むためにスキップするバイト数」を表します。`permute()` は stride の順序を入れ替えるだけで、実データは全く移動しません。

**FlashAttention カーネルが stride-aware な理由**

CUDA カーネルは、テンソルにアクセスする際に stride 情報を受け取り、それに従ってメモリアドレスを計算します。

FlashAttention/FlashInfer の CUDA カーネルはこの stride を正しく処理するため、permute された非連続テンソルでも正しく動作し、性能劣化もありません。
:::
...
**Mooncake コネクタの接続管理改善**（[PR #31034](https://github.com/vllm-project/vllm/pull/31034)）: Prefill/Decode Disaggregated 構成で、P ノードと D ノードが KV キャッシュを転送する際の接続確立が遅く、TTFT を悪化させていました。Prefill 側にブートストラップサーバー（接続情報の中央レジストリ）を導入し、Decode ノードがサーバーに問い合わせるだけで接続先を発見できるようにしました。これにより TTFT が 6ms 程度改善され、P と D が並行して処理を開始できるようになりました。

**MoE モデルの EPLB バグ修正**（[PR #33013](https://github.com/vllm-project/vllm/pull/33013)）: MoE モデルで Expert-Level Pipeline Load Balancing を有効にすると、VERL などの強化学習フレームワークが必要とする「ルーターリプレイ」（推論時のルーティング決定を訓練時に再現する機能）が正しく動作しませんでした。キャプチャのタイミングとバインド方法を修正し、論理エキスパート ID が正しく記録されるようにしました。これにより、MoE モデルの推論と訓練を連携させるワークフローが正常に機能するようになりました。

**運用監視メトリクスの追加**: KV キャッシュオフロード（GPU ↔ CPU 転送）のパフォーマンスを Prometheus 経由で監視できる KV オフロードコネクタメトリクス（[PR #27942](https://github.com/vllm-project/vllm/pull/27942)）が追加されました。また、P/D Disaggregated 構成では Decode インスタンスのプロンプトトークンの大部分が Prefill から転送されたものであり、実際の計算負荷と乖離していました。プロンプトトークンを出所別（ローカル計算/KV 転送/キャッシュヒット）にラベル付けするメトリクス（[PR #33290](https://github.com/vllm-project/vllm/pull/33290)）により、正確な負荷把握が可能になりました。
...
**Unified Parallel Drafting**（[PR #32887](https://github.com/vllm-project/vllm/pull/32887)）により、AMD の PARD と AWS の P-EAGLE が統一フレームワークに統合されました。PR の報告によると、GPT-OSS 120B で K=7 の P-EAGLE が約 560 tok/s（ベースライン比 1.52 倍）、Llama 3.3 70B-NVFP4 で PARD が約 254 tok/s（ベースライン比 3.10 倍）を達成しています。
...
Disaggregated Inference では、NixlConnector V2 の Cross-Layer Layout が RDMA 転送効率を改善し、TTFT の大幅な高速化とスループット向上を実現しました。
