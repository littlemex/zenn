...
## メモ（ここは筆者のメモ欄です）
 ...
 疑問 1: 非同期スケジューリングにおける「先読みスケジューリング」の正確性担保                                                                       
                                                                                                                                                     
  詳細: step_with_batch_queue() では、GPU がバッチ N を実行中に CPU がバッチ N+1 をスケジューリングします。しかし、バッチ N                          
  の実行結果（どのリクエストが完了したか、KV キャッシュがどれだけ消費されたか）はバッチ N+1                                                          
  のスケジューリング時点ではまだ確定していません。AsyncScheduler の num_output_placeholders
  はトークン位置のプレースホルダーを管理していますが、例えば「バッチ N でリクエスト A が EOS を生成して完了する」というケースでは、バッチ N+1
  はリクエスト A に対してもトークンをスケジュールしてしまう可能性があります。

  この「投機的にスケジュールしたが実際には不要だった」場合のロールバック処理はどのように実装されているのでしょうか。KV
  キャッシュの割り当て済みブロックの回収、スケジューラー内部状態の巻き戻し、そして不要なトークンの破棄処理が正しく行われないと、メモリリークやリクエ
  ストのハングアップにつながります。

  なぜこの疑問が重要か: 非同期スケジューリングの「楽観的実行」は性能向上の鍵ですが、その正確性保証はシステムの信頼性に直結します。特に KV
  キャッシュが逼迫した状況で投機的な割り当てが失敗した場合のフォールバック動作を理解することで、本番環境での batch_queue_size チューニングや OOM
  リスクの評価が可能になります。
...
:::message
**記事の目的**: 本記事では、vLLM v0.16.0 の主要アップデートを解説します。特に私の興味範囲である、非同期スケジューリングと Pipeline Parallel の統合による性能向上、NixlConnector V2 による Large Scale Serving の改善、torch.compile の強化に焦点を当てます。
:::
...
v0.16.0 では、v0.14.0 で導入された非同期スケジューリングが Pipeline Parallel（PP）と併用可能になりました（[PR #32618](https://github.com/vllm-project/vllm/pull/32618)）。これにより、大規模モデルを複数 GPU に分割（モデルのレイヤーを異なる GPU に配置）しながら、非同期処理の恩恵を受けられるようになり、**End-to-End（E2E）スループットが 30.8% 向上、TPOT（Time Per Output Token）が 31.8% 改善**しました。

### 非同期スケジューリングとは

vLLM の推論パイプラインは、以下の 2 つの主要なフェーズで構成されます。

第一に、**スケジューリングフェーズ（CPU）**では、リクエストのバッチ構築、KV キャッシュ割り当て、実行順序決定を行います。次に、**実行フェーズ（GPU）**では、モデル推論の実際の計算を実行します。
...
v0.16.0 の非同期パイプラインは、**バッチキュー**と **Future ベースの非同期実行**により実現されています。実装の核心は `vllm/v1/engine/core.py` の `EngineCore` クラスにあります。
...
バッチキューがない場合、`step()` メソッドが使用されます。このメソッドは、`execute_model(non_block=True)` で GPU 実行を非同期開始しますが、すぐに `future.result()` でブロックするため、実質的には同期実行です。

https://github.com/vllm-project/vllm/blob/v0.16.0/vllm/v1/engine/core.py#L389-L422

:::message
以下のコードは説明のため簡略化しています。実装の詳細は上記 GitHub URL を参照してください。
:::
...
def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
    ...
    # 2. GPU 実行を開始（non_block=True で Future 返却）
    future = self.model_executor.execute_model(scheduler_output, non_block=True)

    # 3. 文法制約の取得
    grammar_output = self.scheduler.get_grammar_bitmask(scheduler_output)

    # 4. GPU 実行完了を待つ（ブロッキング）
...
```mermaid
flowchart TB
    Start[step_with_batch_queue 開始]

    CheckReq{スケジューラーに<br/>リクエストあり?}
    Schedule[新しいバッチをスケジュール]
    ExecAsync[execute_model<br/>non_block=True]
    SampleAsync[sample_tokens<br/>non_block=True]
    AddQueue[バッチキューに追加]

    CheckFull{キューが満杯?<br/>または<br/>GPU 実行完了済み?}
    ReturnNone[None を返す<br/>即座にリターン]

    PopQueue[キューから最も古い<br/>バッチを取り出す]
    WaitResult[future.result<br/>GPU 完了を待つ]
    UpdateSched[スケジューラーを更新]
    ReturnOutput[結果を返す]

    Start --> CheckReq
    CheckReq -->|Yes| Schedule
    CheckReq -->|No| PopQueue
    Schedule --> ExecAsync
    ExecAsync --> SampleAsync
    SampleAsync --> AddQueue
    AddQueue --> CheckFull
    CheckFull -->|キューに空きあり| ReturnNone
    CheckFull -->|キューが満杯| PopQueue
    PopQueue --> WaitResult
    WaitResult --> UpdateSched
    UpdateSched --> ReturnOutput

    style Start fill: #e1f5ff
    style ReturnNone fill: #ccffcc
    style ReturnOutput fill: #ccffcc
    style WaitResult fill: #ffcccc
```
...
if (
    ...
    # キューが満杯でない & まだ完了していない場合
    # → 次のステップでさらにスケジュール可能
    return None, True  # ← ここで即座にリターン！
```
...
```python
# ステップ 3: キューから最も古いバッチを取り出して完了を待つ
future, scheduler_output, exec_model_fut = batch_queue.pop()
model_output = future.result()  # ← ここでブロッキング
...
キューが満杯になった場合のみ、最も古いバッチの完了を待ちます。これにより、以下のパイプライン効果が得られます：

```
時刻 t0: CPU がバッチ 1 をスケジュール → GPU がバッチ 1 を実行開始
時刻 t1: CPU がバッチ 2 をスケジュール → GPU はバッチ 1 実行中
時刻 t2: GPU がバッチ 1 完了 → 即座にバッチ 2 を開始（バブルなし）
時刻 t3: CPU がバッチ 3 をスケジュール → GPU はバッチ 2 実行中
```
...
class AsyncScheduler(Scheduler):
    def _update_after_schedule(self, scheduler_output: SchedulerOutput) -> None:
        ...
        for req_id in scheduler_output.num_scheduled_tokens:
            ...
            # 出力プレースホルダーを追加
            # → GPU 実行完了前に次のスケジューリングを進められる
            cur_num_spec_tokens = len(spec_decode_tokens.get(req_id, ()))
            request.num_output_placeholders += 1 + cur_num_spec_tokens
            request.spec_token_ids = self._spec_token_placeholders
```

`num_output_placeholders` により、GPU 実行中にまだ返されていないトークンのプレースホルダー数を管理します。これにより、GPU 実行完了前に次のスケジューリングが可能になります。

#### Executor の非同期実行サポート

`Executor` クラスは、`non_block` パラメータにより非同期実行をサポートします。

https://github.com/vllm-project/vllm/blob/v0.16.0/vllm/v1/executor/abstract.py#L202-L208
...
`non_block=True` の場合、`Future` を返却し、CPU は GPU 完了を待たずに次の処理を続行できます。
...
PP では、最終段の GPU（GPU 3）のみがサンプリング（次トークン選択）を実行します。選択されたトークン ID は、次の forward pass 開始前に全 GPU へブロードキャストされる必要があります。

[PR #32618](https://github.com/vllm-project/vllm/pull/32618) で解決された主な問題は以下の通りです。第一に、トークン ID が GPU → CPU → GPU の経路でブロードキャストされており、非同期実行のボトルネックになっていました。次に、CPU がスケジューリングとブロードキャストを同時に処理する際の同期制約がありました。最後に、バッチキューで複数バッチが並行実行される中での、トークン ID ブロードキャストの管理が複雑でした。

v0.16.0 では、GPU 間直接通信によりこれらの問題が解決され、PP と非同期スケジューリングの併用が可能になりました。
...
::::details v0.14.0 の CPU 経由通信 vs v0.16.0 の GPU 直接通信

**v0.14.0 の CPU 経由の通信パターン**
...
v0.16.0 の実装では、以下の 3 つの主要な変更により、PP と非同期スケジューリングの併用を実現しています。

##### トークン ID の直接ブロードキャスト

最終ランク（GPU 3）がサンプリングしたトークン ID テンソルを `torch.distributed.broadcast` で直接ブロードキャストします（[PR #32618](https://github.com/vllm-project/vllm/pull/32618) で実装）。

サンプリング後、即座にトークン ID をブロードキャストし、非最終ランクでは `self.input_batch.prev_sampled_token_ids` テンソルを受信します。これにより、CPU を経由せずに GPU 間で直接トークン ID を共有できます。

##### リクエストインデックスマッピングの再構築

受信したトークン ID を正しいリクエストに関連付けるため、`prev_req_id_to_index` マッピングを再構築します（[PR #32618](https://github.com/vllm-project/vllm/pull/32618) で実装）。

このマッピングは、トークン ID の受信と同じタイミングで実行されるため、同期オーバーヘッドが最小化されます。各 GPU は自分の担当するリクエストのみを処理するため、マッピング再構築は軽量です。

##### キャッシング機構とバッチキューの統合

非最終ランクでは、受信したトークン ID をキャッシュし、次のイテレーションで再利用可能にします（[PR #32618](https://github.com/vllm-project/vllm/pull/32618) で実装）。

さらに重要なのは、このキャッシング機構がバッチキューと統合されている点です。バッチキューに複数のバッチが存在する場合でも、各バッチのトークン ID が正しく管理されます。これは、各バッチに対して独立した Future を管理することで実現されています（前述の `step_with_batch_queue()` 実装を参照）。
...
| 帯域幅 | PCIe Gen4 x16: 約 32GB/s（片方向） | NVLink 4.0: 約 450GB/s（片方向） |
| レイテンシ | 高い（CPU 往復） | 低い（GPU 間直接） |
| CPU 負荷 | 高い | 低い |
| 非同期スケジューリング | 困難（同期必要） | 可能（完全非同期） |

:::message
NVLink 4.0 の帯域幅 450GB/s（片方向）は、NVIDIA 公式スペックの 900GB/s（双方向）を基に算出しています。詳細は [NVIDIA NVLink 公式ページ](https://www.nvidia.com/en-us/data-center/nvlink/)を参照してください。
:::

トークン ID は数バイト～数 KB 程度の極めて小さなデータであるため、帯域幅の差はスループット向上の主要因ではありません。**30.8% のスループット向上**は主に以下の要因によるものです。

##### レイテンシ削減

CPU 往復（GPU → PCIe → CPU → PCIe → GPU）が不要になったことによるレイテンシ削減が最大の要因です。PCIe Gen4 x16 の理論レイテンシは 200ns ~ 1μs 程度ですが、実際には OS のオーバーヘッド、DMA 転送の準備、CPU でのデータ処理などにより、往復で数十マイクロ秒のオーバーヘッドが発生します。

NVLink 4.0 を使用した GPU 間直接通信では、このオーバーヘッドが数マイクロ秒以下に削減されます。デコードフェーズでは 1 トークンごとにこの通信が発生するため、累積的な効果は大きくなります。

##### CPU 負荷の軽減

CPU がブロードキャスト処理から解放され、スケジューリングに専念できるようになりました。特に非同期スケジューリングでは、CPU は次のバッチのスケジューリングを実行する必要があるため、この解放は重要です。

CPU のスケジューリング処理には、KV キャッシュ割り当て、リクエストキュー管理、トークンバジェット計算などが含まれます。これらの処理を GPU 実行と並行して実行できることが、スループット向上の鍵となっています。

##### 非同期スケジューリングとの整合性

GPU 側でトークン ID ブロードキャストが完結することで、CPU のスケジューリング処理との競合が解消され、デッドロックやレース条件が回避されます。

具体的には、以下のシーケンスが可能になります：

```
時刻 t0: CPU がバッチ 1 をスケジュール
時刻 t1: GPU がバッチ 1 を実行開始
時刻 t2: CPU がバッチ 2 をスケジュール（並行）
時刻 t3: GPU がバッチ 1 のトークン ID をブロードキャスト（GPU 間のみ）
時刻 t4: GPU がバッチ 2 を実行開始（即座）
時刻 t5: CPU がバッチ 3 をスケジュール（並行）
```

このように、CPU と GPU が完全に独立して動作できることが、高いスループットを実現する基盤となっています。

**ベンチマーク結果**

以下の数値は [PR #32618](https://github.com/vllm-project/vllm/pull/32618) で報告されたベンチマーク結果です。

テスト環境: Qwen/Qwen3-30B-A3B-Thinking-2507-FP8、PP=4、最大同時シーケンス 128、入力 2 トークン、出力 512 トークン

| 指標 | v0.16.0 | ベースライン | 改善率 |
|------|---------|--------|--------|
| リクエストスループット | 11.72 req/s | 8.95 req/s | **+30.8%** |
...
第一に、KV Cache Spec が均一モデル（非 HMA）である必要があります。次に、Connector が `prefer_cross_layer_blocks = True` を返す必要があります。最後に、Attention Backend が `get_kv_cache_stride_order()` をサポートしている必要があります。
...
56 Layer のモデルで Block[42] を転送する場合、以下のメモリ領域がすべて非連続に配置されています：
...
RDMA/libfabric 経由でこれらを転送するには、各メモリセグメントに対して個別の**転送ディスクリプタ**（送信元アドレス、送信先アドレス、サイズを含むメタデータ構造）を作成する必要があります。56 Layer x 2（Key/Value）= **112 個の転送ディスクリプタ**が 1 ブロックあたり必要になります。
...
| メトリクス | V1 | V2 | 改善率 |
|-----------|--------|-----|--------|
| TTFT (Time to First Token) | 18,756ms | 8,494ms | **2.2 倍高速** |
...
密モデル向けには MXFP8 量子化（[PR #33786](https://github.com/vllm-project/vllm/pull/33786)）が追加されました。MXFP8 はブロック単位でスケーリングファクターを適用する方式であり、従来のテンソルレベル FP8 量子化と比較して精度劣化の軽減が期待されます。
