---
title: "AWS SageMaker HyperPod の Checkpointless/Elastic Training 徹底解説"
emoji: "🔥"
type: "tech"
topics: ["AWS", "SageMaker", "HyperPod", "Kubernetes", "機械学習"]
published: false
---
...
合計復旧時間: 10-20 分程度           合計: 65-130 分程度
```

### 機能の組み合わせパターン

[注意] 以下は各機能の復旧時間の概算です。実際の復旧時間は環境やモデルサイズによって変動します。「HMA のみ」の場合、チェックポイントをローカルストレージ（FSx）から復元する場合は数十分、S3 から復元する場合は 65-130 分程度（フロー図参照）となります。

| 機能の組み合わせ | 復旧時間（推定） | 用途 |
|----------------|-----------------|------|
| HMA のみ | 数十分～数時間（ストレージによる） | 基本的な自動復旧 |
| HMA + Checkpointless | 数分～20 分 | 高速復旧が必要な本番環境 |
| HMA + Managed Tiered Checkpointing | 数十分～数時間（Tier 1 にキャッシュがある場合は数十分） | 長期訓練でコスト最適化 |
...
4. **訓練時間の削減**: Checkpointless Training を含む HyperPod のレジリエンシー機能全体で最大 40% の訓練時間削減（AWS 公式情報）
...
sequenceDiagram
    ...
    Note over W1, BE: [設定] min_nodes=2, max_nodes=4, last_call_timeout=30s

    rect rgb(230, 245, 255)
        Note over W1, DRH: Worker A 起動
        W1 ->> DRH: next_rendezvous()
        DRH ->> DRH: _stop_heartbeats()
        Note right of DRH: ランダム遅延 (0〜0.3 秒)<br/>バックエンド負荷分散

        DRH ->> Exec: run(exit_op, deadline)
        Exec ->> SH: sync()
        SH ->> BE: get_state()
        BE -->> SH: state (空)
        Note right of Exec: 前ラウンドに未参加<br/>→ 即 FINISH

        DRH ->> Exec: run(join_op, deadline)
        Exec ->> SH: sync()
        SH ->> BE: get_state()
        BE -->> SH: state
        SH ->> SH: _sanitize() 死亡ノード除去
        Exec ->> Exec: _RendezvousJoinOp()
        Note right of Exec: participants=0 < min_nodes<br/>→ ADD_TO_PARTICIPANTS
        Exec ->> SH: _add_to_participants(A)
        Exec ->> SH: mark_dirty()
        Exec ->> SH: sync()
        SH ->> BE: set_state(participants={A})
        BE -->> SH: OK
        Note right of Exec: participants=1 < min_nodes=2<br/>→ SYNC (待機)
    end

    rect rgb(230, 255, 230)
        Note over W2, DRH: Worker B 起動
        W2 ->> DRH: next_rendezvous()
        DRH ->> Exec: run(exit_op, deadline)
        Note right of Exec: 前ラウンドに未参加 → FINISH
        ...
        Exec ->> Exec: _RendezvousJoinOp()
        Note right of Exec: participants=1 < min_nodes=2<br/>→ ADD_TO_PARTICIPANTS
    ...
    rect rgb(255, 255, 220)
        Note over W1, BE: min_nodes=2 到達 → last_call 期間開始 (deadline 設定)
        Exec ->> Exec: deadline = now + last_call_timeout (30s)
        Note right of Exec: 追加ワーカーを待機中...<br/>deadline まで参加可能
    end

    rect rgb(230, 255, 230)
        Note over W3, DRH: Worker C 起動 (last_call 期間中)
        ...
        Exec ->> Exec: _RendezvousJoinOp()
        Note right of Exec: complete=False かつ<br/>participants < max_nodes<br/>→ ADD_TO_PARTICIPANTS
    ...
    rect rgb(255, 230, 230)
        Note over W1, BE: deadline 到達 → Rendezvous 完了
        Exec ->> Exec: _mark_rendezvous_complete()
        Note right of Exec: complete=True<br/>rank をソート順で割り当て<br/>A→0, B→1, C→2
        Exec ->> SH: mark_dirty()
        Exec ->> SH: sync()
        SH ->> BE: set_state(complete=True, ranks 割り当て済み)
        BE -->> SH: OK
        Exec -->> DRH: FINISH
    end

    rect rgb(240, 230, 255)
        Note over W1, BE: 訓練開始準備
        DRH ->> DRH: _start_heartbeats()<br/>keep_alive_interval=5s
        DRH ->> DRH: _get_world() → (rank, world_size)
        DRH ->> DRH: _get_store() → PrefixStore
        ...
        Note over W1, W3: 分散訓練開始 (world_size=3)
    end
```
...
sequenceDiagram
    ...
    participant W_new as Worker D<br/>(新規)
    participant DRH as DynamicRendezvous<br/>Handler
    participant Exec as OpExecutor
    participant SH as StateHolder
    participant BE as RendezvousBackend<br/>(etcd / C10d)

    Note over W1, BE: [現在の状態] round=0, complete=True<br/>participants={A: rank0, B: rank1}, max_nodes=4

    rect rgb(230, 245, 255)
        Note over W1, W2: 訓練実行中...
        W1 ->> W1: 訓練ステップ実行
        W2 ->> W2: 訓練ステップ実行
    end

    rect rgb(255, 245, 220)
        Note over W_new, DRH: Worker D が起動・参加を試みる
        ...
        Exec ->> Exec: _RendezvousJoinOp()
        Note right of Exec: complete=True かつ<br/>participants < max_nodes<br/>→ ADD_TO_WAIT_LIST
        Exec ->> SH: _add_to_wait_list(D)
        Exec ->> SH: mark_dirty()
        Exec ->> SH: sync()
        SH ->> BE: set_state(wait_list={D})
        BE -->> SH: OK
        Note right of Exec: 現在のラウンドが終了するまで<br/>SYNC で待機ループ
    end

    rect rgb(255, 230, 230)
        Note over W1, BE: 既存ワーカーがチェックポイント保存後に Re-rendezvous を開始
        W1 ->> W1: チェックポイント保存
        W2 ->> W2: チェックポイント保存
        ...
        Note right of Exec: 自分が participants に存在<br/>→ REMOVE_FROM_PARTICIPANTS
        Exec ->> SH: participants から A を除去
        Exec ->> SH: mark_dirty()
        Exec ->> SH: sync()
        ...
        Exec ->> SH: sync()
        Exec ->> Exec: _RendezvousExitOp()
        Exec ->> SH: participants から B を除去
        Exec ->> SH: mark_dirty()
        Exec ->> SH: sync()
        SH ->> BE: set_state(participants={})
        BE -->> SH: OK
        Note right of Exec: participants が空<br/>→ round++, complete=False<br/>wait_list={D} が次ラウンドに参加可能
    end

    rect rgb(230, 255, 230)
        Note over W1, BE: 新しいラウンド (round=1) で全ワーカーが参加

        W1 ->> DRH: run(join_op, deadline)
        W2 ->> DRH: run(join_op, deadline)
        W_new ->> DRH: run(join_op, deadline)
        Note right of DRH: SYNC ループから抜けた D も参加
        ...
        Note over W1, BE: min_nodes 到達 → last_call → 完了
        Exec ->> Exec: _mark_rendezvous_complete()
        Note right of Exec: complete=True<br/>A→rank0, B→rank1, D→rank2
    end

    rect rgb(240, 230, 255)
        Note over W1, BE: 新しい world_size=3 で訓練再開
        ...
        Note over W1, W_new: チェックポイントから訓練再開<br/>world_size: 2→3
    end
```
...
sequenceDiagram
    ...
    Note over W1, BE: [現在の状態] round=0, complete=True<br/>participants={A:0, B:1, C:2}

    rect rgb(230, 245, 255)
        Note over W1, Timer: 正常稼働中: ハートビート送信
        loop keep_alive_interval = 5 秒ごと
            Timer ->> DRH: _keep_alive() コールバック
            DRH ->> SH: sync()
            SH ->> BE: get_state()
            BE -->> SH: state
            ...
            DRH ->> SH: mark_dirty()
            DRH ->> SH: sync()
            SH ->> BE: set_state(heartbeat 更新済み)
            BE -->> SH: OK
        end
    end

    rect rgb(255, 200, 200)
        Note over W2: [障害発生] Worker B がクラッシュ
        W2 ->> W2: プロセス異常終了
        Note right of W2: PeriodicTimer も停止<br/>ハートビート送信が途絶える
    end

    rect rgb(255, 245, 220)
        Note over W1, BE: ハートビート期限切れの検出
        Note right of Timer: 検出条件: <br/>expire_time = now - (5s x 3 回) = 15 秒
        Timer ->> DRH: _keep_alive() (Worker A の定期実行)
        DRH ->> SH: sync()
        SH ->> BE: get_state()
        BE -->> SH: state
        SH ->> SH: _sanitize()
        Note right of SH: B の last_heartbeat < expire_time<br/>→ B を死亡ノードと判定
        SH ->> SH: participants から B を削除
        SH ->> SH: last_heartbeats から B を削除
        SH ->> SH: _remove_participant_epilogue()
        Note right of SH: complete=True かつ<br/>participants={A,C} が残存<br/>→ complete のまま維持
        SH ->> BE: set_state(participants={A,C})
        BE -->> SH: OK
    end

    rect rgb(255, 230, 230)
        Note over W1, BE: Agent が障害を検出 → Re-rendezvous 開始
        W1 ->> W1: NCCL エラー / 通信障害を検出
        W3 ->> W3: NCCL エラー / 通信障害を検出
        ...
        DRH ->> DRH: run(exit_op)
        Note right of DRH: 自分が participants に存在<br/>→ REMOVE_FROM_PARTICIPANTS
        DRH ->> SH: sync()
        ...
        SH ->> BE: set_state(participants={})
        BE -->> SH: OK
        Note right of SH: participants が空<br/>→ round=1, complete=False
    end

    rect rgb(230, 255, 230)
        Note over W1, BE: 新ラウンド (round=1) で Re-rendezvous
        W1 ->> DRH: run(join_op)
        DRH ->> SH: sync()
        SH ->> BE: get_state()
        ...
        Note over W1, BE: min_nodes=2 到達 → last_call → 完了
        DRH ->> DRH: _mark_rendezvous_complete()
        Note right of DRH: complete=True<br/>A→rank0, C→rank1<br/>(rank 再割り当て)
    end

    rect rgb(240, 230, 255)
        Note over W1, BE: 縮小した world で訓練再開
        DRH ->> DRH: _start_heartbeats()
        ...
        W1 ->> W1: init_process_group()
        W3 ->> W3: init_process_group()
        W1 ->> W1: チェックポイントからモデル復元
        W3 ->> W3: チェックポイントからモデル復元
        Note over W1, W3: 訓練再開 (world_size: 3→2)<br/>Worker B は離脱済み
    end
```
...
sequenceDiagram
    ...
    rect rgb(230, 245, 255)
        Note over User, CLI: Step 1: CLI コマンド実行
    ...
    rect rgb(255, 245, 220)
        Note over CLI, Valid: Step 2: バリデーションと変換
        CLI ->> CLI: generate_click_command()<br/>schema.json (v1_1) からオプション動的生成
        CLI ->> Valid: PyTorchJobConfig(**params)
        Valid ->> Valid: 型チェック (integer >= 1 等)
        Valid ->> Valid: elastic_replica_increment_step と<br/>elastic_replica_discrete_values の<br/>相互排他チェック
        Note right of Valid: node_count=4 → replicas=4, minReplicas=4<br/>max_node_count=8 → maxReplicas=8

        Valid ->> Valid: to_domain() 変換
        Note right of Valid: Resources (GPU/CPU/Memory/EFA) 構築<br/>Container 構築<br/>NodeSelector (instance-type) 構築<br/>ReplicaSpec (replicas=4, maxReplicas=8)<br/>ElasticPolicy (min=4, max=8, step=2)<br/>RunPolicy 構築
    end

    rect rgb(230, 255, 230)
        Note over CLI, K8sAPI: Step 3: CRD 作成
        CLI ->> CLI: config.load_kube_config()
        CLI ->> CLI: allocate_quotas_if_applicable()<br/>インスタンスタイプからリソース自動計算
    ...
    rect rgb(255, 230, 230)
        Note over K8sAPI, Operator: Step 4: Operator が CRD を検知
        K8sAPI ->> Operator: Watch: HyperPodPyTorchJob 作成イベント
        Operator ->> Operator: CRD spec を解析
        Operator ->> Operator: ElasticPolicy を読み取り<br/>minReplicas=4, maxReplicas=8
        Operator ->> Operator: Rendezvous 設定を自動生成<br/>(masterAddr, masterPort)
    end

    rect rgb(240, 230, 255)
        Note over Operator, Pod: Step 5: Worker Pod 作成
        Operator ->> K8sAPI: Pod 作成 (my-job-pod-0 〜 my-job-pod-3)
        K8sAPI ->> Pod: 4 つの Worker Pod をスケジュール

        Operator ->> Operator: status.masterAddr = Pod-0 の IP<br/>status.masterPort = 割り当てポート
        Operator ->> K8sAPI: status 更新 (masterAddr, masterPort)

        Note over Pod: 各 Pod に環境変数を注入 (推測): <br/>MASTER_ADDR, MASTER_PORT,<br/>WORLD_SIZE, RANK, LOCAL_WORLD_SIZE 等

        Pod ->> Pod: pytorch-job-container 起動
        Pod ->> Pod: torchrun --nproc_per_node=8 ...
    end

    rect rgb(230, 245, 255)
        Note over Pod, RDZV: Step 6: Rendezvous と訓練開始
        ...
        Note over Pod: 分散訓練開始<br/>world_size=32 (4 nodes x 8 GPUs)
    end

    rect rgb(255, 245, 220)
        Note over Operator, Pod: [スケーリングイベント発生時]
        Operator ->> Operator: ElasticScalingStatus 更新<br/>targetReplicas 変更
        Operator ->> K8sAPI: Pod 追加/削除
        Note right of Operator: replicaIncrementStep=2 に従い<br/>2 ノード単位でスケーリング
        Operator ->> Operator: scalingTimeoutInSeconds=300 内に<br/>スケーリング完了を要求
        Pod ->> RDZV: Re-rendezvous
        RDZV -->> Pod: 新しい RendezvousInfo
        Pod ->> Pod: 新しい world_size で訓練再開
    end
```
...
# この実装例は Kubeflow の PyTorchJob を直接使用する方法です。
# HyperPod CLI (hyp) を使用する場合は HyperPodPyTorchJob CRD が自動生成されます。
apiVersion: "kubeflow.org/v1"
kind: PyTorchJob
metadata:
...
| **削減効果** | HyperPod 全体で訓練時間最大 40%、ストレージコスト削減 |
| **対応環境** | HyperPod EKS のみ |
...
| 手法 | 制御レベル | 復旧時間 | ストレージ |
|-----|-----------|---------|----------|
| **アプリ側 Checkpoint** | アプリケーション | 中（数分～数十分、ストレージによる） | 大（100GB+） |
