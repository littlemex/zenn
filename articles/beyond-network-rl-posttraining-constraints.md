...
ここまでと同じH200クラスタ・同じ実験群での実測です。ここが検証の核心的な結果です。KV fp8・学習率1e-5・TIS（重要度比にcapをかける対処法。後述の「対処その1」で説明します）なしの条件（collapse arm）で、25ステップにわたってmis_klとgrad_norm（勾配のノルム、勾配全体の大きさを表す指標）を追跡した実測データです。
...
| | bf16 KV | fp8_e5m2 KV |
|---|---|---|
| **LR 1e-6** | mis_kl ~0.0007, 14 step 安定 | mis_kl ~0.03, **14 step まで安定** [*] |
| **LR 1e-5** | mis_kl ~0.0006, 30 step 安定 | mis_kl 0.03→**31.78, 30 step で崩壊** |

[*] LR 1e-6 + fp8 の run は**14 stepで打ち切られています**。崩壊アーム (LR 1e-5 + fp8) の onset は step 8〜14 付近なので、「14 step で安定」は「崩壊が起きないことの証明」としては不十分です。14 step 時点で mis_kl が 0.028 と step 0 (0.032) から下がっている（上昇していない）ことは好材料ですが、30 step の検証は次回の Capacity Block で実施予定です。

現時点で言えるのは: LR 1e-5 + bf16 が 30 step 安定であること（mis_kl max 0.00063）から、**「崩壊には fp8 増幅が必要条件」は確定**。fp8 + LR 1e-6 が崩壊しないことは「14 step の範囲では発散の兆候がない」までです。
