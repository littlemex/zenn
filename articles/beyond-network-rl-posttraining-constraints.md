...
2. **weight syncの配置コスト**: トポロジを設計しないと見えません（H200・TP1のみで実測しました）
3. **training-inference mismatch** （数値のずれ）: lossやrewardには見えません。mis_klという専用の指標を計測しない限り無症状です（Hopper世代のEKSクラスタで実測、3 seed追試はp5 / H100。本記事の主役です）
...
:::message
この記事に出てくる数値は、実測（measured）・論文からの引用（from paper）・未計測（not measured）を厳密に区別して示します。見出しや文中で明示しない場合でも、グラフや表の注記を確認してください。

H200での実測値は、学習フレームワークが書いたTensorBoardのイベントファイルと、実験ドライバが書いた結果ファイルの両方に同じ値があることを照合ツールで確認しています。3-seed崩壊/救済実験とper-kernel帰属実験はH100 (p5.48xlarge) で実施しています。全runのハードウェア帰属は、CloudTrailのインスタンス生存区間とTensorBoardのwall clockを突き合わせて2026-08-05に機械的に確定しました。以前この注記に「一次記録間で食い違い」と書いていた件は解決済みです（参照先文書のラベル誤りであり、実データの矛盾ではありませんでした）。
:::
...
この5番目のステップが**weight sync** （重み同期）です。学習が進むたびに、trainerが更新した最新の重みをrollout側に届けないと、rolloutは古い方策のままサンプルを生成し続けてしまいます。このweight syncの矢印は、後ほど制約2（配置コスト）で重要な役割を果たすので覚えておいてください。
...
| 2. weight syncの配置コスト | トポロジを設計しないと見えない | H200で実測（TP1のみ） |
| 3. training-inference mismatch | lossやrewardには見えない（mis_klを計測しない限り無症状） | Hopperクラスタで実測、3 seed追試はH100（本記事の主役） |
...
弱い傍証としては、別プロジェクトの検証でCUDAグラフのキャプチャ時に16個のSGLangエンジンがCPUを95%使って競合する現象を観測したことがあります（ただしこれはウォームアップ時の一時的な現象で、フラグ設定で解消しました）。また別プロジェクトの検証データでは、train_wait（trainerが待機している時間の割合）が0.80〜0.93という高い比率を占めており、GPUが遊んでいる時間が長いことの傍証にはなりますが、その原因がCPU側にあるとまでは特定できていません。

## 制約2: weight syncの配置コスト — 設計しないと見えない
...
### weight syncの仕組み: なぜcolocated配置の方が遅いのか

先ほどのGRPOループに出てきたweight syncの矢印を、具体的に見ていきます。実は、ここに驚きが隠れています。

trainerが更新した重みをrolloutに送り返す方法は大きく2つあります。

**colocated** （同居配置）: rolloutとtrainerが同じGPUを共有し、CUDA IPC（GPU内のプロセス間でメモリを直接受け渡す仕組み）で重みを渡します。同じGPU上でのやり取りなので、直感的には速そうに見えます。しかし実際には、重みをchunk（分割したブロック）ごとに処理し、chunkごとに`ipc_collect()`という全GPU同期のガベージコレクション（メモリ回収処理）と、Ray（分散処理フレームワーク）のハンドル往復を呼び出します。さらに、weight syncのたびに`pause_generation`（生成の一時停止）と`flush_cache`（KVキャッシュの破棄）も呼びます。

**disaggregated** （分離配置）: rolloutとtrainerが別々のノードに置かれ、NCCL/EFA（ネットワーク経由の通信）で重みを渡します。全chunkを1つのバッファに集約してから、1回のバッチ処理でbroadcast（一斉配信）します。`pause_generation`や`flush_cache`はこちら側には存在しません。
...
flowchart TB
    subgraph Colo["colocated: CUDA IPC（同一GPU）"]
        ...
        CR --> CF["pause_generation +<br/>flush_cache"]
        CF --> CRes["定常: 0.482秒"]
    end
...
図の`pause_generation + flush_cache`のノードに注目してください。これがこの後の説明の核心です。**colocated側の0.482秒には、この処理の分だけ余分な仕事が含まれています。** つまりこの図だけを見て「IPCのchunk処理そのものが遅い」と結論づけるのは早計です。

### H200実測: 配置を変えたときのコスト差

H200（p5en）でmiles、Qwen3-4B、TP1（tensor parallel size 1）、mem-fraction（GPUメモリの静的確保率）を両方式で0.8に揃え、7周回して先頭3サンプルをウォームアップとして捨てた残り4サンプルの中央値です。

| 方式 | 実装 | 初回 | 定常（中央値） | 変動係数 |
|---|---|---|---|---|
| colocated | CUDA IPC (UpdateWeightFromTensor) | 0.798秒 | **0.482秒** | 0.012 |
| disaggregated | NCCL/EFA (UpdateWeightFromDistributed) | 14.414秒 | **0.170秒** | 0.013 |

まず理由を先に説明します。**2つの経路は同じ仕事をしていません。** colocated側だけが、weight syncのたびに`pause_generation`と`flush_cache`を呼びます（`update_weight_from_tensor.py:213-215`）。disaggregated側にはこの処理がありません。つまり0.482秒と0.170秒の差は「転送方式の差」ではなく「**配置を変えたときのweight sync全体のコスト差**」です。disaggregated側は転送本体だけを取り出せて0.146秒（全体の86パーセント）なのですが、colocated側には対応するタイマーが存在しないため同じ分解ができません。

この区別を踏まえたうえで、定常状態でdisaggregatedが**2.84倍速い**という結果です。ネットワーク経由の方が速いという方向は確かに実測できましたが、それは「転送方式が速い」ではなく「配置全体のコストが低い」という意味です。

もう一つ、初回コストが方式で大きく違うという事実も重要です。disaggregatedは定常では速いものの、初回にNCCLのプロセスグループ確立で14.4秒を払います。colocatedは0.8秒です。この差を定常の差（0.312秒）で回収するには約44ステップかかるので、**30ステップ程度の短い実験ではcolocatedの方が総和が小さくなります**。

そして、この数値がそもそも取れなかった理由も書いておく必要があります。miles のレシピは`--colocate`フラグをハードコードしていて、`COLOCATE`という環境変数はバナー表示にしか使われていませんでした。つまり`COLOCATE=false`と書いてもdisaggregatedにはならず、**この計測アームはハーネスから到達不能でした**。今回それを修正して初めて測れています。

:::message alert
この2.84倍という比率は、TP1・単一フレームワーク・単一クラスタでの1点です。他の並列構成やフレームワークに持ち込めるものではなく、この記事の他の場所でこの比率を一般化した記述があれば誤りです。
:::

なお「遅さの原因がIPCのセマンティクスとRayの往復コストにある」という説明のうち、chunkごとの`ipc_collect()`とRayのハンドル往復については、コードを読んで導いた仮説であり、プロファイラ（nsysなどの実行時解析ツール）で直接確認したものではありません。一方`pause_generation`と`flush_cache`が存在すること自体はコードにそう書かれている事実です。

いずれにしても、この制約の正体はネットワークそのものではなく、その配線の「上」にありました。

そして最後に、weight syncは1ステップ全体のうち1%前後に過ぎません。GRPOの1周の内訳は、rolloutの生成が数十秒、update_weightsが約0.5秒、actor_train（backwardとoptimizer更新）が20〜30秒程度です。実在する制約であり直感に反する結果ではありますが、支配的なコストではないという点は誠実に述べておきます。
...
![インフラ構成と、mismatchが計測される2つの場所](/images/4c8e1a2b6f-arch.png)
...
*重要度比の分布を示します。平均は1付近で無害に見えますが、裾の1トークンが勾配を支配します。TISとvetoはこの分布の別の場所に介入します（TIS/vetoの詳細は後述の「対処その1」「対処その2」で説明します）。*

#### 要因2: 応答が長いほど誤差が二乗で増える

論文（arXiv:2602.01826）が示す誤差の上界（Theorem 3.1）は、応答の長さ $T$ に対して次の式のように増大することを示しています。$O(\cdot)$ （オーダー記法）は「最悪の場合、括弧の中の量に比例するペースで増える」という増え方の上限を表す書き方で、$O(T^2)$ は「Tの2乗に比例するペースを超えない」という意味です。

$$
\text{誤差の上界} = O(T^2)
$$

これは直感的にも理解できます。トークンを1つ生成するたびに、rolloutとtrainerの間の微小なずれが積み重なっていきます。系列が長くなるほど、この積み重ねが最悪の場合には二乗のペースで拡大しうるのです。ここまでは論文の上界の話であり、私たち自身の実測ではありません。実際にどちらに近いかは、後述の「位置プロファイル」の節で直接測ります（結論を先に言うと、静穏な状態では上界には近づかず、崩壊が始まると話が変わります）。
...
*per-tokenのlogprobをダンプする計装の位置を示します。集約値だけでは系列内のどこで差が出たかが分かりません。*
...
ソースコードを確認した結果を書いておきます。`train/mis_kl`は**Schulman k3推定量**（$e^r - r - 1$, $r = \log(\pi_{\text{train}}/\pi_{\text{rollout}})$）であることが`mis.py`の`add_ppl_metrics`関数から確定しています。方向はKL($\pi_{\text{rollout}}$ || $\pi_{\text{train}}$)で、常に非負です。k1（符号付きで負になりうる）やk2（$r^2/2$で常に非負だがbiased）とは異なり、k3はunbiasedかつ非負という性質を持ちます。ただし2分布が大きく乖離するとexp(r)項で分散が爆発する点には注意が必要です。
...
**この実験はH100 (p5.48xlarge) x8で実施しています。** 前後の増幅実測値・キラーチャートはH200ですが、per-kernel帰属はH100です。H100/H200間でmis_klに有意差がないことはキャリブレーション実験（5.4%差、seed CVの7.8%以下）で確認済みです。

上の5箇所は「候補」でした。ではKVキャッシュをfp8にしたときの増幅（この節の実験では基準の0.000716に対して約45倍。後述する49倍・54倍は別のrunで基準値も違います）は、このうちどれがどれだけ効いているのでしょうか。各フラグを1つずつ切り替えて、学習なしでstep-0のmis_klだけを測る実験（per-kernel ablation）を実施しました。全6構成で学習率は1e-6に統一しており（envファイルで機械的に確認済み）、学習を回さない（step 0のみ）ので、学習率もoptimizerも値に影響しません。純粋な数値経路差だけを取り出しています。
...
**条件統一の証拠**: 全6構成のenvファイル（`gen_cells.py`で自動生成）を確認し、学習率は全arm `1e-6`、model/seed/batch/dropout/image が同一であること、差分はSGLangのruntimeフラグ（`--sglang-kv-cache-dtype` / `--sglang-attention-backend` / `--sglang-disable-cuda-graph`）のみであることを機械的に検証済みです。step 0のみの測定なのでoptimizer更新は走っておらず、重みは全armで初期値のままです。
:::
...
ここからは実測データです。単一seedの点推定である旨は後述します。

**ハードウェアの帰属 (2026-08-05 確定)**: この節のbaseline 0.000632は**H200 (p5en.48xlarge) での実測**です。7月クラスタのFSxに残るTensorBoardイベントファイルのwall clockとCloudTrailのインスタンス生存区間を突き合わせて機械的に確定しました (run `miles-smoke`, wall 2026-07-19 14:47 JST)。以前の版で「一次記録が食い違っている」と書いていたのは、参照先の文書のラベルが誤っていたためで、実データ自体の矛盾はありませんでした。

検証環境はH200 x8のクラスタです。Qwen3-4B dense（密なモデル、MoEではない構成）、colocated配置、dropout（学習時に一部ニューロンを無効化する正則化手法）は0です。学習率はベースラインが1e-6、fp8で増幅したrunは1e-5です。増幅側だけ学習率を上げているのは、30ステップという短い実行のなかで崩壊まで到達させるためです（1e-6のままだと崩壊が始まる前にキャパシティブロックが終わってしまいます）。この非対称が**dtypeと学習率を同時に動かしている**という交絡そのものである点は、この節の末尾で改めて述べます。

ただし、直前のper-kernel帰属実験（全6構成で学習率を1e-6に統一し、step 0のみ測定）では、学習率の影響を排除した上で同じ増幅（45倍）が確認されています。per-kernel実験の条件統一はenvファイルで機械的に確認済みです。

まずベースライン（増幅前）です。

| 指標 | miles | slime | 判定 |
|---|---|---|---|
| mis_kl | 0.000632 | 0.000529〜0.000740 (run間ばらつき) | 同オーダー（milesのbf16 4B runも0.000527〜0.000621で同程度に散らばる（8月H200の3 seed実測）） |
| logprob絶対差 | 0.0130 | 0.0129 | ほぼ一致 |
| ppo_kl（dropout=0時） | 0.0 | 0.0 | どちらもゼロ |

:::message
0.000529〜0.000740という範囲について補足します。FSxに残る7月クラスタのTensorBoardデータから確認した結果、これは**同一構成（4B bf16 dropout=0）の異なるrunのrun間ばらつき**です。slime（非miles計装）側のrunが`diag-nodrop` 0.000529から`diag` 0.000740まで散らばっています。miles側の4B bf16 runも0.000527〜0.000621で同程度に散らばります（8月H200の3 seed実測: `pp3_bf16_s123` 0.000621, `pp3_bf16_s777` 0.000527, `pp2_bf16_s42` 0.000606）。なおslime側の0.000529〜0.000740は`diag`と`diag-nodrop`というdropout条件が異なるペアの両端であるため、純粋なrun間ばらつきよりも条件差を含む可能性があります。より保守的に見るなら、8月の3 seed実測（0.000527〜0.000621、CV 7.8%）をノイズ床として採用してください。
:::
...
ここまでと同じH200クラスタ・同じ実験群での実測です。ここが検証の核心的な結果です。KV fp8・学習率1e-5・TIS（重要度比にcapをかける対処法。後述の「対処その1」で説明します）なしの条件（collapse arm）で、25ステップにわたってmis_klとgrad_norm（勾配のノルム）を追跡した実測データです。

grad_normとは、モデルの全パラメータの勾配を1本のベクトルとみなしたときのL2ノルム（長さ）です。健全な学習では0.1〜0.3程度で安定します。これが10を超えると「1回の重み更新でモデルが大きく動きすぎている」状態であり、学習が暴走していることを意味します。今回の崩壊armではgrad_normが通常の0.13から最終的に66.7（約500倍）まで爆発しました。
...
この2.097という値には注意が必要です。後述する3 seedでの追試では、崩壊時のmis_klピークがseed間で2.85から13.7までばらつきました。**つまり「どれだけ大きく壊れるか」は単一runの値では代表できません。** 一貫しているのは「壊れるかどうか」と「その順序」であって、大きさそのものではありません。
...
no correction（補正なし）はstep 14から暴走していくのに対し、TIS rescue armは崩壊せずに推移しています。7月クラスタの`rescue-tis2`は30ステップ完走し、mis_klは最大0.056（step 0: 0.032、最終: 0.028）でした。別ハードの3 seed実験（H100）でも、TISありの3 seedが全て30ステップ崩壊せず完走しています（最大値0.15前後）。
...
flowchart TB
    ...
    subgraph TISArm["TIS rescue arm: cap付き補正"]
        direction LR
        TA0["step0<br/>0.032"] --> TA14["step14<br/>約0.04"] --> TA29["step29<br/>0.028<br/>崩壊せず"]
    end
```

:::message alert
誠実な開示です。この節で示しているキラーチャートの崩壊アーム(`collapse-amp`)は25ステップ完走し、TIS rescue arm(`rescue-tis2`)は30ステップ完走しています。ただし先行する短い試行`rescue-tis`は10ステップで打ち切られており、その時点ではmis_klが0.033→0.067と上昇途中でした。30ステップ版では最終的に0.028に収まりましたが、10ステップ時点での上昇は事実です。「永久に崩壊しない」という主張は一切しておらず、30ステップという限られた窓での観察です。
:::
...
上の観察を、別のハードウェア（p5 / H100 x8のEKSクラスタ）で、3つの異なるseed（1234 / 42 / 123）で追試しました。前倒しの度合いを揃えるため、ここでもQwen3-4B dense、colocated、KV fp8_e5m2、学習率1e-5という同じ設定を使い、no correction（補正なし、collapse）とTIS cap=2.0（rescue）を、同じseedのもとで**TIS補正の有無だけを変えて**並走させています。今度はログをstep 19で止めず、30ステップ最後まで、しかもmis_kl・grad_normに加えて**reward（報酬）**まで追い切りました。

seedごとの、no correction側のmis_klのピーク値と、両アームの最終rewardは次の通りです。

| seed | no-corr mis_kl最大 | no-corr 最終reward | TIS mis_kl最大 | TIS mis_kl最終 | TIS 最終reward |
|---|---|---|---|---|---|
| 1234 | 12.6 | 0.0 | 0.150 | 0.071 | 0.61 |
| 42 | 2.85 | （崩壊） | 0.159 | 0.133 | （維持） |
...
第一に、**TISは3 seedすべてで mis_kl の最大値が0.15前後に収まります**（0.150 / 0.159 / 0.150）。no correctionの2.85から13.7に対して1桁から2桁低く、cap付き補正が効いていることはseedによらず再現します。ただし最終値で見ると0.071から0.133で1.9倍のばらつきがあるので、「seedに依存しない」と言えるのは崩壊を止めるという方向についてであって、到達する値そのものではありません。第二に、**no correctionは3 seedすべてで崩壊します**が、崩壊の激しさ（mis_klのピーク）はseedによって2.85から13.7まで大きくばらつきます。つまり「崩壊するかどうか（方向性）」はseed不変で決まっているのに対し、「どれくらい派手に崩壊するか」はseed依存でぶれる。この構造こそ、単一seedでは見えず、複数seedで初めて見える性質です。
...
### 欠けていた対照実験: 崩壊させているのは学習率ではなくdtypeだった

この対照実験はH200 x8のクラスタで実施しました（比較対象の崩壊アームは別ハードのH100 3 seed実験のseed 1234です。ハードが違うので、比較できるのは「崩壊するかしないか」という方向であって、値そのものではありません）。

ここまで読んで、鋭い読者はある反論を思いつくはずです。崩壊アームは**2つのものを同時に変えている**、と。KVキャッシュのdtype（bf16 → fp8_e5m2）と、学習率（1e-6 → 1e-5）です。しかもこの記事に出てくるbf16の実行は、すべて学習率1e-6です。だとすれば「fp8が崩壊を起こした」のではなく「学習率1e-5がこの構成では単純に高すぎた」だけかもしれません。

この反論はもっともで、しかも**この検証には長らくその対照実験がありませんでした**。登壇資料のレビュー過程でこの穴が指摘されたので、実測しました。

条件は崩壊アーム（seed 1234）と、**ハードウェア以外のすべてが同一**です。bf16 KV、学習率1e-5、TISなし、30ステップ、同じseed。ただし比較対象の崩壊アームはH100、この対照はH200なので、**厳密な単一変数比較ではありません。** 比較できるのは「崩壊するかしないか」という方向であって、値の大小ではありません。

| step | fp8_e5m2のmis_kl | fp8_e5m2のgrad_norm | bf16のmis_kl | bf16のgrad_norm |
|---|---|---|---|---|
| 0 | 0.0312 | 0.158 | 0.000594 | 0.127 |
| 8 | 0.0722 | 0.429 | 0.000477 | 0.169 |
| 14 | 0.399 | 2.78 | 0.000620 | 0.124 |
| 18 | 1.659 | 16.4 | 0.000581 | 0.0657 |
| 24 | 11.03 | 50.6 | 0.000562 | 0.135 |
| 最大 | **12.59** | **57.5** | **0.00063** | **0.169** |

**学習率1e-5それ自体は崩壊を起こしません。** 同じ学習率でdtypeをbf16に戻すと、mis_klは約0.0006という良性の水準から30ステップ通して動かず、grad_normも0.17を超えません。fp8アームがstep 24でmis_kl 11.0・grad_norm 50.6に達している時点で、bf16アームはmis_kl 0.00056・grad_norm 0.135です。最大値どうしで比べると、mis_klで**約2万倍**、grad_normで**約340倍**の差があります。bf16側はrewardも0.562で生き残り、`repetition_frac`も最大0.031で生成は退化していません。**崩壊の兆候が1つも出ていません。**

これで交絡が1つ潰れました。**「fp8で増幅されたmismatchが崩壊を駆動している」という因果の主張は、論文の図に頼らず、この検証自身の実測で支持できます。** 逆に言えば、この対照実験を回すまでは、その主張は交絡を注記しただけの状態でした。注記することと、実験で潰すことは違います。

さらに、LR 1e-6のままfp8を有効にした14 step実行（`pt_e5m2_train`、H200）もFSxに残っています。mis_klは0.0324から始まり、14 step後に0.0283で安定したままです。つまり**fp8による増幅は起きる（bf16の0.0007に対して約45倍）が、LR 1e-6では崩壊しない**。これで2x2の交差実験が揃いました:

| | bf16 KV | fp8_e5m2 KV |
|---|---|---|
| **LR 1e-6** (14 step) | mis_kl 0.0007 / reward 0.68 / grad_norm 0.12 | mis_kl 0.028 / reward 0.55 / grad_norm 0.17 |
| **LR 1e-5** (30 step) | mis_kl 0.0006 / reward 0.56 / grad_norm 0.12 | mis_kl **31.78** / reward **0.023** / grad_norm **66.7** |

4セルすべてでrepetition_frac = 0（反復ループなし）です。**崩壊しているのは右下の1セルだけ**で、reward が 0.44→0.023 に落ち、grad_norm が 0.13→66.7 に爆発しています。

LR 1e-6 + fp8（右上セル）は、14 step の時点で:
- mis_kl が step 0 (0.032) から**微減** (0.028) — 上昇していない
- reward が 0.42→0.55 と**上昇中** — 学習が正常に進んでいる
- grad_norm が 0.17 で安定 — 崩壊 arm の step 14 (2.78) とは桁違いに低い

:::message alert
ただし LR 1e-6 + fp8 の run は**14 step で打ち切られています**。崩壊アーム (LR 1e-5 + fp8) の onset は step 8〜14 付近なので、「14 step で安定」は「30 step でも崩壊しないことの証明」としては不十分です。30 step の検証は次回の Capacity Block で実施予定です。

とはいえ、崩壊 arm が step 14 で既に mis_kl=0.399 / grad_norm=2.78 と明確に上昇しているのに対して、LR 1e-6 arm は mis_kl=0.028 / grad_norm=0.17 で全く動いていないので、「同じ onset タイミングで崩壊に向かっている」可能性は極めて低いと考えています。
:::

現時点で言えるのは: LR 1e-5 + bf16 が 30 step 安定であること（mis_kl max 0.00063）から、**「崩壊には fp8 増幅が必要条件」は確定**。fp8 + LR 1e-6 が崩壊しないことは「14 step の範囲では崩壊の兆候が一切ない」までです。

:::message
この対照は単一seed（1234）です。またdtype以外を揃えたので、「fp8とバックエンド強制切替の2変化が合わさっている」という前述の注記（増幅の実測値の節）はそのまま残ります。潰せたのは学習率という交絡です。2x2交差の全セルのenvファイルでLR/dtype以外の条件が同一であることを確認済みです。
:::
...
大半のトークンは健全な $\rho$ を持っていますが、1つだけ $\rho = 3 \times 10^{-5}$ という極端に低い値が混ざっています。$\rho = \pi_{\text{train}}/\pi_{\text{rollout}}$ なので、これは**trainerがそのトークンにrolloutよりはるかに低い確率を割り当てている**（= trainerから見て「ほぼ不可能」なトークンをrolloutが生成した）状況です。
...
| 中央: TIS、cap = 2.0 | 通常のTIS設定 | mis_klは30 step完走しmax 0.056で崩壊せず。救済成功。slimeではrewardが0.44→0.73に上昇 |
...
両方とも、baseline（bf16）のmis_klは約 $6 \times 10^{-4}$ で、step 0でのKV fp8増幅後は約 $3 \times 10^{-2}$ に達します（前掲の増幅の実測値節と同じrunです）。倍率にすると、slimeで約54倍、miles で約49倍です。2つのフレームワークで、ほぼ同じ数値が観測されました。

このことは、この差の大きさが「共有されているrollout対trainerの数値経路（計算の仕方の違い）」によって決まっており、フォークごとに分岐したコードパスに由来するものではないことを示唆しています。

なお本記事にはbf16 baselineのmis_klとして0.000632、0.000716、0.000627という3つの値が登場します。ハードウェア帰属が確定しているので整理しておきます。0.000632は7月H200での主計測（run `miles-smoke`）、0.000627は8月H200のキャリブレーション（run `h200_p4_calib`）、0.000716は8月**H100**のper-kernel帰属実験の基準run（`e4_c0_auto`）です。per-kernel実験はH100で実施しているため、0.000716をH200の4 seed分布（0.000527〜0.000625）と直接比べるのは不適切です。H100 baseline（`baseline` run, 0.000663）と比べれば0.000716は約8%差で、seed CVの7.8%とほぼ同水準です。

この一致は、別のハードウェア（p5 / H100 x8のEKSクラスタ）でも再確認しました。slimeとmilesはベースイメージが異なり（slimeはNGCベース、milesはnvidia/cudaベース）、Megatronのフォークも別で、torch_distチェックポイントもそれぞれのMegatronで別々に変換しています。それでも、このEKSクラスタで測ったbaseline（bf16）のstep-0 mis_klは、slimeが0.000615、milesが0.00072と、やはり同じオーダーに収まりました。共有する数値経路が差を決めているという見立ては、H200に加えてこのH100クラスタでも支持されています。
...
1 seedあたり128問（応答32768トークン）で測った結果です。`k3_kl`はmis_klと同じSchulman k3推定量（$e^r - r - 1$）で、常に非負です。
...
この実験はH200 x8のクラスタで実施しました。結果は予想と違いました。エンジン間の差（k3_kl $8.06 \times 10^{-4}$）は、SGLang対Megatronのベースライン（mis_kl 約 $6 \times 10^{-4}$）と**同じオーダー**にとどまりました。しかもこの量はとても安定しています。3 seedでの変動係数はわずか2.5パーセントで、崩壊時のmis_klピーク値（seed間で2.85から13.7までばらつく）やfork間の倍率よりも桁違いに締まった量です。
...
この表はすべてseed 1234の単独値です。4Bだけは3 seedまで広げてあり、その平均は0.000806（変動係数2.5パーセント）でした。1.7Bと8Bは単一seedなので、モデル間の差を論じるときはこの非対称に注意が必要です。
...
### 位置プロファイル: O(T²)を測ってみたら、崩壊前はTでした

ここまで、応答が長いほど誤差が二乗で増えるというO(T²)の話は、論文からの引用として紹介しました。上界の式であって、私たちの実測ではないと注記もしました。今回、これを直接測りました。

先に結論の範囲を明確にしておきます。**これから示す「Tで伸びる」という結論は、崩壊が始まる前の静穏な状態に限った話です。** 崩壊が進むとこの結論は崩れ、T²の上界すら越えていきます。詳細は後述の「崩壊の中で測る」の節で示します。

![累積誤差の分散スケーリング](/images/4c8e1a2b6f-scaling.png)
*累積誤差の分散が系列長Tに対してどうスケールするか（分散 ~ T^alphaのalphaが指数。1ならTに比例、2ならT²に比例）。静穏域では alpha=1 の側に寄りますが、崩壊が進むと alpha=2 の上界すら越えていきます。alphaの定義は後段の「分散のスケーリング」で詳しく説明します。*

測り方に一つ工夫があります。素直にやるなら`--rollout-max-response-len`を8192、16384、32768と振って、系列長ごとに誤差を比べるところです。しかしそれをすると、KVキャッシュの予算、バッチの形、chunked prefill（長い入力プロンプトを複数チャンクに分割して処理する仕組み）の境界、生成される内容の分布、打ち切り率が全部同時に動いてしまい、何の効果を見ているのか分からなくなります。
...
*seed 4本の傾きを軸上に並べています。bf16は密集し、fp8は符号を跨いで散ります。破線が撤回した区間です。*

Qwen3-4B、TP1（Tensor Parallel 1、つまり分割なし）/CP1（Context Parallel 1、系列を複数GPUに分割しない設定。理由は下の折りたたみで説明します）、step 0のみ、1実行あたり128系列。**4つのseed × 3つのarm = 12実行**を回しました。区間はseed間のばらつきから求めています。
...
原因は手法の側にあります。傾きの推定には2種類の偶然が入っています。1つ目は、rolloutがどんな128系列を生成したかという偶然で、これはseedで決まります。2つ目は、その128系列という有限の標本から傾きを推定するときの標本誤差です。系列を選び直すブートストラップは、手元の128系列を固定したまま「そこから引き直したら」を繰り返すので、**2つ目しか測れません**。1つ目は何万回リサンプルしてもゼロのままです。分散の分解で書くとこうなります（$\hat\beta$ は傾きの推定値そのもの、つまりここまで表で見てきた「平均傾き」の値です）。
...
教訓は単純です。**モデルが自分でデータを作る実験では、seed複製なしに符号を主張してはいけません。** 2本目のseedを取るコストは、この構成なら1本あたり10分程度でした。これを省くと結論が反転します。

そして同時に、seedを4本まで増やしたことで、bf16の減少は実在する効果として言えるようになりました。**seed複製は主張を弱めるだけでなく、生き残った主張を強くします。**

#### 分散のスケーリング: これはseedに対して安定していた

傾きとは別の量を見ます。傾きは $|r_t|$ の**平均**が位置に対してどう動くかでした。ここで見るのは $\sum r_t$ の**分散**が系列長に対してどうスケールするかです。この2つは独立で、平均が緩やかに減りながら分散がTで伸びることは両立します。誤差 $r_t$ が互いに独立なら分散は足し算で増えるので $\mathrm{Var}[\sum_{t<T} r_t] = T\sigma^2$（$\sigma^2$ は1トークンあたりの誤差の分散）、つまり $\alpha = 1$ です。逆に毎トークン同じ向きのバイアス $b$ が乗り続ける最悪ケースでは $\sum_{t<T} r_t = bT$ となり、$\mathrm{Var} = T^2\,\mathrm{Var}[b]$、つまり $\alpha = 2$ です。論文のO(T²)はこの後者を上界として置いています。したがって $\alpha$ が1に近ければ誤差は「ただのノイズの足し合わせ」、2に近ければ「位置に対する系統的な蓄積」という読み方になります。

ここで1つ注意が必要です。系列ごとに平均バイアスが違うだけで、位置依存性がまったく無くても $\alpha$ は2に漸近してしまいます。そこで、系列を先頭から重複しない2個組（$(r_0, r_1)$、$(r_2, r_3)$、……）に分け、各組の差を取ります。系列 $i$ の誤差を $r_t = b_i + \varepsilon_t$（$b_i$ はその系列に固有の一定バイアス）と書くと、各組の差 $r_{2j} - r_{2j+1} = \varepsilon_{2j} - \varepsilon_{2j+1}$ から $b_i$ が消えます。この差分系列で推定した指数を $\alpha_{\text{within}}$ と呼びます。
...
したがって**この静穏域では系列レベルの累積誤差はTで伸びており、T²ではありません**。論文のO(T²)は上界としては正しくても、この構成の実測はその上界に張り付いていません。実務的には、**崩壊を未然に防ぐという局面では**tokenレベルのTISから試すべきです。ただし「tokenレベルで足りる」と言い切るのは行き過ぎで、生の $\alpha$ が $\alpha_{\text{within}}$ を上回るrunがあり、系列ごとのバイアスのばらつき自体は実在します。それはtokenレベルのcapでは扱えない量です。この結論が静穏域限定である理由は、後述の「崩壊の中で測る」で崩壊後の実測とともに示します。

そして「この領域では」と限定を付けたのには理由があります。ここまでの12実行はすべてstep 0、つまり**optimizerを一度も踏む前**の測定です。崩壊アームは`mis_kl`が0.8から14まで上がるのに、それを測っていません。この限定は後述の「崩壊の中で測る」で外します。結論は変わります。

:::message
この推定には限界が2つあります。どちらも生の $\alpha$ と併読することで塞げます。

第一に、$\alpha_{\text{within}}$ が何を分離しているかを正確に述べる必要があります。隣接差分が厳密に消すのは**系列ごとの一定オフセット**だけです。ここでの「隣接差分」は、系列を先頭から2個ずつの組（$r_0$と$r_1$、$r_2$と$r_3$、……）に分け、各組で偶数番目から奇数番目を引いたものです（重複しない組であり、単純に1つずつずらした差分ではありません）。系列ごとに傾きの違う線形トレンド $r_t = b_j t$ の場合、どの組でも差は定数 $-b_j$ になるので、先頭から$k$個までの差分を足した$D_k$は $D_k = -b_j k/2$ となり、$\mathrm{Var}(b_j) k^2/4$ が残ります。つまりトレンドは消えず、半分残ります。

これは直接確認しました。ノイズのない純粋な線形トレンドだけの生成器では $\alpha_{\text{within}}$ が2.03になり、同じトレンドを現実的なノイズの下に置くと1.04になります。後者はノイズ項が支配するためで、トレンドが消えたからではありません。

したがって $\alpha_{\text{within}}$ は「系列に沿って分散が増えるか」に答える量であり、その増加が位置依存のスケールから来るのか系列ごとのトレンドのばらつきから来るのかは区別しません。区別して除外できるのは一定オフセットのばらつきだけで、それは生の $\alpha$ を押し上げる成分です。**2つの指数は必ず対で読んでください。**

第二に、生の $\alpha$ が1.29に達するrunがあること自体は、系列ごとのバイアスのばらつきが実在することを意味します。系列単位のバイアスはtokenレベルのcapでは補正しきれない量です。合成データで確認すると、系列ごとの一定バイアスのみを持つ（位置依存性ゼロの）生成器では生の $\alpha$ が2.00、$\alpha_{\text{within}}$ が1.00になります。差分が一定オフセットだけを厳密に取り除いていることの確認です。
:::
...
### 崩壊の中で測る: Tスケーリングは崩壊すると成り立たない

ここまでの位置プロファイルは全部step 0、あるいは`mis_kl`が0.032を超えなかった14 stepの実行での測定でした。一方で崩壊アームは`mis_kl`が0.8から14まで上がります。**250倍離れた領域について、何も測っていない状態でTスケーリングを主張していた**わけです。ここを測りました。

別ハード・3 seed実験のseed 1234アームと同じ設定（fp8 e5m2、LR 1e-5、30 step）で、per-tokenダンプを崩壊の全行程にわたって取りました。**この測定はH200 x8で実施しています（参照した3 seed実験はH100）。** `mis_kl`は0.0325から**31.78**まで上がり、そのseed 1234アームの最大値12.6を2.5倍超えました。同じ設定でこれだけ差が出た理由は特定できていません。候補はハードウェアの違い、per-tokenダンプの計装がもたらす実行順序の変化、そして崩壊そのものの非決定性の3つで、切り分けていません。**したがって31.78という値は「崩壊域では$\alpha_{\text{within}}$が上がる」という定性的な結論の根拠としてのみ使い、大きさを他と比べる用途には使いません。** ダンプは1424ファイルです。

学習の進行で4分割し、各区間がどの`mis_kl`帯に対応するかを対応付けます。

| `mis_kl`帯 | 状態 | $\alpha_{\text{within}}$ | 傾き$\lvert r \rvert$ |
|---|---|---|---|
| 0.026 - 0.046 | 静穏 | **0.95** | $-6.3 \times 10^{-7}$ |
| 0.046 - 0.40 | 立ち上がり | **1.48** | $+4.3 \times 10^{-5}$ |
| 0.40 - 3.7 | 崩壊進行 | **3.12** | $+9.8 \times 10^{-4}$ |
| 3.7 - 31.8 | 崩壊後 | **3.55** | $+1.5 \times 10^{-2}$ |

$\alpha_{\text{within}}$が0.95から3.55へ単調に上がります。**「誤差はTで伸びる、T²ではない」は静穏域限定の性質でした。** 崩壊が始まると、誤差は系列に沿ってTよりはるかに速く積み上がり、論文が上界として置いた$\alpha = 2$すら越えていきます。傾きも符号を反転させた上で約5桁大きくなります。静穏域では「位置とともにわずかに減少」だったものが、崩壊域では「位置とともに激しく増加」に変わります。

:::message alert
この測定には交絡が2つあり、どちらも潰さないと結論が変わります。

**第一に、崩壊すると応答長が動きます。** 中央値が8048 → 3961 → 4000 → 6118と4区間で移動し、打ち切り率も静穏域の0.562から立ち上がり域の0.117、崩壊後の0.953へと大きく動きます（打ち切り率は3区間分しか記録が残っていません）。$\alpha$はprefix長の関数なので、区間ごとに違う長さで測ると長さの効果を構造の変化と誤認します。全区間が到達する2048に固定して測り直したのが上の表です。固定しない場合は0.95 / 1.70 / 3.14 / 1.75で、最終区間だけ下がります。**固定後のほうが単調になる**ので、この「下がり」は長さのartefactでした。

**第二に、これが反復を測っていないことを確認する必要があります。** 30B MoEでは生成が反復ループに入り、`mis_kl`が反復そのものを測っていました。この実行の`repetition_frac`は**全30 stepで0.0156以下**で、生成は退化していません。一方`raw_reward`は0.742から0.0234まで落ちており、政策としては確実に崩壊しています。つまり測っている$|r|$の増大は、反復トークンの副作用ではなく崩壊そのものに伴う数値的性質です。
:::

実務的な含意が変わります。ここまで「tokenレベルのTISから試すべき」と書いてきましたが、**その根拠は静穏域の$\alpha \approx 1$でした**。崩壊が始まった後は$\alpha$が3を超えるので、崩壊後の状態から回復させる目的ではtokenレベルの補正では足りない可能性が高いです。一方でTISの実務的な価値は「崩壊させないこと」にあり、TISアームが3 seedで`mis_kl`を0.15に抑え込んだ事実は変わりません。**予防にはtokenレベルで足りる、しかし事後の回復を論じるなら別の話**という整理になります。

なお単一seedです。位置プロファイルの傾きがseedで符号を変えることは上で実証したので、この4区間の**傾きの値**をseed不変として扱ってはいけません。ただし$\alpha_{\text{within}}$は12実行で0.76から1.09とseedに対して安定していた量で、それが3.5まで動くのはseed分散では説明できない大きさです。
...
この線引きは、本記事の他の数値にも適用すべきです。キャリブレーション実験として、H100（8月クラスタの`baseline` run, mis_kl=0.000663）とH200（同クラスタの`h200_p4_calib` run, mis_kl=0.000627）を比べると5.4%差です。なお以前の版でbaseline 0.000632を「H100」と記載していましたが、FSxの一次データから確認した結果、0.000632は7月のH200実測でした（run `miles-smoke`）。H100のbaseline実測値は0.000663です。5.4%差はseed CVの7.8%より小さいです。正確には「ハードによる差はseedによるばらつきより小さく、mis_klの水準として区別できない」です。並列構成（TPやPPの値）を振った実験でも同じことが言えます。構成間の差はseed分散と同じオーダーにとどまったので、「並列構成はmis_klを動かさない」という方向の結論は保てますが、具体的な倍率をseed分散と区別して語ることはできません。
...
- **E5**: SGLangとvLLMに同一のトークン列を通し、推論エンジン同士のlogprob一致度を3 seedで測りました。結果は「vLLMだと差がもっと大きくなるのか」の節に反映済みで、エンジン間の差がSGLang対Megatronのベースラインと同じオーダー（k3_kl $8.06 \times 10^{-4} \pm 0.20 \times 10^{-4}$）にとどまることを示しました。事前の機構的推論とは逆の結果です。
- **位置プロファイル** （H200で追加）: per-token logprobをダンプし、誤差が系列内で蓄積するかを4 seed × 3 armで測りました。結果は「位置プロファイル」の節に反映済みです。さらに崩壊アームと同一設定で30 step完走させ、崩壊の全行程でダンプを取りました。結果は「崩壊の中で測る」の節に反映済みで、静穏域で得た$\alpha \approx 1$という結論の適用範囲を狭める内容です。
...
*数値のずれから学習が死ぬまでの5段階。30B MoEは段3に到達する前、段1で壊れていました。原因はSGLangのexpert並列です。*

数値のずれが崩壊まで進む経路は、5つの段に分けられます。段1: rolloutのカーネル選択（融合attentionなど）。段2: KVキャッシュの精度（本記事のfp8増幅）。段3: 重要度比への反映（裾が爆発するかどうか）。段4: 勾配ステップへの反映（entropy崩壊）。段5: TISによる補正。**先ほどの「mis_kl→grad_norm→reward」という3段階の順序は、この5段のうち段3から段4への移行に対応します。**30B MoEは段2（bf16なので量子化は関係ありません）や段3をまだ経ずに、段1の生成品質そのものが崩れていたという意味です。
...
4回の独立した実行すべてで、モデルは反復ループに入り、停止せず、答えに到達しませんでした。報酬は構造的にゼロになります。なお**この4実行はすべてKVキャッシュをbf16のまま**（量子化なし）で回しているので、この反復は本記事の主題であるfp8量子化とは無関係です。このとき記録されるmis_klは0.309ですが、反復（`repetition_frac` 0.48〜0.70）と同時に観測された値であり、**量子化の誤差ではなく反復ループの副産物である可能性が高いです**（どのトークンがmis_klに寄与しているかのper-token帰属は未実施なので、これは断定ではありません）。
...
上限を上げても打ち切り率が動かないという結果が決定的でした。長さが足りないなら上限を上げれば率が下がるはずで、下がらなかったことは「長さの問題ではない」ことを意味します。

そして4つとも否定されたことには理由がありました。**4つすべてがmilesの内部を疑っていたのです。**

`repetition_frac`はSGLangが生成したテキストに対して計算されます。だとすれば、milesのGRPOループを一切通さず、SGLang単体に同じチェックポイントをロードさせて生成させれば、原因が訓練ループの内か外かが決まります。それをやりました。

**再現しました。** しかも1軸ずつ動かすと、原因が一意に特定できました。32プロンプトの反復率です。

| | EP=1 | EP=2 | EP=4 |
|---|---|---|---|
| TP=1 | **0.000** | （起動不可） | — |
| TP=4 | **0.000** | 0.875 | — |
| TP=8 | **0.000** | 0.594 | 0.844 |

**原因はexpert parallelism（EP、エキスパート並列）でした。** EP=1ならTPを1でも4でも8でも健全、EP>1ならTPを4でも8でも崩壊します。TP=8に固定してEPだけを1→2→4と動かすと0.000→0.594→0.844と悪化しますが、EP=2の列だけを見るとTP=4で0.875・TP=8で0.594とTPでも同程度動いており、「EPを増やせば増やすほど単調に悪化する」と言えるだけのデータではありません。確実に言えるのは、EP=1が全構成で健全・EP>1が全構成で崩壊するという二値の対比です。

Web調査で最有力だった「サンプリングパラメータが推奨値から外れているせい」という仮説も、この過程で棄却されました。Qwen公式推奨値（temperature 0.6 / top_p 0.95 / top_k 20）に変えても反復率は0.875で動きません。`moe_runner_backend`の明示指定も原因ではなく、既定値に戻しても0.844でした。

ここで一つ疑問が浮かびます。反復はrollout（SGLang）側の現象なので、**rolloutだけEP=1にして、trainer（Megatron）側はEP=2以上のままにできないか**という発想です。TP=8・EP=1でrolloutが健全だったのはすでに実測済みです。

しかし今回使ったmilesのGRPOレシピは、`EP_SIZE`という1つの変数を`--expert-model-parallel-size`（trainer側）と`--sglang-expert-parallel-size`（rollout側）の両方にそのまま渡しています。つまりrolloutとtrainerのEPを別の値に設定する経路が、このレシピには実装されていません。原理的に不可能というわけではなく、レシピの実装がその分離を提供していないということです（改修すれば試せる可能性がありますが、今回は未実施です）。

そこで今回試した「trainer側もEP=1にする」方法を見てみます。**これは回避策になりません。** EP=1で訓練しようとすると、各rankが128個のexpert全部を持つことになり、1 rankあたり108.76 GiBを要求してOOMします（GPU容量は139.80 GiB）。この要求はrolloutと同居させるcolocated配置でGPUメモリを共有しているために起きており、trainerの重み＋optimizer状態＋activationの合計がrollout側の確保分と競合した結果です。TPを8から16に倍にしても要求は108.41 GiBでほぼ変わりません。**tensor parallelはexpertの重みを分割していない**のです。

つまり今回試した範囲では、EP=1なら生成は健全だが訓練できない、EP>1なら訓練できるが生成が崩壊するという状況です。rollout/trainerのEPを分離するレシピ改修は試していないので、それが回避策になるかどうかは未検証のまま残っています。この構成での30B MoEのmismatch計測は、少なくとも今回の実装のままでは成立しません。
...
ここまではslime/milesという特定のフレームワーク上での実測でした。verlやOpenRLHFなど別のRLフレームワークを使っている場合でも、考え方自体は移植できます。

必要なのは、rolloutが生成した各トークンのlogprob（推論エンジンが吐く値。多くの推論エンジンはこれをオプションで返せます）を保存しておき、同じトークン列をtrainer側に**teacher-forcing**（生成済みの系列をそのまま入力として与え、trainer自身に同じトークンのlogprobを再計算させること）で通すことです。2つのlogprobの差、またはそのKL版がmis_klに相当します。フレームワークに組み込みの仕組みがなくても、rollout側のログ出力とtrainer側の評価パスを流用すれば自作できます。

どこからが「大きい」かの目安も付け加えます。本記事の実測では、良性の水準（bf16、量子化なし）が概ね $10^{-4}$ から $10^{-3}$ のオーダーでした。ここからKV量子化だけで10倍から50倍に増幅し、崩壊した状態では静穏時から数百倍から数万倍に達します。したがって絶対値そのものよりも、**自分の構成のbaseline（量子化なし、健全な状態）を一度測っておき、そこからの倍率で監視する**のが実務的です。baselineから1桁動いたら注意、2桁動いたら介入を検討する、というのが本記事のデータから読み取れる目安です。
...
2. **小さければ補正はまだ不要ですが、測るのはやめないでください**。今回の検証では約 $6 \times 10^{-4}$ という値でしたが、これくらい小さければ良性の差であり、補正は不要です。ただしキラーチャートの崩壊アームも、増幅後のstep 8まではmis_klが約0.03前後という別の（より高い）水準で静かに推移し続けており、そこから崩壊が始まりました。「今の値が小さいから安全」ではなく「小さい間も監視を止めない」が正確な読み方です。素のbf16での $10^{-3}$ 前後という水準は、推論エンジンをSGLangからvLLMに丸ごと入れ替えても同じオーダーにとどまりました。最適化された推論スタック同士が単にお互いに対して持ってしまう差の水準であり、良性かどうかを判断するときの目安として使えます。
3. **大きければ重要度比にcapをかける**。特に、trainerのlogprobを生のrollout logprobでそのまま置き換えることは避けてください。それはvariance駆動の失敗を招き、勾配ノイズが爆発します。capはまずtokenレベルで試してください。静穏域では誤差の累積が系列長Tに対してTで伸びる（T²ではない）ことを実測したので、この局面で系列レベルの補正を先に持ち出す根拠はありません。ただし崩壊が始まった後は$\alpha_{\text{within}}$が3を超え、Tでは説明できない速さで積み上がることも実測しました。**tokenレベルのcapは崩壊の予防には十分ですが、崩壊後の回復には別の議論が必要です。**
...
seed分散を測ったことで、確立した主張とそう言えない主張の線引きができました。生き残るのは、KV量子化による10倍から50倍の増幅、量子化精度への単調性、$\alpha_{\text{within}}$ が12実行すべてで0.76から1.09に収まること、bf16の位置プロファイルの減少（4 seedすべてで負）、そして崩壊と救済の方向性（3 seedすべてでno correctionは崩壊し、TISはmis_kl約0.15で救済される）です。$\alpha_{\text{within}}$の0.76から1.09という範囲は**静穏域（step 0、またはmis_klが0.03程度で頭打ちの14 step実行）に限った話**である点も明記しておきます。崩壊の全行程を追った実行では$\alpha_{\text{within}}$が3.55まで上がることを別に実測済みです（「崩壊の中で測る」の節）。一方で残る限界は次のとおりです。

**単一seedの点推定どうしの比較は、seed分散（CV 7.8から11.3%）より小さい差については何も言えません。** これはH100とH200の差である5.4%（H100 baseline 0.000663 vs H200 calibration 0.000627）、そして並列構成を振ったときの変動に直接当てはまります。倍率どうしの比較（fork間の54倍対49倍、モデルサイズ間の4Bで51.7倍対8Bで34.6倍）については、倍率が分子と分母それぞれのノイズを合成するため、単純にseed分散と比べることはできません。51.7対34.6は1.49倍の差でseed分散の1.19〜1.27倍より大きく見えますが、合成後の不確かさを計算していないので、この差が実在するとも実在しないとも本記事のデータでは言えません。逆に言えば、これらの構成変更がmis_klをseed分散を超えて動かすことはない、という形の結論は保たれます。
...
**30B MoEでの検証（E7）は計測不能でした。** 構成は完走しますが生成が反復ループに陥り、報酬がゼロになります。原因の候補4つを実測で否定した後、GRPOループの外にSGLang単体で切り出して原因をexpert並列に特定しました。ただし回避策にはならず（EP=1では訓練時にOOMする）、少なくとも今回試したレシピの実装のままでは、この構成での計測は不可能です。したがって学習を伴う崩壊・救済の実験、およびエンジン間の一致度の検証は、いずれもdense構成に限られます。
