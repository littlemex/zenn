...
H200での実測値は、学習フレームワークが書いたTensorBoardのイベントファイルと、実験ドライバが書いた結果ファイルの両方に同じ値があることを照合ツールで確認しています。H100時代の計測は別クラスタで取ったもので、この照合の対象外です。なお一部の数値についてはH100とH200のどちらで取ったかが一次記録間で食い違っており、その旨を本文の該当箇所で明示しています。記事の後半では、自分の結論を撤回・訂正した経緯を複数書いています。
:::
...
上の5箇所は「候補」でした。ではKVキャッシュをfp8にしたときの増幅（この節の実験では基準の0.000716に対して約45倍。後述する49倍・54倍は別のrunで基準値も違います）は、このうちどれがどれだけ効いているのでしょうか。各フラグを1つずつ切り替えて、学習なしでstep-0のmis_klだけを測る実験（per-kernel ablation）を実施しました。学習を回さないので安価で、しかも「学習率がまだ影響を持たない」step 0の純粋な数値経路差だけを取り出せます。
...
ここからは実測データです。単一seedの点推定である旨は後述します。ハードウェアについて1つ正直に書いておきます。この節のbaseline 0.000632という値は、後述するH100/H200キャリブレーション実験ではH100側の値として扱われている一方、この実験群をまとめた別の記録ではH200での計測と記載されています。**一次記録どうしが食い違っており、どちらか一方が誤記です。** したがって本記事ではこの節と次のキラーチャート節のハードウェアを断定しません（H100とH200はどちらもHopper世代で、後述のキャリブレーション実験ではmis_klの差が0.8%しかないため、この節の結論には影響しません）。Qwen3-4B dense（密なモデル、MoEではない構成）、colocated配置、dropout（学習時に一部ニューロンを無効化する正則化手法）は0です。学習率はベースラインが1e-6、fp8で増幅したrunは1e-5です。増幅側だけ学習率を上げているのは、30ステップという短い実行のなかで崩壊まで到達させるためです（1e-6のままだと崩壊が始まる前にキャパシティブロックが終わってしまいます）。この非対称が**dtypeと学習率を同時に動かしている**という交絡そのものである点は、この節の末尾で改めて述べます。
...
ここまでと同じクラスタ・同じ実験群での実測です（ハードウェアの帰属が一次記録間で食い違っている点は前節に書いた通りです）。ここが検証の核心的な結果です。KV fp8・学習率1e-5・TIS（重要度比にcapをかける対処法。後述の「対処その1」で説明します）なしの条件（collapse arm）で、25ステップにわたってmis_klとgrad_norm（勾配のノルム、勾配全体の大きさを表す指標）を追跡した実測データです。
...
なお本記事にはbf16 baselineのmis_klとして0.000632、0.000716、0.000627という3つの値が登場します。これらは同じ構成の別々のrunです。0.000632がfork間concordanceの主計測、0.000716がper-kernel帰属実験（E4）の基準run、0.000627がH100/H200キャリブレーションのH200側です。**後述する4 seedの実測範囲は0.000527〜0.000625なので、0.000632と0.000627はその上端のすぐ外、0.000716は平均比で約20パーセント高く、seed分散（変動係数7.8パーセント）の2倍以上外れています。** 0.000716が高い理由は特定できていません。 どれか1つを「正しい値」として他と差を論じることはできません。
...
1 seedあたり128問（応答32768トークン）で測った結果です。`k3_kl`はmis_klと同じ「KL divergenceの近似推定量」の一種で、常に非負になる性質を持ちます（$k_1$/$k_2$/$k_3$のどの定義に厳密に対応するかは前述の理由で断定しません）。
...
この線引きは、本記事の他の数値にも適用すべきです。別のキャリブレーション実験として、H100とH200で全く同一の構成（Qwen3-4B dense、TP1、LR 1e-6、bf16、seed 1234）を1つのseedだけで比べたところ、mis_klは0.000632と0.000627で0.8%差でした（この実験の記録では前者をH100、後者をH200としていますが、前述の通り0.000632の帰属は一次記録間で食い違っています。したがってこの0.8%は「ハード間の差」ではなく「同一設定の別run間の差」である可能性も残ります。いずれにせよseed分散より小さいという結論は変わりません）。ハード非依存の証拠として使えそうに見えますが、**0.8%はseed CVの7.8%より小さいです。** 正確には「ハードによる差はseedによるばらつきより小さい」と述べるべきで、これは主張としては弱まりますが、実務上の含意（ハードを変えてもmis_klの水準で驚くことはない）は変わりません。並列構成（TPやPPの値）を振った実験でも同じことが言えます。構成間の差はseed分散と同じオーダーにとどまったので、「並列構成はmis_klを動かさない」という方向の結論は保てますが、具体的な倍率をseed分散と区別して語ることはできません。
