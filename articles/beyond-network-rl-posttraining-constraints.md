...
no correction（補正なし）はstep 14から暴走していくのに対し、TIS rescue armはログが残っているstep 19まで0.03〜0.045の範囲でフラットに保たれています。なお後述する別ハードの3 seed実験ではTISの最大値が0.15前後になりますが、そちらは30ステップ完走した別のrunで、ピークがstep 20以降に出ています。この節のrunはstep 19で記録が途切れているので、両者の数値を直接比べることはできません。
...
:::message
この対照は単一seed（1234）です。またdtype以外を揃えたので、「fp8とバックエンド強制切替の2変化が合わさっている」という前述の注記（増幅の実測値の節）はそのまま残ります。潰せたのは学習率という交絡です。
:::
