...
ここからは実測データです。検証環境はH100 x8のクラスタ（当時のキャパシティブロックが終了しており、この節の数値の生データは残っていません。単一seedの点推定である旨は後述します）、Qwen3-4B dense（密なモデル、MoEではない構成）、colocated配置、dropout（学習時に一部ニューロンを無効化する正則化手法）は0です。学習率はベースラインが1e-6、fp8で増幅したrunは1e-5です（理由は後述します）。
