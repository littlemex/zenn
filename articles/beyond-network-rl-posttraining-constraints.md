...
3. **training-inference mismatch** （数値のずれ）: lossやrewardには見えません。mis_klという専用の指標を計測しない限り無症状です（H200に加え、p5 / H100 x8のEKSクラスタで3 seed実測。本記事の主役です）
...
H200での実測値は、学習フレームワークが書いたTensorBoardのイベントファイルと、実験ドライバが書いた結果ファイルの両方に同じ値があることを照合ツールで確認しています。H100時代の計測とB300のweight sync計測は別クラスタで取ったもので、この照合の対象外です。記事の後半では、自分の結論を2つ撤回した経緯も書いています。
:::
...
| 3. training-inference mismatch | lossやrewardには見えない（mis_klを計測しない限り無症状） | H200 + p5(H100)で3 seed実測（本記事の主役） |
...
第二に**規模軸** （A2とB1の比較）です。30B（300億パラメータ）のMoE（Mixture of Experts）モデルは、4Bモデルの**約34倍**の時間（10.1秒）がかかります。ただしこの比較はパラメータ量だけでなく、denseからMoEへのアーキテクチャの変化（MoE側は`--sglang-moe-runner-backend triton`が必須という制約も伴います）も同時に動いているので、34倍をパラメータ量単独に帰属することはできません。それでも、規模とアーキテクチャが同時に大きくなるとweight syncが定性的に重くなるという方向性は実測できています。
...
*重要度比の分布。平均は1付近で無害に見えますが、裾の1トークンが勾配を支配します。TISとvetoはこの分布の別の場所に介入します（TIS/vetoの詳細は後述の「対処その1」「対処その2」で説明します）。*
...
ここが検証の核心的な結果です。KV fp8・学習率1e-5・TIS（重要度比にcapをかける対処法。後述の「対処その1」で説明します）なしの条件（collapse arm）で、25ステップにわたってmis_klとgrad_norm（勾配のノルム、勾配全体の大きさを表す指標）を追跡した実測データです。
...
別ハード・3 seed実験のseed 1234アームと同じ設定（fp8 e5m2、LR 1e-5、30 step）で、per-tokenダンプを崩壊の全行程にわたって取りました。`mis_kl`は0.0325から**31.78**まで上がり、そのseed 1234アームの最大値12.6を超えました。ダンプは1424ファイルです。

学習の進行で4分割し、各区間がどの`mis_kl`帯に対応するかを対応付けます。

| `mis_kl`帯 | 状態 | $\alpha_{\text{within}}$ | 傾き$|r|$ |
|---|---|---|---|
...
2. **小さければ補正はまだ不要ですが、測るのはやめないでください**。今回の検証では約 $6 \times 10^{-4}$ という値でしたが、これくらい小さければ良性の差であり、補正は不要です。ただしキラーチャートの崩壊アームもstep 8まではまさにこの同じ0.03前後の水準で静かに推移していました。「小さいから安全」ではなく「小さい間も監視を止めない」が正確な読み方です。この $10^{-3}$ 前後という水準は、推論エンジンをSGLangからvLLMに丸ごと入れ替えても同じオーダーにとどまりました。最適化された推論スタック同士が単にお互いに対して持ってしまう差の水準であり、良性かどうかを判断するときの目安として使えます。
