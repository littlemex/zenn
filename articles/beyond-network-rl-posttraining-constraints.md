...
そこで実際に測りました。ただし測ったのはtrainerとの比較ではなく、**推論エンジン同士の比較**です。同じQwen3-4Bの重みに対して、SGLangで32問（応答8192トークン）をサンプリングし、**まったく同じトークン列**をvLLMにteacher-forcingで通してtoken毎のlogprobを取り、slimeの`mis_kl`と同じ推定量を適用しました。trainerを介在させないので、「エンジン間の数値経路差そのもの」を分離して測れます。
...
| **平均 ± 標準偏差** | **0.000982 ± 0.000570** | **0.01206 ± 0.00065** | **0.000835 ± 0.000107** |

結果は予想と違いました。エンジン間の差（k3_kl $8.35 \times 10^{-4}$）は、SGLang対Megatronのベースライン（mis_kl 約 $6 \times 10^{-4}$）と**同じオーダー**にとどまりました。3 seedでの変動係数は13パーセントで、崩壊の倍率やfork間の倍率よりもはるかに安定しています。
...
:::message
この測定にはいくつか限界があります。単一モデル（dense 4B）であり、seedあたり32問・8192トークンです。またvLLMはこの環境ではeagerモード（`enforce_eager=True`かつ`TORCHDYNAMO_DISABLE=1`）で動かす必要がありました。ノードのCUDAドライバ13.1に対して、vLLM 0.11.0が固定するtorch 2.8/triton 3.4の組み合わせがコンパイルできなかったためです。eager実行はカーネル選択を変えるので、これは「SGLang対vLLM（eagerモード）」の比較です。ただしconcordanceの主張にとってはむしろ保守的な条件です。カーネル経路が違うにもかかわらず、両エンジンは $10^{-3}$ 程度で一致したことになります。
:::

## 追加検証: 実行済みと、これからの課題

登壇に向けた追加検証は、GPUインスタンス2ノード（当初p5enを想定し、実際にはp5 / H100 x8 x2のキャパシティブロックを確保）で進めました。それぞれの実験にはE1、E3、E4、E5、E7という識別番号を振っています（E2とE6は計画段階で検討したものの実施しないことに決めた項目です）。このうちE1、E4、E5は本記事の執筆時点で**実行済み**で、その結果は既に上の各節に織り込んであります。
...
- **E5** （実行済み）: SGLangとvLLMに同一のトークン列を通し、推論エンジン同士のlogprob一致度を3 seedで測りました。結果は「vLLMだと差がもっと大きくなるのか」の節に反映済みで、エンジン間の差がSGLang対Megatronのベースラインと同じオーダー（k3_kl $8.35 \times 10^{-4} \pm 1.07 \times 10^{-4}$）にとどまることを示しました。事前の機構的推論とは逆の結果です。
...
:::message
E1・E4・E5は実行済みで、本文の該当節に実測として反映しています。E3・E7はこの記事の執筆時点では未実施で、今後の課題です。
:::
...
最後に、この検証の誠実な限界を改めて確認しておきます。崩壊と救済の**方向性**については、collapse / rescueを3 seed（1234 / 42 / 123）で30ステップ完走させ、「no correctionは必ず崩壊し、TISは必ずmis_kl約0.15で救済される」ことを確認しました。per-kernelの帰属も、KV量子化精度がmismatchを単調に駆動することを実測しました。推論エンジン間の一致度も3 seedで測り、SGLangとvLLMの差がベースラインと同じオーダーにとどまることを確認しました。一方で残る限界もあります。fork間concordanceの倍率（54倍対49倍）の差そのものは依然として単一seedの点推定であり、その分散は評価していません。KV fp8による増幅は研究のための人工的な手法であり、実運用の設定ではありません。検証したモデル構成はdense 4Bという単一の構成にとどまっています。milesはslimeのフォークであり、独立した第二の実装ではありません。応答長の軌跡、30B MoEでの検証（E7）、CPU/GILの実測（E3）は、いずれも今後の追試課題として残されています。
