...
3. **training-inference mismatch** （数値のずれ）: 計測しても見えません。崩壊するまで無症状です（H200/p5enで実測、本記事の主役です）
...
| 3. training-inference mismatch | 計測しても見えない（崩壊するまで無症状） | H200/p5enで実測（本記事の主役） |
...
:::message alert
ここで正直な開示をします。各要因の寄与を分離するper-kernel ablation（1つずつ要因を切り分ける実験）は**実施していません**。これは機構としての仮説であり、計測結果ではありません。ちょうどこれは、今後実施予定のキャパシティブロック実験（後述するE4。キャパシティブロックはAWSがGPUインスタンスを一定期間まとめて予約できる仕組みで、この記事の末尾で紹介する追加検証はこの仕組みを使って予約したものです）で詰められる領域です。各フラグを単独で切り替えて、学習なしでstep-0のmis_klを測るだけなので、コストは低く済みます。
:::
...
「因果」という言葉はここでは慎重に扱います。この単一シード（1回だけの実行）での順序（mismatchが先に発散し崩壊が後を追う）は、mis_klが「先行指標」として使えることを示すものです。しかし「capが崩壊を直す」という因果関係の証拠は、この節のデータからではなく、次のセクションで説明するcapの値だけを変えた介入実験（3アーム対照実験）から得られます。
...
:::message alert
誠実な限界を明記します。milesはslimeのフォークであり、ゼロから作られた独立した第二の実装ではありません。したがってこれは「fork間の一致」であり、真に独立した2つの実装での再現ではありません。また、この結果は単一seed（1回の乱数シード）によるものです。54倍対49倍という差そのものについて、n=1（1サンプルのみ）の状態で何かを論じることはしません。
:::
...
これは仮定の話ではありません。実際にp5enインスタンス2ノードを48時間予約済みで、以下がその実行計画です。前半24時間と後半24時間に分けています。それぞれの実験にはE1、E3、E4、E5、E7という識別番号を振っています（E2とE6は計画段階で検討したものの実施しないことに決めた項目で、番号に欠番がありますが記事中で扱う実験は以下の5つですべてです）。

**前半24時間: 最大のギャップを埋める**

- **E1** （最優先）: collapse実験とrescue実験を3つの異なるseed（乱数の初期値）で実行し、誤差棒（エラーバー）を得ます。これにより「n=1（1サンプルのみ）批判」に対する回答になります。
- **E4**: 各バックエンドフラグを単独で切り替えて、step-0のmis_klを測ります。学習を必要としないため、E1と並行して安価に実行できます。「人工的に増幅しただけではないか」という疑問への、per-kernel（カーネル単位）の帰属による回答です。
...
- 余裕があれば、再撮り用のバッファ時間も確保します。

:::message
これらは計画された実行であり、キャパシティは確保済みですが、**まだ実行されていません**。この記事の執筆時点ではすべて未実行です。
:::
...
最後に、この検証の誠実な限界を改めて確認しておきます。今回の実測はすべて単一seed（1回の乱数シード）によるものであり、複数seedでの再現性はまだ確認できていません。KV fp8による増幅は研究のための人工的な手法であり、実運用の設定ではありません。検証したモデル構成はdense 4Bという単一の構成にとどまっています。milesはslimeのフォークであり、独立した第二の実装ではありません。per-kernelの帰属、応答長の軌跡、vLLMとの直接比較は、いずれも今後の追試課題として残されています。
