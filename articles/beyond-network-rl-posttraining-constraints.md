...
最後に、この検証の誠実な限界を改めて確認しておきます。崩壊と救済の**方向性**については、collapse / rescueを3 seed（1234 / 42 / 123）で30ステップ完走させ、「no correctionは必ず崩壊し、TISは必ずmis_kl約0.15で救済される」ことを確認しました。per-kernelの帰属も、KV量子化精度がmismatchを単調に駆動することを実測しました。推論エンジン間の一致度も3 seedで測り、SGLangとvLLMの差がベースラインと同じオーダーにとどまること、測る向きを逆にしても値が変わらないこと、モデルサイズを1.7Bから8Bまで変えても1.24倍しか動かないことを確認しました。一方で残る限界もあります。fork間concordanceの倍率（54倍対49倍）の差そのものは依然として単一seedの点推定であり、その分散は評価していません。KV fp8による増幅は研究のための人工的な手法であり、実運用の設定ではありません。学習を伴う崩壊・救済の実験はdense 4Bのみで、エンジン間の一致度についても検証したのはdense構成に限られます（MoEは未検証）。milesはslimeのフォークであり、独立した第二の実装ではありません。応答長の軌跡、30B MoEでの検証（E7）、CPU/GILの実測（E3）は、いずれも今後の追試課題として残されています。
