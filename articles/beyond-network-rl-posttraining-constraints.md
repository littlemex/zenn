...
両方とも、baseline（bf16）のmis_klは約 $6 \times 10^{-4}$ で、step 0でのKV fp8増幅後は約 $3 \times 10^{-2}$ に達します。なおこのfork間比較はH100クラスタでの計測で、後半で紹介するH200での照合ツールの対象外です（キャパシティブロック終了で当時の共有ストレージが残っていません）。倍率にすると、slimeで約54倍、miles で約49倍です。2つのフレームワークで、ほぼ同じ数値が観測されました。
...
第一に、$\alpha_{\text{within}}$ が何を分離しているかを正確に述べる必要があります。隣接差分が厳密に消すのは**系列ごとの一定オフセット**だけです。系列ごとに傾きの違う線形トレンド $r_t = b_j t$ の場合、隣接差はどこでも定数 $-b_j$ になるので $D_k = -b_j k/2$ となり、$\mathrm{Var}(b_j) k^2/4$ が残ります。つまりトレンドは消えず、半分残ります。
