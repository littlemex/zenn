...
3. **training-inference mismatch** （数値のずれ）: lossやrewardには見えません。mis_klという専用の指標を計測しない限り無症状です（H100とH200の両方で実測、3 seed実測はH200。本記事の主役です）
...
H200での実測値は、学習フレームワークが書いたTensorBoardのイベントファイルと、実験ドライバが書いた結果ファイルの両方に同じ値があることを照合ツールで確認しています。H100時代の計測は別クラスタで取ったもので、この照合の対象外です。記事の後半では、自分の結論を2つ撤回した経緯も書いています。
:::
...
| 3. training-inference mismatch | lossやrewardには見えない（mis_klを計測しない限り無症状） | H100とH200の両方で実測、3 seed実測はH200（本記事の主役） |
...
ここからはH200 x8のクラスタでの実測です（前の節はH100でした。ハードウェアを切り替えた理由は後述します）。ここが検証の核心的な結果です。KV fp8・学習率1e-5・TIS（重要度比にcapをかける対処法。後述の「対処その1」で説明します）なしの条件（collapse arm）で、25ステップにわたってmis_klとgrad_norm（勾配のノルム、勾配全体の大きさを表す指標）を追跡した実測データです。
...
### 3 seedでの確証: reward崩壊まで追い切る

上の観察を、同じH200 x8のEKSクラスタ上で、3つの異なるseed（1234 / 42 / 123）で追試しました。前倒しの度合いを揃えるため、ここでもQwen3-4B dense、colocated、KV fp8_e5m2、学習率1e-5という同じ設定を使い、no correction（補正なし、collapse）とTIS cap=2.0（rescue）を、同じseedのもとで**TIS補正の有無だけを変えて**並走させています。今度はログをstep 19で止めず、30ステップ最後まで、しかもmis_kl・grad_normに加えて**reward（報酬）**まで追い切りました。
...
条件は崩壊アーム（seed 1234）と**dtype以外すべて同一**です。bf16 KV、学習率1e-5、TISなし、30ステップ、同じseed。
...
両方とも、baseline（bf16）のmis_klは約 $6 \times 10^{-4}$ で、step 0でのKV fp8増幅後は約 $3 \times 10^{-2}$ に達します（前掲のH100での実測です）。倍率にすると、slimeで約54倍、miles で約49倍です。2つのフレームワークで、ほぼ同じ数値が観測されました。

このことは、この差の大きさが「共有されているrollout対trainerの数値経路（計算の仕方の違い）」によって決まっており、フォークごとに分岐したコードパスに由来するものではないことを示唆しています。

この一致は、別のハードウェア（p5en / H200 x8のEKSクラスタ）でも再確認しました。slimeとmilesはベースイメージが異なり（slimeはNGCベース、milesはnvidia/cudaベース）、Megatronのフォークも別で、torch_distチェックポイントもそれぞれのMegatronで別々に変換しています。それでも、このEKSクラスタで測ったbaseline（bf16）のstep-0 mis_klは、slimeが0.000615、milesが0.00072と、やはり同じオーダーに収まりました。共有する数値経路が差を決めているという見立ては、H100に加えてこのH200クラスタでも支持されています。
...
結果は予想と違いました。エンジン間の差（k3_kl $8.06 \times 10^{-4}$）は、SGLang対Megatronのベースライン（mis_kl 約 $6 \times 10^{-4}$）と**同じオーダー**にとどまりました。しかもこの量はとても安定しています。3 seedでの変動係数はわずか2.5パーセントで、崩壊時のmis_klピーク値（seed間で2.85から13.7までばらつく）やfork間の倍率よりも桁違いに締まった量です。
...
3 seed実験のseed 1234アームと同じ設定（fp8 e5m2、LR 1e-5、30 step）で、per-tokenダンプを崩壊の全行程にわたって取りました。`mis_kl`は0.0325から**31.78**まで上がり、そのseed 1234アームの最大値12.6を超えました。ダンプは1424ファイルです。
...
**第一に、崩壊すると応答長が動きます。** 中央値が8048 → 3961 → 4000 → 6118と移動し、打ち切り率も0.562 → 0.117 → 0.953と動きます。$\alpha$はprefix長の関数なので、区間ごとに違う長さで測ると長さの効果を構造の変化と誤認します。全区間が到達する2048に固定して測り直したのが上の表です。固定しない場合は0.95 / 1.70 / 3.14 / 1.75で、最終区間だけ下がります。**固定後のほうが単調になる**ので、この「下がり」は長さのartefactでした。
...
この線引きは、本記事の他の数値にも適用すべきです。別のキャリブレーション実験として、H100とH200で全く同一の構成（Qwen3-4B dense、TP1、LR 1e-6、bf16、seed 1234）を1つのseedだけで比べたところ、mis_klは0.000632（H100）と0.000627（H200）で0.8%差でした。ハード非依存の証拠として使えそうに見えますが、**0.8%はseed CVの7.8%より小さいです。** 正確には「ハードによる差はseedによるばらつきより小さい」と述べるべきで、これは主張としては弱まりますが、実務上の含意（ハードを変えてもmis_klの水準で驚くことはない）は変わりません。並列構成を振っても1.21倍の範囲に収まるという結果も同様で、この1.21倍はseed分散の1.19から1.27倍とほぼ同じです。並列構成がmis_klを動かさないという結論は保たれますが、「1.21倍」という具体的な数字をseed分散と区別して語ることはできません。
...
- **E1** （最優先）: collapse実験とrescue実験を3つのseed（1234 / 42 / 123）で30ステップ完走させました。結果はキラーチャートの節の「3 seedでの確証」に反映済みです。「n=1批判」への回答になっています。
