---
title: "Agentic AI セキュリティツール比較：garak と Snyk Evo の ATFAA/SHIELD 対応調査"
emoji: "🛡️"
type: "tech"
...
Snyk は Developer Security Platform を提供する商用 SaaS であり、2025 年 10 月 22 日に [Evo by Snyk](https://snyk.io/news/snyk-launches-evo/) をリリースした。Evo は世界初の Agentic Security Orchestration System として位置づけられており、OODA ループ（Observe-Orient-Decide-Act）フレームワークを採用することで AI-native アプリケーションのセキュリティを包括的に管理する。
...
Snyk Evo は Kubernetes Network Policies 統合、API ゲートウェイ制御、マイクロセグメンテーション推奨設定、クラウドネイティブセキュリティ統合（AWS Security Groups 等）により、包括的なセグメンテーション機能を提供する。
...
両ツールの強みを活かした統合セキュリティアーキテクチャの概観を整理してみる。実際にこのアーキテクチャを構築して試したわけではない。多層防御の観点でこれまでの既存セキュリティ対策に Agentic AI 特有の不確定性に対する対策レイヤとして開発フェーズで garak、実運用フェーズで snyk や Amazon Bedrock Guardrails などを併用するのが良さそうである。
