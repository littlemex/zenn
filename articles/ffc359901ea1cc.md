...
flowchart TB
    ...
    HF -->|"forward()"| BRIDGE
    XTTS_IMPL -->|"store_prefix_emb():<br>prefill_app()"| PREFILL
...
| Neuron 推論実行 | 前編で実装した `NeuronApplicationXTTSv2GPT`（`prefill_app` / `decode_app` を束ねるコーディネーター） | 不要 |
| ブリッジ | `NeuronGPT2InferenceModel`（本実装サンプル） | **必要** |

前編で実装した各 Application クラス（`decode_app` 等）が `forward()` → `self.traced_model()` を通じて `NeuronGPTTransformer` を NeuronCore 上で実行し、aliases 機構により KV-Cache をデバイス上で自動更新します。ブリッジ層はこの `decode_app` を呼びます。
...
sequenceDiagram
    ...
    loop EOS まで繰り返し
        ...
        alt Prefill Bridge パス（初回、cached_prefix_emb あり）
            ...
            GPT->>GPT: lm_head（CPU）→ logits [B, N, vocab_size]（generate() が末尾トークンを使用）
...
`store_prefix_emb()` は前編で解説した `prefill_app`（各 Application クラスの `forward()` 経由）を使い、話者の音声 prefix を一括処理して Neuron 上の KV-Cache を構築済みの状態にします。その後 `sync_kv_cache_prefill_to_decode()` で Prefill 側のキャッシュを Decode 側に転送します。
...
ループの大半を占める標準パスです。`mel_embedding` と `get_fixed_embedding()` で 1 トークン分の埋め込みを構築し、前編で解説した `decode_app` に渡します。Neuron から返ってきた hidden state に CPU 上の `lm_head`（`gpt_ln_f` → `final_norm` → `mel_head`）を適用して mel コードの logits を生成します。
...
sequenceDiagram
    ...
    B->>B: neuron_gpt_app.sync_kv_cache_prefill_to_decode(actual_len)
    Note over B,D: .state 直接アクセスパターン（前編 Section 7）<br/>prefill_state[rank][name] → .cpu() → padding 位置ゼロマスク → decode_state[rank][name].copy_()
...
| `is_prefill` | `True` | 推論状態の可視化と初期化確認用フラグ。`store_prefix_emb()` 呼び出し後に `True` を保持。`forward()` 内では直接参照されない。 |
...
:::message
手動での動作確認のためにあえてあまり実装を作り込んでいません。一撃で全て完了すると何が起こっているのかわからないためです。
:::
...
instance-id: i-0xxxxxxxxxxxxxxxxxxxx
+--------+--------+----------+--------+--------------+----------+------+
...
ln -s ~/samples/ml_distributed_experiment_collection/xttsv2-nxd-inference ~/nxd-inference-xttsv2
cd ~/nxd-inference-xttsv2
```
...
本記事では XTTSv2 を AWS Trainium 上の NxD Inference で動かすための実装について解説し、実際に E2E で精度と性能を検証しました。CPU から精度劣化させることなく性能を向上させることに成功しました！

:::message
ドチャクソ大変だった。ようやくおわったンゴ、トータル検証に 6 日もかかりましたにゃーん。そして良い結果が出てよかった！実際にゼロから実装してみると良いですが AI でも Neuron 特有の概念や Hugging Face とのインタフェース、Forward Override など多岐にわたる動かさないとわからないような部分に関して全くまともに実装してくれなかったので、分割統治的に整理しながら進めることでようやくうまく実装してくれるようになりました。色々なはまりどころがありまだ低レイヤ？ドメインコンテキストが複雑？な箇所では人間様の出番はありそうです。
:::

今後の展望としては、バッチサイズの拡張によるスループット向上、vllm-neuron への統合が考えられます。また、今回確立した「自己回帰デコーダを Prefill + Decode 分離アーキテクチャで Neuron に移植する」パターンは、同様の GPT ベースデコーダを持つ TTS モデルへの応用も可能なため、非常にシンプルな Config ファイルを準備するだけで類似のカスタムモデルを NxD Inference に対応させることがもしかすると可能かもしれません。
