...
ひとつ制約を挙げておきます。Secure Boot を有効にしている場合、カーネルはモジュールの署名も検証します。Ubuntu の既定の構成では、DKMS はビルドしたモジュールにホスト上で生成した鍵で署名しますが、その公開鍵を MOK として登録し、shim から信頼される状態にしなければ検証は通りません。Secure Boot を使う環境では、この鍵の運用を別途用意する必要があります。

## DKMS が動く瞬間はカーネルパッケージの導入処理の中です

肝心なのは、DKMS が動くタイミングです。カーネルのパッケージには導入後に走る処理が付いていて、その中から DKMS が呼ばれます。Ubuntu の場合、呼ばれる場所は 2 か所あり、カーネル本体側の `/etc/kernel/postinst.d/dkms` と、ヘッダ側の `/etc/kernel/header_postinst.d/dkms` です。後者は Ubuntu のパッケージが用意しているもので、他のディストリビューションには無いことがあります。ビルドには対象カーネルのヘッダが要るので、本体だけが先に入った時点ではビルドは走らず、ヘッダが入った時点で走ります。執筆時点の noble の `linux-aws` のように、本体とヘッダを両方引くメタパッケージを使っていれば、利用者が `apt install` を 1 回叩けば、その処理の内側でモジュールのビルドまで終わります。
...
利点の裏返しとして、注意すべき挙動があります。DKMS のビルドはカーネルパッケージの導入処理の内側で走るので、そこで失敗すると、失敗がカーネルパッケージ側に伝わります。実際に、当時のソースがまだ対応していなかった 7.0 系のカーネルを、対象を絞らない状態で導入したときの出力です。
...
測定条件を書いておきます。上の出力は Ubuntu 24.04 と dkms 3.0.11 の組み合わせで、`lustre-source` が 2.15.6-1fsx34 だった時点のものです。終了コードや対象外カーネルの扱いは dkms のバージョンに依存します。
...
裏を返せば、ノードを入れ替えて更新する運用にとって、DKMS はビルド環境という不要な荷物を本番ノードに残すだけです。また、自分でビルドしたモジュールで問題が起きた場合、配布されているパッケージでの切り分けが必要になると筆者は考えています。サポート契約を前提とする環境では、1 段目の選択から検討してください。

無人更新を有効にしている場合も注意が要ります。DKMS のビルドはカーネル導入処理の内側で走るので、夜間の自動更新の中で重いコンパイルが走り、失敗しても誰も見ていません。

# クライアントの公開状況を一次情報から整理します

「対象を絞る」ためには、どのカーネル系列が対応済みなのかを知る必要があります。ここは推測ではなく、リポジトリに置かれているパッケージそのものから読み取れます。以下の日付は、2026 年 9 月 12 日に索引を取得し、その時点で存在するパッケージの Last-Modified を系列ごとに集めて、最も古いものを取ったものです。Last-Modified は再アップロードで更新され、削除されたパッケージは索引に残らないので、初回公開日そのものではありません。取得時点で観測できた最も古い更新日として読んでください。同じ集計は次の手順で自分の環境でも取れます。
...
取得時点で確認できた区画は、16.04 以降の LTS 向けだけです。中間リリース向けの区画はひとつもありませんでした。過去に存在して削除されたものまでは索引から分からないので、方針として断定はできませんが、少なくとも今は LTS 向けだけが並んでいます。26.04 の区画は、この記事を書いた時点ではまだありません。コードネームとリリース日は Canonical が公開している [meta-release-lts](https://changelogs.ubuntu.com/meta-release-lts) で確認できます。

次に、OS のリリースから、その区画で観測できる最古のモジュールまでの間隔です。リリース日は同じ meta-release-lts の値です。
...
そして、区画ごとのカーネル系列と、その系列で観測できた最古の更新日です。

| 区画 | カーネル系列 | 観測できた最古の更新日 | 索引にあるパッケージ数 |
| --- | --- | --- | --- |
...
ふたつめは、Ubuntu で利用できる系列がすべて対応されるわけではないことです。22.04 の 5.19 系と 24.04 の 6.11 系は、表に行がありません。取得時点の索引に、その系列向けのモジュールは存在しません。

みっつめは、系列に対応が入っても、その系列のすべてのリリースが対象になるわけではないことです。取得時点の索引で noble の 6.8 系に並んでいるモジュールは `6.8.0-1024` から `6.8.0-1057` までで、その範囲の外にも中にも欠けているリリースがあります。系列としては対応済みでも、動かしているリリースが対象外という状態は起こります。

以上から、運用として置きたい前提は「対応系列は変わる」というものになります。であれば、対応系列を書き留めるのではなく、リポジトリに問い合わせて決めるのが素直です。

# 対応をスクリプト 1 本にまとめました

ここまでの内容を、[リファレンス実装](https://github.com/littlemex/distributed-ai/tree/main/2026-09-10-fsx-lustre-client-kernel-abi) として 1 本のスクリプトにまとめました。ソースからビルドすることで、そのリリース向けのビルド済みパッケージが公開されていることへの依存を外し、DKMS への登録とカスタム AMI への焼き込みの両方を同じ入口から扱えるようにしています。モジュール自体は、ビルドした後も対象カーネルに依存したままです。
...
ビルドが失敗したときの調査先も書いておきます。DKMS はビルドログを `/var/lib/dkms/lustre-client-modules/<バージョン>/build/make.log` に残すので、コンパイルエラーの内容はそこで読めます。カーネルパッケージが `half-configured` で残っている場合、`dpkg --configure -a` は同じ処理をやり直すだけなので、失敗の原因を取り除くか `dkms remove -m lustre-client-modules -v <バージョン> --all` で登録を外してから実行します。
