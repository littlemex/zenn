...
> 厳密には Localf、Global の定義は画一的なものではなく構成の仕方によって変わります。
...
Ring All-reduce は効率的な勾配同期を実現するアルゴリズムです。Preferred Networks の実験では、独自実装で NCCL と同等の性能（1.6% の差）を達成しています。
...
具体例で帯域幅の重要性を考えてみましょう。以下の表は 8 ノード（64 GPU）での Llama 70B 訓練時の通信時間比較です。

| ネットワーク | 帯域幅 | 通信時間/iteration | 1000 steps での累積 |
|------------|--------|-------------------|-------------------|
| EFA v1 (P4d) | 400 Gbps | 4.9 秒 | 1.36 時間 |
| EFA v2 (P5) | 3,200 Gbps | 0.61 秒 | 0.17 時間 |
| EFA v4 (P6-B300) | 6,400 Gbps | 0.31 秒 | 0.09 時間 |

**帯域幅 8 倍 → 通信時間 1/8 → 訓練全体の効率大幅向上**
...
::::details PyTorch FSDP での実測性能向上

**T5 3B モデルでの最適化効果（A100 GPU）**
- Transformer wrapping policy: 2x 向上
- アクティベーション チェックポイント: 10x 向上（バッチサイズ増加による）
- 混合精度（BFloat16 vs FP32）: 5x 向上
- FSDP vs DDP: 1.5x 向上

**T5 11B モデルでの最適化効果**
- 混合精度: 10x 向上（3B モデルより大きな効果）
- アクティベーション チェックポイント: 100x 向上
- バッチサイズ: 3 → 50（16 倍以上の改善）

**大規模モデルでの実績**
- PyTorch FSDP + AWS での 1T パラメータモデル訓練を 512 GPU で実現
- FSDP により DDP と比較して約 4 倍大きなモデルを同一リソースで訓練可能
- アクティベーション チェックポイントとオフロードにより 20 倍までスケール可能

**参考**: [Efficient Large-Scale Training with PyTorch FSDP and AWS](https://pytorch.org/blog/efficient-large-scale-training-with-pytorch/)
::::

### 重要な性能指標

ネットワーク性能を評価する際には 3 つの主要な指標を理解する必要があります。まず帯域幅（Bandwidth）は 1 秒間に送信できるデータ量を示し、GPU 間通信の速度を決定します。次にホップ数（Hop Count）はデータが目的地に到達するまでに通過する中継点の数を表し、通信レイテンシに直結します。最後に Bisection Bandwidth はネットワークを半分に分割した際の境界部分での総帯域幅を示し、大規模な All-reduce 操作などでシステム全体の性能上限を決定する重要な指標となります。

::::details 各指標の詳細

**Bisection Bandwidth の計算例**
- DGX H100: 8 GPU を 4+4 に分割
- 各 GPU: 900GB/s の NVLink 帯域幅
- Bisection Bandwidth: 3.6TB/s（4 GPU × 900GB/s）
- これが All-reduce 操作の理論的限界を決定
...
**トポロジー別の比較**
- Full-mesh（NVSwitch）: 理論上最大の bisection bandwidth
- Ring: シンプルだが帯域幅制限あり
- Tree: 階層的だがルートがボトルネック
...
この表から重要な観察結果が得られます。帯域幅には顕著な非対称性が存在し、Local 接続は Global 接続の約 2 倍の帯域幅を GPU あたりで提供します。レイテンシにおいても大きな差があり、Local 接続はサブマイクロ秒レベルであるのに対し、Global 接続は数マイクロ秒を要します。このような性能差が生じる理由として、より高速な Global 接続の実現にはコストとケーブル長という物理的制約が存在し、経済的・技術的な観点から現実的な実装が困難であることが挙げられます。
::::
...
ストレージ選択には 3 つの重要な考慮事項があります。

第 1 にプロトコルです。既存のツールやアプリケーションとの互換性を保つため、データアクセス方法の継続性が重要になります。多くの組織では特定の方法でデータにアクセスするように構築された既存の環境があり、新しいワークロードでも同じアクセス方法を継続できることが求められます。

第 2 にスループットです。これは 1 秒間に読み書きできるデータ量で測定され、高性能ワークロードでは GB/秒や TB/秒単位になります。小さなファイルを多く扱うワークロードでは、1 秒あたりのトランザクション数（TPS）も重要な指標となります。

第 3 にレイテンシーです。これはストレージへの小さな操作のラウンドトリップ時間を指します。Amazon S3 では「ファーストバイトレイテンシー」、つまりデータ提供開始までの時間が重要な指標となります。レイテンシーはワークロードの同時実行性に影響を与え、特にエンドユーザーが関与するケースでは応答性を決定する重要な要因となります。

### AWS ストレージオプション

#### Amazon S3（オブジェクトストレージ）

Amazon S3 は 18 年の実績を持つデータレイク向けオブジェクトストレージの事実上の標準です。実質無制限のスケーラビリティと 11 ナインの耐久性を提供します。
