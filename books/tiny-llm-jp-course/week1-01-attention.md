...
### axis=-1 を理解する：形状から考える

`axis=-1` は「最後の次元」を意味しますが、なぜこれが重要なのでしょうか。形状から理解しましょう。

**scores の形状と axis の関係**

```python
scores.shape = (..., L_query, L_key)
                      ↑        ↑
                      │        └─ axis=-1（最後の次元）
                      └────────── axis=-2（最後から 2 番目）
```

`axis=-1` を指定すると、最後の次元（L_key）に沿って正規化されます。結果として、各クエリについて「全キーへの注目度の合計が 1」になります。

**具体例で理解する**

3 つのクエリと 3 つのキーがある場合を考えます。

```python
scores = [
  [0.5, 0.8, 0.2],  ← クエリ 1 が各キーに対するスコア
  [0.3, 0.9, 0.4],  ← クエリ 2 が各キーに対するスコア
  [0.7, 0.1, 0.6]   ← クエリ 3 が各キーに対するスコア
]
# 形状: (3, 3) つまり (L_query=3, L_key=3)
```
...
# 各行（クエリ）について、列方向（キー方向）に正規化
# → 横方向に Softmax を適用

結果 = [
  [0.27, 0.49, 0.24],  ← 合計 = 1.0 ✓
  [0.21, 0.54, 0.25],  ← 合計 = 1.0 ✓
  [0.46, 0.14, 0.40]   ← 合計 = 1.0 ✓
]
```

意味：「クエリ 1 は、キー 1 に 27%、キー 2 に 49%、キー 3 に 24% 注目する」

これが Attention の正しい動作です。各クエリが、どのキーにどれだけ注目するかを決定します。

**axis=-2 の場合（間違い）**

```python
softmax(scores, axis=-2)
# 各列（キー）について、行方向（クエリ方向）に正規化
# → 縦方向に Softmax を適用

結果 = [
  [0.33, 0.48, 0.19],  ↑
  [0.24, 0.49, 0.27],  ↑ 各列の合計 = 1.0
  [0.43, 0.03, 0.54]   ↑
]
```

意味：「キー 1 は、クエリ 1 から 33%、クエリ 2 から 24%、クエリ 3 から 43% 注目される」

これは Attention の逆で、意味がありません。
...
MultiHeadAttention の実装で最も分かりづらい点は、**数式上の表現と実際の実装が大きく異なる**ことです。ここでは、この「ギャップ」を段階的に説明します。

#### 数式での表現（概念的）

論文や PyTorch ドキュメントの数式：
...
for i in range(num_heads):
    # 各ヘッド用に個別の重み行列
    Q_i = Q @ W_i_Q  # ヘッド i 用の Query
    K_i = K @ W_i_K  # ヘッド i 用の Key
    V_i = V @ W_i_V  # ヘッド i 用の Value
...
この方法は分かりやすいですが、**ループが遅く、GPU の並列処理を活用できません**。

#### 実装での工夫（効率的）

実際の実装では、全ヘッドの重みを 1 つの大きな行列にまとめ、一度に計算します。

```python
# 効率的な実装
# 全ヘッド用の重みを 1 つにまとめる
W^Q = [W_1^Q | W_2^Q | ... | W_h^Q]  # 横に連結
# 形状: (E, H*D)

# 一度に全ヘッド分を投影
Q_all = Q @ W^Q.T  # 形状: (N, L, H*D)
```

この `H*D` 次元のベクトルには、実は `H` 個のヘッドの情報が**隠れて**います。

```python
# Q_all の内部構造（概念的）
Q_all = [ヘッド0の次元, ヘッド1の次元, ..., ヘッドH-1の次元]
#        ↑D個       ↑D個              ↑D個
#        |← 合計 H*D 次元 →|
```

#### reshape の役割：隠れた構造を顕在化

reshape は、この隠れていたヘッド構造を明示的に表現します。

```python
# reshape 前: (N, L, H*D)
Q_all = [...ヘッド0...ヘッド1...ヘッド2...]
#           混ざっている（区別がつかない）
...
#### transpose の役割：Attention 関数への適合

`scaled_dot_product_attention_simple` は最後の 2 次元 `(L, D)` に対して動作します。しかし reshape 直後の形状は `(N, L, H, D)` で、最後の 2 次元は `(H, D)` です。

```python
# reshape 直後: (N, L, H, D)
# Attention 関数は最後の2次元を処理
# → (H, D) に対して計算してしまう
# → これは間違い！ヘッド間の関係を見ることになる
```

transpose により、ヘッドをバッチ次元の直後に移動します。
...
データ構造 = [
  バッチ0: [
    トークン0: [ヘッド0, ヘッド1, ヘッド2, ヘッド3],
    トークン1: [ヘッド0, ヘッド1, ヘッド2, ヘッド3],
    トークン2: [ヘッド0, ヘッド1, ヘッド2, ヘッド3]
  ]
]
# トークンごとに全ヘッドがまとまっている
...
データ構造 = [
  バッチ0: [
    ヘッド0: [トークン0, トークン1, トークン2],
    ヘッド1: [トークン0, トークン1, トークン2],
    ヘッド2: [トークン0, トークン1, トークン2],
    ヘッド3: [トークン0, トークン1, トークン2]
  ]
]
# ヘッドごとに全トークンがまとまっている
# → 各ヘッドが独立してトークン間の関係を計算できる
```
...
```python
# 1. 効率的な投影（全ヘッドを一度に）
Q_all = Q @ W^Q.T  # (N, L, E) → (N, L, H*D)

# 2. reshape でヘッド構造を明示
Q_all = Q_all.reshape(N, L, H, D)  # (N, L, H*D) → (N, L, H, D)

# 3. transpose でヘッドをバッチ次元に
Q_all = Q_all.transpose(0, 2, 1, 3)  # (N, L, H, D) → (N, H, L, D)

# 4. アテンション計算（各ヘッドが独立に処理される）
output = Attention(Q_all, K_all, V_all)  # (N, H, L, D)

# 5. transpose で元の順序に戻す
output = output.transpose(0, 2, 1, 3)  # (N, H, L, D) → (N, L, H, D)

# 6. reshape でヘッドを結合（Concat に相当）
output = output.reshape(N, L, H*D)  # (N, L, H, D) → (N, L, H*D)

# 7. 出力投影
output = output @ W^O.T  # (N, L, H*D) → (N, L, E)
```

この一連の処理により、**数式の Concat(head1...headh) を明示的に実行することなく、同じ結果を効率的に得られます**。
...
`linear` 関数は内部で重み行列を転置 `.T` します。そのため、この関数を使う場合は自分で `.T` を書く必要はありません。
