...
`terraform apply` が完了すると、`accelerator_pools` に追加した trn2 プールの `has_neuron_pool` が `true` になったことを検知して、Neuron device plugin（と `neuron_enable_scheduler = true` にしていれば Scheduler Extension）が導入されます。EFA device plugin もすでに `aws.amazon.com/neuron` taint を tolerate する設定で導入済みのため、追加の変更は不要です。

Neuron probe pod を投入すると、Pod が `Running` になった時点で「AMI にドライバが載っている」「device plugin がリソースを advertise している」「Pod がそのリソースを要求してスケジュールされる」という一連の流れが単一ノードでは通っていることが確認できます。具体的な確認コマンドは後述の「ワークショップ実施」で扱います。
