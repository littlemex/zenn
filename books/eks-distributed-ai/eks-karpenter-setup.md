...
`terraform apply` 後、`karpenter` namespace にコントローラ Pod が `Running` で立ちます。CRD は `karpenter-crd` chart によって別リリースとして登録され、`ec2nodeclasses` / `nodeclaims` / `nodepools` の 3 つが確認できます。この時点では NodePool を 1 つも定義していないため、ノードは前章の System ノードのみで、GPU/Neuron ノードは増えません。これが demand-driven の動作確認になります。具体的なコマンドは後述の「ワークショップ実施」で扱います。
