...
この book は、NVIDIA GPU や AWS Trainium/Inferentia（Neuron）を使った分散学習・推論の実験を、Amazon EKS 上で回すための基盤を Terraform で構築するワークショップです。Amazon VPC・EKS・Karpenter といった土台から始めて、`accelerator_pools` によるアクセラレータノードの動的プロビジョニング、EFA によるマルチノード通信、Capacity Block の取得、共有ストレージ、安全な破棄までを順に扱います。
...
- Terraform による Amazon EKS クラスタ + Karpenter の構築（Basic01, 02）
...
- AWS Trainium/Inferentia（Neuron）対応の設計（Basic07）
- 共有ストレージ Amazon EFS / Amazon FSx for Lustre の使い分け（Basic08）
...
:::message alert
この book が構築する Amazon EKS クラスタ・GPU/Neuron ノード・NAT ゲートウェイ・Amazon FSx などは、起動している間 AWS 利用料金が発生します。特に GPU/Neuron インスタンスと Capacity Block は高額です。実験が終わったら Basic09 の手順で必ず破棄してください。
:::
