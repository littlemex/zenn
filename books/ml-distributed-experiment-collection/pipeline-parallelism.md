...
3. 全順伝播が完了したら、逆伝播を実行
4. 各マイクロバッチで計算された勾配を足し合わせ（**勾配蓄積: gradient accumulation**）、全マイクロバッチの処理完了後にまとめて重みを更新
...
:::message
**バブルサイズの構成要素**: GPipe では全マイクロバッチが順伝播を完了してから逆伝播を開始するため、F, B, W のすべてがパイプラインの依存関係に含まれ、バブル時間に寄与します。後述の 1F1B や Zero Bubble では、この依存関係を緩和してバブルを削減します。
:::
...
```
            バブル率               メモリ（アクティベーション数）
1F1B:       (p-1)(F+B+W) / m      p
ZB-H1:      (p-1)(F+B+W-W)/m      p
          = (p-1)(F+B) / m
ZB-H2:      (p-1)(F+B-2W) / m     約 2p
ZB-V:       (p-1)(F+B-W) / m      p（1F1B と同等）
          ≈ 0 (W を空き時間に配置)
