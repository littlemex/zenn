...
※ フェーズ区分線は GPU 0 の視点で記載しています。下位ステージ（GPU 1-3）
  では定常状態が延長され、ウォームアップで投入されなかったマイクロバッチの
  Forward が定常状態の一部として処理されます（例: GPU 1 の F7）。
```
...
※ ZB-H2 のバブル率: 論文では整数線形計画法（ILP）ソルバーを使って
  ...
  理論的にバブルをほぼゼロに近づけられます。

※ ZB-V のバブル率: F ≈ B ≈ 2W という典型的な条件下（例: SwiGLU を
  使った Transformer では W ≈ F/2）で、(p-1)(F+B-W) ≈ (p-1)F
  となり、バブルが大幅に削減されます。
```
