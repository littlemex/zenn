...
**マイクロバッチ（Microbatch）**: ミニバッチをさらに小さく分割したものです。パイプラインの並列度を高めるために使用されます。
...
- **B（dL/dx）**: 入力に対する勾配計算。次のステージの逆伝播に必要なため、ステージ間の依存関係がある
- **W（dL/dw）**: パラメータに対する勾配計算。他のステージに依存しないため、自由にスケジューリング可能
...
**ZB-V（バランス型）**:
- モデルを 2p 個のチャンクに分割し、各デバイスに V 字型に 2 チャンクを配置
...
※ バブル率の正確な表現は (p-1) * (該当する実行時間の合計) で、
  全体のコスト m * (F + B + W) で割った値
```
...
DualPipe の核心は、2 つの 1F1B スケジュールを逆方向に同時実行することです。
...
※ F&B は Forward と Backward が重なっている部分の実行時間
```

通常の 1F1B と比較して、ステージ数の係数が半分（p/2 - 1）になるため、バブルが大幅に削減されます。
...
通常の 1F1B では、Forward の結果が確定してから次のステージに送信します。Eager スケジューリングでは、**Forward の送信を早期に開始**し、次のステージでの受信と計算を重ねます。
...
| ZB-H2 | 約 0 | O(2p) | 低 | 高 |
| ZB-V | ほぼ 0 | O(p) | 中（2 倍） | 高 |
| DualPipe | 大幅削減 | O(2p) | 中 | 高 |
| Eager 1F1B | (p-1)/m（改善あり） | O(p) + alpha | 低 | 中 |
