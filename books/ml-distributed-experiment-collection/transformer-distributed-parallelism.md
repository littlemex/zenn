...
| 3 | TP 演算 | 完全なテンソル | QKV 射影、Attention、出力射影 | AllReduce（tp 内） | 各 GPU: $[B, S, D]$ |
...
[^8]: Liu & Abbeel, 2023 "Ring Attention with Blockwise Transformers for Near-Infinite Context"
...
1. **送信側（AllToAll 1 回目）**
   ...
   - 例: GPU 0 の「T0→E2」（トークン 0 がエキスパート 2 を選択）の場合
     - Expert 2,3 は GPU 1 が担当しているため、トークン 0 を GPU 1 に送信
     - 同様に、T1→E0（Expert 0 は GPU 0 担当）なので GPU 0 内で処理
     - T2→E5（Expert 4,5 は GPU 2 担当）なので GPU 2 に送信
     - T3→E1（Expert 0,1 は GPU 0 担当）なので GPU 0 内で処理

2. **受信側**
   - 各 GPU が自分の担当エキスパートに割り当てられたトークンを全 GPU から受信
   - 受信したトークンのみを処理（負荷分散）
   - 例: GPU 0 は Expert 0,1 担当なので、T1, T3, T6 などを受信
...
| Tensor Parallel (tp) | 隠れ次元 D | モデルメモリ削減 | AllReduce | 独立 |
...
| Vocab Parallel (vp) | 語彙次元 V | 埋め込み層メモリ削減 | AllReduce or ReduceScatter | tp と同じ[^5] |
