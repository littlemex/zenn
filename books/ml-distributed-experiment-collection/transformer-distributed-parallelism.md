...
この組み合わせにより、通信回数を最小化しています（2 層で通信は 1 回のみ）。
...
Tensor Parallel (tp) は重み行列を分割しますが、アクティベーション（中間出力）は各 GPU で完全なテンソル `[B, S, D]` を保持する必要があります。長いシーケンスでは、このアクティベーションメモリが大きな負担となります。
...
- 大規模モデル（LLaMA, GPT など）では語彙サイズが 32K～100K になるため、vp が有効
