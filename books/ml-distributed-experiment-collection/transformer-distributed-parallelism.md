...
上記の 4 つは集団通信操作ですが、Context Parallel では **P2P（Point-to-Point）通信**が使用されます。P2P 通信は 2 つの GPU 間で直接データを送受信する操作（Send/Recv）であり、Ring Attention のパイプライン処理で重要な役割を果たします。詳細は Context Parallel のセクションで説明します。
...
:::message
**注**: 純粋な Tensor Parallel のみの場合は AllReduce で結果を全 GPU に配布します。

Sequence Parallel と組み合わせる場合は、AllReduce を ReduceScatter + AllGather に分解し、ReduceScatter の結果（各 GPU が結果の一部のみを保持）をそのまま sp 形式 $[B, S/sp, D]$ として保持します。本記事では sp との組み合わせを前提に ReduceScatter と記載しています。
:::
...
| 3 | TP 演算 | 完全なテンソル | QKV 射影、Attention、出力射影 | ReduceScatter（tp 内） | 全 GPU: $[B, S, D]$ |
...
:::message
Attention 計算は $\text{softmax}(Q \times K^T / \sqrt{d}) \times V$ の形で行われますが、ここでは処理の流れに注力するため数式の詳細は省略しています。重要なのは、各 GPU が Ring 通信によって全シーケンスの Key/Value にアクセスし、自分の Query トークンについて完全な Attention を計算できる点です。
:::
...
| 2 | Lookup | 各 GPU の語彙範囲 | GPU 0: ID 0～V/4 の埋め込みを取得<br/>GPU 1: ID V/4～V/2 の埋め込みを取得<br/>該当しないトークンは 0 ベクトル | なし | 各 GPU: sparse な埋め込み $[B, S, D]$ |
| 3 | 集約 | sparse tensor | 各 GPU の結果を合計して分散 | **ReduceScatter** | 各 GPU: 完全な埋め込みの一部 |
...
:::message
**注**: 現代の分散訓練では、単純な dp ではなく ZeRO（DeepSpeed）や FSDP（PyTorch）がよく使用されます。これらはオプティマイザ状態や勾配をシャーディングすることでメモリ効率を大幅に改善します。ZeRO-3/FSDP を使用すると、tp なしでも 70B モデルを扱えるケースがあります。本記事では初学者向けにシンプルな dp として説明していますが、実務では ZeRO/FSDP の採用を検討してください。
:::
