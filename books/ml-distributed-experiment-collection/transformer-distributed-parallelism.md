...
Top-K ルーティング（各トークンが K 個のエキスパートを選択）では、人気のあるエキスパートに多くのトークンが集中し、他のエキスパートにはほとんど送られないことがあります。これは一部の GPU が過負荷、他の GPU がアイドル状態になるような利用率の不均衡を招きます。
...
出力層は埋め込み層の逆で、D 次元ベクトル → V 次元の logits（次トークンの確率分布）を計算します。
...
**重要なポイント**
- vp は tp と同じデバイスグループで動作（vp = tp）[^6]
- 語彙サイズ 32K～100K の大規模モデル（LLaMA[^4], GPT-3[^5]）で有効
...
語彙サイズが大きい場合、または埋め込み層のメモリが無視できない場合に **vp を追加**します。

- **理由**: 埋め込みテーブルと出力層の行列が巨大になるため、vp で分割
- **制約**: vp = tp（同じデバイスグループ）
- **目安**: 語彙サイズ 100K 以上、または大規模モデル（175B+）で埋め込み層がボトルネックになる場合
...
| Vocab Parallel (vp) | 語彙次元 V | AllReduce or ReduceScatter | 埋め込み層、損失計算 |
