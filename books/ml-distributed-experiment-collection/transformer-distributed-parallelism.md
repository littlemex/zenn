...
| **勾配** | 各パラメータを調整する方向と大きさを示す値。訓練時に逆伝播で計算され、モデルの重みを更新するために使用される |
| **アクティベーション** | 各層の順伝播で計算される中間結果。訓練時には逆伝播のために全層のアクティベーションをメモリに保持する必要があり、大きなメモリ負担となる |
| **要素単位演算** | 各トークンの処理が他のトークンに依存しない演算（LayerNorm, Dropout など）。シーケンスを分割したまま処理できる |
| **デバイスグループ** | 特定の並列化戦略に参加する GPU の集合。例えば tp=4 の場合、4 つの GPU が 1 つのデバイスグループを形成し、互いにテンソルの断片を通信する |
::::
...
:::message alert
本記事で扱う 6 つの並列化戦略は、Data Parallel、Tensor Parallel、Sequence Parallel、Context Parallel、Expert Parallel、Vocab Parallel です。なお、Pipeline Parallelism（モデルの層を複数の GPU に分割してパイプライン処理する手法）については次章で解説します。
:::

**本記事の視点 -- HPC 的な理解**: 本記事では、Transformer のアーキテクチャ詳細の深い理解は前提とせず、以下の観点から並列化を理解します。
...
:::message
**注**: Transformer の中間層では B, S, D の 3 次元で処理されますが、モデルの入口（埋め込み層）と出口（出力層）では、テキストのトークンと内部表現を変換するために**語彙次元 V**（語彙サイズ、Vocabulary size）が必要になります。V はモデルが扱える単語・トークンの総数（例: 32,000 個、100,000 個）を表します。詳細は「Vocab Parallel (vp)」セクションで説明します。
:::
...
| 黄色 | tp (Tensor Parallel) | 重み行列（D または中間次元） | AllReduce（sp 併用時は ReduceScatter + AllGather） |
...
:::message
**端的に言うと**: 同じモデルのコピーを各 GPU に持たせ、異なるデータで同時に訓練する。各 GPU の学習結果を統合し、全 GPU が同じモデルを維持する。
:::
...
簡略化のため B=1, S=1 とします。入力 $X = [[1, 2, 3, 4]]$ （形状 $[1, 4]$、D=4 に相当）と重み行列 $W1[4, 4]$ を考えます。これを 2 つの GPU で列方向に分割します。
...
graph TB
    ...
    GPU0 --> RS["AllReduce または ReduceScatter<br/>Y = Y0 + Y1"]
    GPU1 --> RS
...
:::message
入力が分割されているため、各 GPU が部分的な行列積を計算します。結果を **AllReduce**（tp 単独）または **ReduceScatter**（sp 併用時）で集約（合計）して完全な出力を得ます。
:::
...
:::message alert
多くの実装（Megatron-LM, NeMo など）では、効率上 sp は tp と**同じデバイスグループ・同じ並列度**で動作します（sp の並列度 = tp の並列度）[^2]。これは、tp の行列演算の前後で AllGather/ReduceScatter によりテンソル形状を切り替えるためです。
:::

:::message
**端的に言うと**: LayerNorm のような各トークン独立の演算では、シーケンスを分割したまま処理できる。行列積のように全シーケンスが必要な時だけ結合し、終わったら再分割してメモリを節約する。
:::
...
:::message
**端的に言うと**: MoE の各 Expert を異なる GPU に配置し、AllToAll でトークンをルーティングする、MoE 専用の並列化戦略です。
:::
...
Top-K ルーティング（各トークンが K 個のエキスパートを選択）では、人気のあるエキスパートに多くのトークンが集中し、他のエキスパートにはほとんど送られないことがあります。これは一部の GPU が過負荷になり、他の GPU がアイドル状態になるような利用率の不均衡を招きます。
...
AllToAll は **全対全通信** であり、GPU 数が増えると通信コストが急増します。各 GPU は他の全ての GPU とデータを送受信する必要があるため、通信パターンが複雑になり、ネットワーク負荷が GPU 数とともに増加します。例えば、ep=16 の場合、各 GPU が他の 15 個の GPU とデータを送受信します。このため、Expert Parallel では **ep = dp** とすることで、Data Parallel の既存デバイスグループを再利用し、通信コストを抑えるのが一般的です。
...
語彙サイズ V = 50,000、隠れ次元 D = 4,096 の場合、埋め込みテーブルは $[50000, 4096]$ の巨大な行列になり、float32 で約 780 MB のメモリを消費します。大規模モデル（V = 100,000 以上）ではさらに大きくなります。
::::

#### Vocab Parallel の動作

Vocab Parallel は、巨大な埋め込みテーブルを語彙次元 V で分割します。各 GPU が語彙の一部を担当します。

:::message
**端的に言うと**: 巨大な語彙テーブル [V, D] を語彙次元で分割し、各 GPU が担当範囲で計算後、AllReduce で合体する。

**具体例**: GPU 0 が「私」「は」、GPU 1 が「公園」「で」を担当。入力「私は公園で」→ 各 GPU が担当トークンを処理 → AllReduce で合体 → 完全な埋め込みベクトル。
:::
...
Vocab Parallel は、語彙サイズ V が大きい（数万～数十万）場合に、埋め込みテーブルと出力層の行列を分割します。
...
出力層では、各トークンの D 次元の隠れ表現を、V 次元の logits（各語彙トークンに対するスコア）に変換します。logits とは、softmax を適用する前の生のスコアのことです。この logits に softmax を適用すると、次のトークンの確率分布が得られます。例えば V=32,000 の場合、32,000 個のスコアが計算され、例えば最もスコアの高いトークンが次の予測トークンとなります。
...
1. 各 GPU が担当トークンのみ処理（範囲外は 0）
   GPU 0（語彙 0-3）: ID 1,2 を処理 → [[0.1,0.2], [0.0,0.0], [0.3,0.4], [0.0,0.0]]
   GPU 1（語彙 4-7）: ID 5,6 を処理 → [[0.0,0.0], [0.5,0.6], [0.0,0.0], [0.7,0.8]]

2. AllReduce で合算
   → [[0.1,0.2], [0.5,0.6], [0.3,0.4], [0.7,0.8]]（完全な埋め込み）
```

:::message
sp と組み合わせる場合は AllReduce の代わりに ReduceScatter を使い、各 GPU が結果の一部のみを保持します。
:::
...
6 つの並列化戦略を紹介しましたが、実際にはどの順序で適用すべきでしょうか。適用順序は、**メモリ制約**、**通信オーバーヘッド**、**利用可能な GPU 数**に基づいて決定するのが良いでしょう。あくまで目安のフローチャートであらゆるケースを想定しているものではありません。
...
| 順序 | 戦略 | いつ適用するか | 理由 | 制約 | 目安 |
|------|------|--------------|------|------|------|
| 1 | **Tensor Parallel (tp)** | モデルの重み行列が 1 GPU のメモリに収まらない場合 | tp はモデルの各層の重み行列を分割するため、単一の層内のモデルメモリを削減できる主要な戦略 | 通信オーバーヘッドが大きいため、tp=2, 4, 8 程度に抑える（単一ノード内推奨） | 70B パラメータ以上のモデルでは tp=4 または tp=8 が必要 |
| 2 | **Pipeline Parallel (pp)** | tp を適用してもメモリが不足する場合 | Transformer の層を GPU に分散させ、各 GPU が保持する層数を削減 | パイプラインバブルによる GPU アイドル時間が発生（効率 80-90%） | 100B パラメータ以上、または tp だけでは不足する場合。詳細は [GPipe 論文](https://arxiv.org/abs/1811.06965) や [PipeDream 論文](https://arxiv.org/abs/1806.03377) を参照 |
| 3 | **Data Parallel (dp)** | メモリ問題が解決した後、スループットを向上させる | 複数のバッチを並列処理し、訓練・推論のスループットを向上 | 勾配同期の通信コスト（AllReduce）があるが、tp/pp より効率的 | 利用可能な GPU 数に応じて dp を最大化 |
| 4 | **Sequence Parallel (sp)** | tp と組み合わせて、アクティベーションメモリを削減 | tp だけではアクティベーションが完全なテンソル $[B, S, D]$ を保持するため、sp で $[B, S/sp, D]$ に削減 | tp と同じデバイスグループで動作[^2] | 長シーケンス（4K トークン以上）で効果的 |
| 5 | **Context Parallel (cp)** | 128K トークン以上の超長シーケンスを扱う場合 | Attention の計算に必要なメモリ（Attention スコアや KV キャッシュ）が巨大になるため、cp でシーケンスを分割 | Ring Attention のような特殊な実装が必要 | 128K トークン以上のコンテキスト長 |
| 6 | **Vocab Parallel (vp)** | 語彙サイズが大きい場合、または埋め込み層のメモリが無視できない場合 | 埋め込みテーブルと出力層の行列が巨大になるため、vp で分割 | vp = tp（同じデバイスグループ） | 語彙サイズ 100K 以上、または大規模モデル（175B+）で埋め込み層がボトルネックになる場合 |
| 7 | **Expert Parallel (ep)** | MoE モデルを扱う場合 | エキスパート数が多い（8-64 個）ため、各 GPU がエキスパートの一部を担当 | AllToAll 通信のコストが高い | ep = dp として既存の dp グループを再利用 |

:::message
実際の分散訓練では、単純な dp ではなく ZeRO（DeepSpeed）や FSDP（PyTorch）がよく使用されます。これらはオプティマイザ状態や勾配をシャーディングすることでメモリ効率を大幅に改善します。ZeRO-3/FSDP を使用すると、tp なしでも 70B モデルを扱えるケースがあります。本記事では初学者向けにシンプルな dp として説明していますが、実務では ZeRO/FSDP の採用を検討してください。次回の発展編で Pipeline Parallel と合わせて解説します。
:::

::::details 具体例

あくまで考え方を示す際の一例なので実際とは異なる可能性があります。
...
**例 5: MoE モデル（8 エキスパート、モデル全体で約 11B 規模）**
```
構成: ep=8, tp=2 (合計 16 GPU)
理由: ep=8 で 8 個のエキスパートを各 GPU に分散配置、各エキスパートは tp=2 で分割
注: モデル構成は共有部分（Attention 層など）約 3B + エキスパート 8 x 1B = 約 11B
```
...
- **デバイスグループの制約**: sp, vp は tp と同じグループで動作[^2][^6]、ep は dp と同じグループを使用するため、それぞれ独立した GPU 数を増やさない
::::
...
| MoE           | o   | o*  |     |     | o   |     | AllToAll |
| Loss          | o   | o   |     |     |     | o   | AllReduce |

**注**: MoE の tp* は、各エキスパート内の FFN に tp を適用する場合に使用されます。
