...
emoji: "⚡"
type: "tech"
topics: ["機械学習", "分散学習", "Transformer", "GPU", "並列処理"]
published: false
free: true
---
...
Sequence Parallel と組み合わせる場合は、AllReduce を ReduceScatter + AllGather に分解し、ReduceScatter の結果（各 GPU が結果の一部のみを保持）をそのまま sp 形式 $[B, S/sp, D]$ として保持します。
:::
...
:::message alert
効率上 sp は tp と**同じデバイスグループ**で動作します（sp = tp）。これは、tp の行列演算の前後で AllGather/ReduceScatter によりテンソル形状を切り替えるためです。
:::
...
:::message
**端的に言うと**: MoE の各 Expert を異なる GPU に配置し、AllToAll でトークンをルーティングすることで、モデルを大規模化する。
:::
...
:::message
**端的に言うと**: 語彙次元を分割して並列計算し、ReduceScatter で集約することで、巨大な語彙サイズに対応する。
:::
